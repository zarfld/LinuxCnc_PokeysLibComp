...
int main() {
    ...
    if (module < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "fifousr main: rtapi_init returned %d\n", module);
        return -1;
    }
    ...
    if (fifo < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "fifousr main: rtapi_fifo_new returned %d\n", fifo);
        rtapi_exit(module);
        return -1;
    }
    ...
    while (1) {
        ...
        if (nchars <= 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "fifousr main: rtapi_fifo_read returned %d\n", nchars);
        } else {
            buffer[nchars] = '\0';
            rtapi_print_msg(RTAPI_MSG_INFO, "fifousr main: read %d chars: '%s'\n", nchars, buffer);
        }
    }
...
END:
    ...
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "fifousr main: rtapi_fifo_delete returned %d\n", retval);
        rtapi_exit(module);
        return -1;
    }
