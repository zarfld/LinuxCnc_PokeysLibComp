...
static int proc_read_modules(char *page, char **start, off_t off, int count, int *eof, void *data) {
    int n;
    char *state_str;
...
static int proc_read_tasks(char *page, char **start, off_t off, int count, int *eof, void *data) {
    int n;
    char *state_str;
    ...
    for (n = 1; n <= RTAPI_MAX_TASKS; n++) {
        if (task_array[n].state != EMPTY) {
            ...
            PROC_PRINT("%02d  %02d   %3d   %s  %p\n", n, task_array[n].owner, task_array[n].prio, state_str, task_array[n].taskcode);
        }
    }
    PROC_PRINT("\n");
    PROC_PRINT_DONE;
...
static int proc_read_shmem(char *page, char **start, off_t off, int count, int *eof, void *data) {
    int n;
    ...
    for (n = 1; n <= RTAPI_MAX_SHMEMS; n++) {
        if (shmem_array[n].key != 0) {
            PROC_PRINT("%02d  %2d/%-2d  %-10d  %-10lu\n", n, shmem_array[n].rtusers, shmem_array[n].ulusers, shmem_array[n].key, shmem_array[n].size);
        }
    }
    PROC_PRINT("\n");
    PROC_PRINT_DONE;
...
static int proc_read_sems(char *page, char **start, off_t off, int count, int *eof, void *data) {
    int n;
...
static int proc_read_fifos(char *page, char **start, off_t off, int count, int *eof, void *data) {
    int n;
    char *state_str;
