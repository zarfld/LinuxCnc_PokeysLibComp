...
int axis_init_hal_io(int mot_comp_id) {
    ...
    for (n = 0; n < EMCMOT_MAX_AXIS; n++) {
        ...
        CALL_CHECK(hal_pin_float_newf(HAL_OUT, &axis_data->external_offset_requested, mot_comp_id, "axis.%c.eoffset-request", c));
...
void axis_handle_jogwheels(bool motion_teleop_flag, bool motion_enable_flag, bool homing_is_active) {
    ...
    for (axis_num = 0; axis_num < EMCMOT_MAX_AXIS; axis_num++) {
        ...
        if ((*(axis_data->ajog_accel_fraction) > 1) || (*(axis_data->ajog_accel_fraction) < 0)) {
        ...
        if (axis->locking_joint >= 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "Cannot wheel jog a locking indexer AXIS_%c\n", "XYZABCUVW"[axis_num]);
            continue;
        }
        ...
        if (pos > axis->max_pos_limit) {
            break;
        }
        if (pos < axis->min_pos_limit) {
            break;
        }
...
void axis_sync_teleop_tp_to_carte_pos(int extfactor, double *pcmd_p[]) {
    ...
    for (n = 0; n < EMCMOT_MAX_AXIS; n++) {
        axis_array[n].teleop_tp.curr_pos = *pcmd_p[n] + extfactor * axis_array[n].ext_offset_tp.curr_pos;
    }
}
...
void axis_sync_carte_pos_to_teleop_tp(int extfactor, double *pcmd_p[]) {
    ...
    // expect extfactor =  -1 || 0 || +1
    for (n = 0; n < EMCMOT_MAX_AXIS; n++) {
        *pcmd_p[n] = axis_array[n].teleop_tp.curr_pos + extfactor * axis_array[n].ext_offset_tp.curr_pos;
    }
}
    ...
    for (n = 0; n < EMCMOT_MAX_AXIS; n++) {
        ...
        *(axis_data->external_offset) = axis->ext_offset_tp.curr_pos;
        ...
        if (!*(axis_data->eoffset_enable)) {
            ...
            if (last_eoffset_enable[n] && (fabs(*(axis_data->external_offset)) > ext_offset_epsilon) && motion_enable_flag && axis->ext_offset_tp.enable) {
                // to stdout only:
                rtapi_print_msg(RTAPI_MSG_NONE,
                                ...
                                "XYZABCUVW"[n], *(axis_data->external_offset), ext_offset_epsilon);
            }
...
void axis_check_constraints(double pos[], int failing_axes[]) {
    ...
    for (axis_num = 0; axis_num < EMCMOT_MAX_AXIS; axis_num += 1) {
        ...
        if ((fabs(pos[axis_num]) < eps) && (fabs(axis_array[axis_num].min_pos_limit) < eps) && (fabs(axis_array[axis_num].max_pos_limit) < eps)) {
            continue;
        }
    ...
    for (n = 0; n < EMCMOT_MAX_AXIS; n++) {
        ...
        if (*pcmd_p[n] >= axis->max_pos_limit) {
            ...
            *pcmd_p[n] = axis->max_pos_limit;
            ...
            axis->ext_offset_tp.curr_pos = axis->max_pos_limit - save_pos_cmd[n];
        ...
        if (*pcmd_p[n] <= axis->min_pos_limit) {
            *pcmd_p[n] = axis->min_pos_limit;
            axis->ext_offset_tp.curr_pos = axis->min_pos_limit - save_pos_cmd[n];
            if (axis->ext_offset_tp.pos_cmd < save_offset_cmd[n]) {
                axis->ext_offset_tp.pos_cmd = save_offset_cmd[n];
            }
            axis->ext_offset_tp.curr_vel = 0;
...
static int update_teleop_with_check(int axis_num, simple_tp_t *the_tp, double servo_period) {
    ...
    if ((axis->ext_offset_tp.curr_pos + axis->teleop_tp.curr_pos) >= axis->max_pos_limit) {
        // positive error, restore save_curr_pos
        the_tp->curr_pos = save_curr_pos;
        the_tp->curr_vel = 0;
        return 1;
    }
    if ((axis->ext_offset_tp.curr_pos + axis->teleop_tp.curr_pos) <= axis->min_pos_limit) {
