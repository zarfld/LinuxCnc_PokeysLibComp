 ...
 * @brief code to handle homing - originally in control.c, but moved out
...
/** @brief  internal states for homing */
typedef enum {
    HOME_IDLE = 0,
    HOME_START,                 // 1
    HOME_UNLOCK,                // 2
...
/** @brief  a couple of helper functions with code that would otherwise be
   repeated in several different states of the homing state machine */

/** @brief  'home_start_move()' starts a move at the specified velocity.  The
...
/** @brief  'home_do_moving_checks()' is called from states where the machine
...
static bool home_do_moving_checks(int jno) {
    /** @brief  check for limit switches */
    ...
    /** @brief  check for reached end of move */
    if (!(&joints[jno])->free_tp.active) {
        /** @brief  reached end of move without hitting switch */
...
static void set_all_unhomed(int unhome_method, motion_state_t motstate) {
    ...
    */
    ...
    /** @brief  we want all or none, so these checks need to all be done first.
    ...
    for (jno = 0; jno < all_joints; jno++) {
        ...
        if (GET_JOINT_ACTIVE_FLAG(joint)) {
            if (get_homing(jno)) {
                rtapi_print_msg(RTAPI_MSG_ERR, _("Cannot unhome while homing, joint %d"), jno);
                return;
            }
            if (!GET_JOINT_INPOS_FLAG(joint)) {
                rtapi_print_msg(RTAPI_MSG_ERR, _("Cannot unhome while moving, joint %d"), jno);
                return;
            }
        }
        if ((jno >= (all_joints - extra_joints)) // jno is extrajoint
            && (motstate != EMCMOT_MOTION_DISABLED)) {
            rtapi_print_msg(RTAPI_MSG_ERR, _("Cannot unhome extrajoint <%d> with motion enabled"), jno);
            return;
        }
    }
    /** @brief  we made it through the checks, so unhome them all per unhome_method */
    for (jno = 0; jno < all_joints; jno++) {
        joint = &joints[jno];
        if (GET_JOINT_ACTIVE_FLAG(joint)) {
            if ((unhome_method == -1) || ((unhome_method == -2) && (H[jno].volatile_home))) {
                H[jno].homed = 0;
            }
        }
    }
...
static int base_homing_init(int id, double servo_period, int njoints, int nextrajoints, emcmot_joint_t *pjoints) {
    int i;
    all_joints = njoints;
...
static void base_set_unhomed(int jno, motion_state_t motstate) {
    ...
    if (jno > all_joints) {
        /** @brief  invalid joint number specified */
        rtapi_print_msg(RTAPI_MSG_ERR, _("Cannot unhome invalid joint %d (max %d)\n"), jno, (all_joints - 1));
        return;
    }
    /** @brief  request was for one joint number */
    if ((jno >= (all_joints - extra_joints)) // jno is extrajoint
        && (motstate != EMCMOT_MOTION_DISABLED)) {
        rtapi_print_msg(RTAPI_MSG_ERR, _("Cannot unhome extrajoint <%d> with motion enabled\n"), jno);
        return;
    }
    joint = &joints[jno];
    if (GET_JOINT_ACTIVE_FLAG(joint)) {
        if (get_homing(jno)) {
            rtapi_print_msg(RTAPI_MSG_ERR, _("Cannot unhome while homing, joint %d\n"), jno);
            return;
        }
        if (!GET_JOINT_INPOS_FLAG(joint)) {
            rtapi_print_msg(RTAPI_MSG_ERR, _("Cannot unhome while moving, joint %d\n"), jno);
            return;
        }
        H[jno].homed = 0;
    } else {
        rtapi_print_msg(RTAPI_MSG_ERR, _("Cannot unhome inactive joint %d\n"), jno);
    }
} // base_set_unhomed()

static void base_set_joint_homing_params(int jno, double offset, double home, double home_final_vel, double home_search_vel, double home_latch_vel, int home_flags, int home_sequence, bool volatile_home) {
...
static void base_update_joint_homing_params(int jno, double offset, double home, int home_sequence) {
    H[jno].home_offset = offset;
    H[jno].home = home;
    H[jno].home_sequence = home_sequence;
    update_home_is_synchronized();
...
static bool base_get_allhomed(void) {
    ...
    for (joint_num = 0; joint_num < all_joints; joint_num++) {
        ...
        if (!GET_JOINT_ACTIVE_FLAG(joint)) {
            /** @brief  if joint is not active, don't even look at its limits */
            continue;
        }
        if (!get_homed(joint_num)) {
            /** @brief  if any of the joints is not homed return false */
            return 0;
        }
    }
    /** @brief  return true if all active joints are homed*/
    return 1;
} // base_get_allhomed()
...
static bool sync_ready(int joint_num) {
    ...
    if ((ABS(H[joint_num].home_sequence) == current_sequence) && !sync_now) {
        int jno;
        for (jno = 0; jno < all_joints; jno++) {
            if (!H[jno].joint_in_sequence) {
                continue;
            }
            if (ABS(H[jno].home_sequence) != current_sequence) {
                continue;
            }
            if (H[jno].home_flags & HOME_ABSOLUTE_ENCODER) {
                continue;
            }
...
*/
int homing_init(int id, double servo_period, int njoints, int nextrajoints, emcmot_joint_t *pjoints) {
    return base_homing_init(id, servo_period, njoints, nextrajoints, pjoints);
}

bool do_homing(void) {
    return base_do_homing();
}
bool get_allhomed(void) {
    return base_get_allhomed();
}
bool get_homed(int jno) {
    return base_get_homed(jno);
}
bool get_home_is_idle(int jno) {
    return base_get_home_is_idle(jno);
}
bool get_home_is_synchronized(int jno) {
    return base_get_home_is_synchronized(jno);
}
bool get_home_needs_unlock_first(int jno) {
    return base_get_home_needs_unlock_first(jno);
}
int get_home_sequence(int jno) {
    return base_get_home_sequence(jno);
}
bool get_homing(int jno) {
    return base_get_homing(jno);
}
bool get_homing_at_index_search_wait(int jno) {
    return base_get_homing_at_index_search_wait(jno);
}
bool get_homing_is_active(void) {
    return base_get_homing_is_active();
}
bool get_index_enable(int jno) {
    return base_get_index_enable(jno);
}
...
void set_joint_homing_params(int jno, double offset, double home, double home_final_vel, double home_search_vel, double home_latch_vel, int home_flags, int home_sequence, bool volatile_home) {
    base_set_joint_homing_params(jno, offset, home, home_final_vel, home_search_vel, home_latch_vel, home_flags, home_sequence, volatile_home);
}
