...
int rtapi_app_main(void) {
    ...
    for (i = 0; i < 8; i++) {
        retval = hal_pin_bit_newf(HAL_IN, &(port_data_array->signals[i]), comp_id, "speaker.%d.pin-%02d-out", n, i);
        if (retval < 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "SPEAKER: ERROR: port %d var export failed with err=%i\n", n, retval);
            hal_exit(comp_id);
            return -1;
        }
    }
    ...
    retval = hal_export_functf(write_port, &(port_data_array[n]), 0, 0, comp_id, "speaker.%d.write", n);
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SPEAKER: ERROR: port %d write funct export failed\n", n);
        hal_exit(comp_id);
        return -1;
    }

    rtapi_print_msg(RTAPI_MSG_INFO, "SPEAKER: installed driver for %d ports\n", num_ports);
    hal_ready(comp_id);
    return 0;
}
