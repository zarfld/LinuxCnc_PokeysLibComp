...
static int Device_ExportPinsParametersFunctions(Device *this, int componentId) {
    ...
    // Export DACs.
    if (!error)
        error = Device_ExportDacPinsParametersFunctions(this, componentId, boardId);

    // Export ADCs.
    if (!error)
        error = Device_ExportAdcPinsParametersFunctions(this, componentId, boardId);

    // Export digital I/O.
    if (!error)
        error = Device_ExportDigitalInPinsParametersFunctions(this, componentId, boardId);
    if (!error)
        error = Device_ExportDigitalOutPinsParametersFunctions(this, componentId, boardId);

    // Export miscellaneous.
    if (!error)
        error = Device_ExportMiscPinsParametersFunctions(this, componentId, boardId);
...
static int Device_ExportEncoderPinsParametersFunctions(Device *this, int componentId, int boardId) {
    ...
    for (channel = 0; channel < this->numFpga * MOTENC_FPGA_NUM_ENCODER_CHANNELS; channel++) {
        // Pins.
        if ((halError = hal_pin_s32_newf(HAL_OUT, &(this->encoder[channel].pCount), componentId, "motenc.%d.enc-%02d-count", boardId, channel)) != 0)
            break;

        if ((halError = hal_pin_float_newf(HAL_OUT, &(this->encoder[channel].pPosition), componentId, "motenc.%d.enc-%02d-position", boardId, channel)) != 0)
            break;

        if ((halError = hal_pin_bit_newf(HAL_OUT, &(this->encoder[channel].pIndex), componentId, "motenc.%d.enc-%02d-index", boardId, channel)) != 0)
            break;

        if ((halError = hal_pin_bit_newf(HAL_IO, &(this->encoder[channel].pIndexEnable), componentId, "motenc.%d.enc-%02d-index-enable", boardId, channel)) != 0)
            break;

        if ((halError = hal_pin_bit_newf(HAL_IN, &(this->encoder[channel].pReset), componentId, "motenc.%d.enc-%02d-reset", boardId, channel)) != 0)
            break;

        // Parameters.
        if ((halError = hal_param_float_newf(HAL_RW, &(this->encoder[channel].scale), componentId, "motenc.%d.enc-%02d-scale", boardId, channel)) != 0)
            break;
...
static int Device_ExportDacPinsParametersFunctions(Device *this, int componentId, int boardId) {
    ...
    for (channel = 0; channel < MOTENC_NUM_DAC_CHANNELS; channel++) {
        // Pins.
        if ((halError = hal_pin_float_newf(HAL_IN, &(this->dac[channel].pValue), componentId, "motenc.%d.dac-%02d-value", boardId, channel)) != 0)
            break;

        // Parameters.
        if ((halError = hal_param_float_newf(HAL_RW, &(this->dac[channel].offset), componentId, "motenc.%d.dac-%02d-offset", boardId, channel)) != 0)
            break;

        if ((halError = hal_param_float_newf(HAL_RW, &(this->dac[channel].gain), componentId, "motenc.%d.dac-%02d-gain", boardId, channel)) != 0)
            break;
...
static int Device_ExportAdcPinsParametersFunctions(Device *this, int componentId, int boardId) {
    ...
    for (channel = 0; channel < MOTENC_NUM_ADC_CHANNELS; channel++) {
        // Pins.
        if ((halError = hal_pin_float_newf(HAL_OUT, &(this->adc[channel].pValue), componentId, "motenc.%d.adc-%02d-value", boardId, channel)) != 0)
            break;

        // Parameters.
        if ((halError = hal_param_float_newf(HAL_RW, &(this->adc[channel].offset), componentId, "motenc.%d.adc-%02d-offset", boardId, channel)) != 0)
            break;

        if ((halError = hal_param_float_newf(HAL_RW, &(this->adc[channel].gain), componentId, "motenc.%d.adc-%02d-gain", boardId, channel)) != 0)
            break;
...
static int Device_ExportDigitalInPinsParametersFunctions(Device *this, int componentId, int boardId) {
    ...
    for (channel = 0; channel < (this->numFpga * MOTENC_FPGA_NUM_DIGITAL_INPUTS - 4); channel++) {
        // Pins.
        if ((halError = hal_pin_bit_newf(HAL_OUT, &(this->in[channel].pValue), componentId, "motenc.%d.in-%02d", boardId, channel)) != 0)
            break;

        if ((halError = hal_pin_bit_newf(HAL_OUT, &(this->in[channel].pValueNot), componentId, "motenc.%d.in-%02d-not", boardId, channel)) != 0)
            break;
...
static int Device_ExportDigitalOutPinsParametersFunctions(Device *this, int componentId, int boardId) {
    ...
    for (channel = 0; channel < this->numFpga * MOTENC_FPGA_NUM_DIGITAL_OUTPUTS; channel++) {
        // Pins.
        if ((halError = hal_pin_bit_newf(HAL_IN, &(this->out[channel].pValue), componentId, "motenc.%d.out-%02d", boardId, channel)) != 0)
            break;

        // Parameters.
        if ((halError = hal_param_bit_newf(HAL_RW, &(this->out[channel].invert), componentId, "motenc.%d.out-%02d-invert", boardId, channel)) != 0)
            break;
...
static int Device_ExportMiscPinsParametersFunctions(Device *this, int componentId, int boardId) {
    ...
    // Export Pins.
    halError = hal_pin_bit_newf(HAL_OUT, &(this->misc.pEstopIn), componentId, "motenc.%d.estop-in", boardId);

    if (!halError) {
        halError = hal_pin_bit_newf(HAL_OUT, &(this->misc.pEstopInNot), componentId, "motenc.%d.estop-in-not", boardId);
    }

    if (!halError) {
        halError = hal_pin_bit_newf(HAL_IO, &(this->misc.pWatchdogReset), componentId, "motenc.%d.watchdog-reset", boardId);
    }

    // Export Parameters.
    if (!halError) {
        halError = hal_param_u32_newf(HAL_RW, &(this->misc.watchdogControl), componentId, "motenc.%d.watchdog-control", boardId);
    }
...
static void Device_EncoderRead(void *arg, long period) {
...
static void Device_DacWrite(void *arg, long period) {
    Device *this = (Device *)arg;
    MotencRegMap *pCard = this->pCard;
    DacPinsParams *pDac;
    int i;
    ...
    for (i = 0; i < MOTENC_NUM_DAC_CHANNELS; i++, pDac++) {
        ...
        dacCount = (hal_u32_t)(volts * MOTENC_DAC_SCALE_MULTIPLY / MOTENC_DAC_SCALE_DIVIDE + MOTENC_DAC_COUNT_ZERO);
...
static void Device_AdcRead(void *arg, long period) {
    Device *this = (Device *)arg;
    MotencRegMap *pCard = this->pCard;
...
static void Device_DigitalInRead(void *arg, long period) {
    Device *this = (Device *)arg;
    MotencRegMap *pCard = this->pCard;
...
static void Device_DigitalOutWrite(void *arg, long period) {
    Device *this = (Device *)arg;
    MotencRegMap *pCard = this->pCard;
...
static void Device_MiscUpdate(void *arg, long period) {
    Device *this = (Device *)arg;
    MotencRegMap *pCard = this->pCard;
