...
static int export(char *prefix, struct port *inst, int ioaddr, int dir) {
    ...
    if (inst->dir_ & 8)
        ...
        direction = HAL_IN;
    for (i = 0; i < 8; i++) {
        r = hal_pin_bit_newf(direction, &(inst->a[i]), comp_id, "%s.a%d", prefix, i);
        if (r != 0)
            return r;
        if (direction == HAL_OUT) {
            r = hal_pin_bit_newf(direction, &(inst->ai[i].not_), comp_id, "%s.a%d-not", prefix, i);
        } else {
            r = hal_param_bit_newf(HAL_RW, &(inst->ai[i].invert), comp_id, "%s.a%d-invert", prefix, i);
        }
        if (r != 0)
            return r;
    }
    if (inst->dir_ & 2)
        ...
        direction = HAL_IN;
    for (i = 0; i < 8; i++) {
        r = hal_pin_bit_newf(direction, &(inst->b[i]), comp_id, "%s.b%d", prefix, i);
        if (r != 0)
            return r;
        if (direction == HAL_OUT) {
            r = hal_pin_bit_newf(direction, &(inst->bi[i].not_), comp_id, "%s.b%d-not", prefix, i);
        } else {
            r = hal_param_bit_newf(HAL_RW, &(inst->bi[i].invert), comp_id, "%s.b%d-invert", prefix, i);
        }
        if (r != 0)
            return r;
    }
    for (i = 0; i < 8; i++) {
        ...
        r = hal_pin_bit_newf(direction, &(inst->c[i]), comp_id, "%s.c%d", prefix, i);
        if (r != 0)
            return r;
        if (direction == HAL_OUT) {
            r = hal_pin_bit_newf(direction, &(inst->ci[i].not_), comp_id, "%s.c%d-not", prefix, i);
        } else {
            r = hal_param_bit_newf(HAL_RW, &(inst->ci[i].invert), comp_id, "%s.c%d-invert", prefix, i);
        }
        if (r != 0)
            return r;
    }
    r = hal_param_u32_newf(HAL_RO, &(inst->dir_), comp_id, "%s.dir", prefix);
    if (r != 0)
        return r;
...
int rtapi_app_main(void) {
    ...
    for (i = 0; i < count; i++) {
        ...
        for (j = 0; j < 3; j++) {
            ...
            r = export(buf, &inst[i].ports[j], io[i] + 0xc0 + 16 * j, (dir[i] >> (4 * j)) & 0xf);
            if (r != 0)
                goto out_error;
        }
        hal_pin_bit_newf(HAL_IN, &(inst[i].relay), comp_id, "pci8255.%d.relay", i);
        hal_param_bit_newf(HAL_RW, &(inst[i].relay_invert), comp_id, "pci8255.%d.relay-invert", i);
        r = hal_export_functf((void (*)(void *inst, long))write_relay, &inst[i], 0, 0, comp_id, "pci8255.%d.write-relay", i);
        r = hal_param_u32_newf(HAL_RO, &(inst->ioaddr), comp_id, "pci8255.%d.io-addr", i);
        inst->ioaddr = io[i];
        if (r != 0)
            return r;
    }
...
static void write(struct port *inst, long period) {
    ...
    if ((p & 5) == 0) {
        ...
        if (first)
            rtapi_print_msg(RTAPI_MSG_DBG, "write: 2a %02x\n", byte);
    } else if ((p & 5) == 4) {
        int byte = 0;
        for (i = 0; i < 4; i++) {
            int t = (c(i) != 0) ^ (ci_invert(i) != 0);
        ...
        pci_8255_outb(byte, ioaddr, 2);
        if (first)
            rtapi_print_msg(RTAPI_MSG_DBG, "write: 2b %02x\n", byte);
    } else if ((p & 5) == 1) {
        int byte = 0;
        for (i = 4; i < 8; i++) {
            int t = (c(i) != 0) ^ (ci_invert(i) != 0);
        ...
        pci_8255_outb(byte, ioaddr, 2);
        if (first)
            rtapi_print_msg(RTAPI_MSG_DBG, "write: 2c %02x\n", byte);
    }
    ...
    if ((p & 2) == 0) {
        ...
        if (first)
            rtapi_print_msg(RTAPI_MSG_DBG, "write: 1 %02x\n", byte);
    }
    ...
    if ((p & 8) == 0) {
        ...
        if (first)
            rtapi_print_msg(RTAPI_MSG_DBG, "write: 0 %02x\n", byte);
    }
    first = 0;
}
