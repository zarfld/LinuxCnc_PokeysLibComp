...
int rtapi_app_main(void) {
    ...
    for (n = 0; n < MAX_BOARDS; n++) {
        ...
        if (pboard->base == NULL) {
            rtapi_print_msg(RTAPI_MSG_ERR, "ERROR OPTO_AC5---  could not find board %d PCI base address\n", pboard->num);
            rtapi_app_exit();
            return -1;
        } else {
            rtapi_print("INFO OPTO_AC5--- board %d mapped to %08lx, Len = %ld\n", pboard->num, (long)pboard->base, (long)pboard->len);
        }
        // Initialize device.
...
static int Device_Init(board_data_t *pboard) {
    ...
    if ((portconfig0 & 0x80000000) == 0) {
        portconfig0 |= 0x80000000;
    }
    if ((portconfig0 & 0x40000000) == 0) {
        portconfig0 |= 0x40000000;
    }
    if ((portconfig1 & 0x80000000) == 0) {
        portconfig1 |= 0x80000000;
    }
    if ((portconfig1 & 0x40000000) == 0) {
        portconfig1 |= 0x40000000;
    }
...
static int Device_ExportDigitalInPinsParametersFunctions(board_data_t *this, int comp_id, int boardId) {
    ...
    while (portnum < 2) {
        ...
        for (channel = 0; channel < 24; channel++) {
            ...
            {
                // Pins.
                if ((halError = hal_pin_bit_newf(HAL_OUT, &(this->port[portnum].io[channel].pValue), comp_id, "opto-ac5.%d.port%d.in-%02d", boardId, portnum, channel)) != 0)
                    break;

                if ((halError = hal_pin_bit_newf(HAL_OUT, &(this->port[portnum].io[channel].pValueNot), comp_id, "opto-ac5.%d.port%d.in-%02d-not", boardId, portnum, channel)) != 0)
                    break;
...
static int Device_ExportDigitalOutPinsParametersFunctions(board_data_t *this, int comp_id, int boardId) {
    ...
    while (portnum < 2) {
        ...
        for (channel = 0; channel < 24; channel++) {
            ...
            {
                // Pins.
                if ((halError = hal_pin_bit_newf(HAL_IN, &(this->port[portnum].io[channel].pValue), comp_id, "opto-ac5.%d.port%d.out-%02d", boardId, portnum, channel)) != 0)
                    break;

                // Parameters.
                if ((halError = hal_param_bit_newf(HAL_RW, &(this->port[portnum].io[channel].invert), comp_id, "opto-ac5.%d.port%d.out-%02d-invert", boardId, portnum, channel)) != 0)
                    break;
    ...
    for (channel = 0; channel < 2; channel++) {
        if ((halError = hal_pin_bit_newf(HAL_IN, &(this->port[portnum].io[24].pValue), comp_id, "opto-ac5.%d.led%d", boardId, channel + portnum)) != 0)
            break;

        if ((halError = hal_pin_bit_newf(HAL_IN, &(this->port[portnum].io[25].pValue), comp_id, "opto-ac5.%d.led%d", boardId, channel + portnum + 1)) != 0)
            break;
        portnum++;
    }
    // Export functions.
    ...
    while (portnum < 2) {
        ...
        for (j = 0; j < 24; j++, pDigital++) {
            if ((pboard->port[portnum].mask & mask) != 0) // is it an output?
            {
                ...
                if ((!*(pDigital->pValue) && !(pDigital->invert)) || (*(pDigital->pValue) && (pDigital->invert))) {
                    pins |= mask;
                }
            }
            mask <<= 1; // shift mask
