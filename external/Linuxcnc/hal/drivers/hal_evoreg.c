...
#define ISA_MAX 0x100000 /* allgemeiner Speicherzugriff */
    ...
    if (comp_id < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "EVOREG: ERROR: hal_init() failed\n");
        return -1;
    }
    ...
    if (port_data_array == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "EVOREG: ERROR: hal_malloc() failed\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    for (num_dac = 1; num_dac <= MAX_DAC; num_dac++) {
        retval = hal_pin_float_newf(HAL_IN, &(port_data_array->dac_out[num_dac - 1]), comp_id, "evoreg.%d.dac-%02d-out", 1, num_dac);
        if (retval < 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "EVOREG: ERROR: port %d var export failed with err=%i\n", n + 1, retval);
            hal_exit(comp_id);
            return -1;
        }
    }
    ...
    for (num_enc = 1; num_enc <= MAX_ENC; num_enc++) {
        retval = hal_pin_float_newf(HAL_OUT, &(port_data_array->position[num_enc - 1]), comp_id, "evoreg.%d.position-%02d-in", 1, num_enc);
        if (retval < 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "EVOREG: ERROR: port %d var export failed with err=%i\n", n + 1, retval);
            hal_exit(comp_id);
            return -1;
    ...
    for (i = 0; i <= 45; i++) {
        retval += hal_pin_bit_newf(HAL_OUT, &(port_data_array->digital_in[i]), comp_id, "evoreg.%d.pin-%02d-in", 1, i);
    ...
    for (i = 0; i <= 23; i++) {
        retval += hal_pin_bit_newf(HAL_IN, &(port_data_array->digital_out[i]), comp_id, "evoreg.%d.pin-%02d-out", 1, i);
    ...
    retval = hal_param_float_newf(HAL_RW, &(port_data_array->pos_scale), comp_id, "evoreg.%d.position-scale", 1);
    if (retval != 0) {
        return retval;
    }

    /* STEP 4: export function */
    retval = hal_export_functf(update_port, &(port_data_array[n]), 1, 0, comp_id, "evoreg.%d.update", n + 1);
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "EVOREG: ERROR: port %d write funct export failed\n", n + 1);
        hal_exit(comp_id);
        return -1;
    }

    rtapi_print_msg(RTAPI_MSG_INFO, "EVOREG: installed driver for %d card(s)\n", num_ports);
    hal_ready(comp_id);
    return 0;
}
