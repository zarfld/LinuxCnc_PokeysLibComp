...
MODULE_DESCRIPTION("Driver for Vigilant Technologies ENCDAC 4 channel controller");
MODULE_LICENSE("GPL");
...
int rtapi_app_main(void) {
    ...
    if ((num_chan <= 0) || (num_chan > MAX_CHAN)) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: invalid num_chan: %d\n", num_chan);
        return -1;
    }
    ...
    if ((retval = vti_init_card()) != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: vti_init_card() failed\n");
        hal_exit(comp_id);
        return retval;
    }
    ...
    if (diocount == -1) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: bad config info for port.\n");
        return -1;
    }
    ...
    if (vti_counter_init(num_chan) == -1) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: bad config info counter.\n");
        return -1;
    }
    ...
    /* export functions */
    retval = hal_export_funct("vti.capture-position", vti_counter_capture, vti_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: vti.counter-capture funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "VTI: installed %d encoder counters\n", num_chan);

    retval = hal_export_funct("vti.write-dacs", vti_dacs_write, vti_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: vti.write-dacs funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "VTI: installed %d dacs\n", num_chan);

    retval = hal_export_funct("vti.read-adcs", vti_adcs_read, vti_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: vti.read-adcs funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "VTI: installed %d adcs\n", 0);

    retval = hal_export_funct("vti.di-read", vti_di_read, vti_driver, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: vti.di-read funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }

    rtapi_print_msg(RTAPI_MSG_INFO, "VTI: installed %d digital inputs\n", inputpinnum);

    retval = hal_export_funct("vti.do-write", vti_do_write, vti_driver, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: vti.do-write funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "VTI: installed %d digital outputs\n", outpinnum);
...
static void vti_dacs_write(void *arg, long period) {
    ...
    for (i = 0; i < num_chan; i++) {
        ...
        volts = (*(vti->dac_value[i]) - vti->dac_offset[i]) * vti->dac_gain[i];
...
static void vti_di_read(void *arg, long period) // reads digital inputs from the vti
{
    ...
    if (diocount <= 8)
        return; // No extended I/O enabled
...
static int vti_counter_init(int counters) {
    ...
    for (i = 0; i < counters; i++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: counter %d var export failed\n", i + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
...
static int vti_dac_init(int channels) {
    ...
    for (i = 0; i < channels; i++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "VTI: ERROR: dac %d var export failed\n", i + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
...
static long vti_counter_read(int axis) {
    ...
    if ((EncData.Long - lastCount) > 0x7fff)
        EncData.Word[1] -= 1;
    else if ((lastCount - EncData.Long) > 0x7fff)
        EncData.Word[1] += 1;
...
static int vti_init_card() {
    ...
    if (retval == 0) {
        encoder = (volatile struct encoder *)ioremap(pci_resource_start(dev, 2), sizeof(encoder));
        dac = (volatile struct dac *)ioremap(pci_resource_start(dev, 4), sizeof(dac));
        timer = (volatile struct timer *)ioremap(pci_resource_start(dev, 3), sizeof(timer));
        ip = (volatile struct ip *)ioremap(pci_resource_start(dev, 5), sizeof(ip));
    } else {
        return (retval);
    }
...
static int vti_autodetect() {
    ...
    if (dev) {
        pci_dev_put(dev);
        rtapi_print_msg(RTAPI_MSG_INFO, "VTI: Card detected in slot: %2x\n", PCI_SLOT(dev->devfn));
        return (0);
    } else {
...
static int export_counter(int num, vti_struct *addr) {
    ...
    retval = hal_pin_s32_newf(HAL_OUT, &addr->count[num], comp_id, "vti.%d.counts", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled position captured by update() */
    retval = hal_pin_float_newf(HAL_OUT, &addr->pos[num], comp_id, "vti.%d.position", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for scaling */
    retval = hal_param_float_newf(HAL_RW, &addr->pos_scale[num], comp_id, "vti.%d.position-scale", num);
    if (retval != 0) {
        return retval;
    }
...
static int export_dac(int num, vti_struct *addr) {
    ...
    retval = hal_pin_float_newf(HAL_IN, &addr->dac_value[num], comp_id, "vti.%d.dac-value", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for offset */
    retval = hal_param_float_newf(HAL_RW, &addr->dac_offset[num], comp_id, "vti.%d.dac-offset", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for gain */
    retval = hal_param_float_newf(HAL_RW, &addr->dac_gain[num], comp_id, "vti.%d.dac-gain", num);
    if (retval != 0) {
        return retval;
    }
...
static int export_pin(int num, int dir, vti_struct *addr) {
    int retval;
    if (dir != 0)
        retval = export_output_pin(outpinnum++, &(addr->port[num / 8][num % 8]));
    else
        retval = export_input_pin(inputpinnum++, &(addr->port[num / 8][num % 8]));
    if (retval != 0)
        return retval;
    return 0;
}
static int export_input_pin(int pinnum, io_pin *pin) {
    ...
    retval = hal_pin_bit_newf(HAL_OUT, &(pin->data), comp_id, "vti.in-%02d", pinnum);
    if (retval != 0)
        return retval;
    /* export additional pin for inverted input data */
    retval = hal_pin_bit_newf(HAL_OUT, &(pin->io.not ), comp_id, "vti.in-%02d-not", pinnum);
    /* initialize HAL pins */
    *(pin->data) = 0;
    *(pin->io.not ) = 1;
    return retval;
...
static int export_output_pin(int pinnum, io_pin *pin) {
    ...
    retval = hal_pin_bit_newf(HAL_IN, &(pin->data), comp_id, "vti.out-%02d", pinnum);
    if (retval != 0)
        return retval;
    /* export parameter for polarity */
    retval = hal_param_bit_newf(HAL_RW, &(pin->io.invert), comp_id, "vti.out-%02d-invert", pinnum);
