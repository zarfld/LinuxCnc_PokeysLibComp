...
int rtapi_app_main(void) {
    ...
    if ((num_chan <= 0) || (num_chan > MAX_CHAN)) {
        rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: invalid num_chan: %d\n", num_chan);
        return -1;
    }
    ...
    if ((retval = stg_init_card()) != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: stg_init_card() failed\n");
        hal_exit(comp_id);
        return retval;
    }
    ...
    for (m = 0; m < 4; m++) {
        ...
        if ((dio[m] == 'i') || (dio[m] == 'I')) {
            ...
            rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: bad config info for port %d\n", m);
            return -1;
        }
    ...
    for (n = 0; n < num_chan; n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: counter %d var export failed\n", n + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
        ...
        *(stg_driver->index_latch[n]) = 0;
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: dac %d var export failed\n", n + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
        *(stg_driver->dac_value[n]) = 0;
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: adc %d var export failed\n", n + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
    ...
    /* export functions */
    retval = hal_export_funct("stg.capture-position", stg_counter_capture, stg_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: stg.counter-capture funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "STG: installed %d encoder counters\n", num_chan);

    retval = hal_export_funct("stg.write-dacs", stg_dacs_write, stg_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: stg.write-dacs funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "STG: installed %d dacs\n", num_chan);

    retval = hal_export_funct("stg.read-adcs", stg_adcs_read, stg_driver, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: stg.read-adcs funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "STG: installed %d adcs\n", num_chan);

    retval = hal_export_funct("stg.di-read", stg_di_read, stg_driver, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: stg.di-read funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    retval = hal_export_funct("stg.do-write", stg_do_write, stg_driver, 0, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "STG: ERROR: stg.do-write funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "STG: installed %d digital outputs\n", outpinnum);
...
static int stg_init_card() {
    ...
    if (stg_driver->model == 1) {
        ...
        outb(0xff, base + OCW1); // mask off all interrupts
        rtapi_print_msg(RTAPI_MSG_INFO, "STG: Initialised stg%1d card at address %x\n", stg_driver->model, base);
...
unsigned short stg_autodetect() {
    ...
    for (i = 15; i >= 0; i--) {
        ...
        if (k == 0x75) {
            rtapi_print_msg(RTAPI_MSG_INFO, "STG: Autodetected stg1 card at address %x\n", address);
            stg_driver->model = 1;
        ...
        if (k == 0x74) {
            rtapi_print_msg(RTAPI_MSG_INFO, "STG: Autodetected stg2 card at address %x\n", address);
            stg_driver->model = 2;
            retval = address;
            break;
        }
    ...
    if ((retval == 0) || (stg_driver->model == 0)) {
        rtapi_print_msg(RTAPI_MSG_ERR, "STG: stg_autodetect() did not find any stg1 or stg2 card\n");
    }
...
static int export_counter(int num, stg_struct *addr) {
    ...
    retval = hal_pin_s32_newf(HAL_OUT, &addr->count[num], comp_id, "stg.%d.counts", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled position captured by update() */
    retval = hal_pin_float_newf(HAL_OUT, &addr->pos[num], comp_id, "stg.%d.position", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for scaling */
    retval = hal_param_float_newf(HAL_RW, &addr->pos_scale[num], comp_id, "stg.%d.position-scale", num);
    if (retval != 0) {
        return retval;
    }

    /* export pin for index homing */
    retval = hal_pin_bit_newf(HAL_IO, &addr->index_enable[num], comp_id, "stg.%d.index-enable", num);
    if (retval != 0) {
        return retval;
    }

    /* export pin for reading the index latch */
    retval = hal_pin_bit_newf(HAL_OUT, &addr->index_latch[num], comp_id, "stg.%d.index-latch", num);
    if (retval != 0) {
        return retval;
    }
    ...
    if (addr->model == 1) {
        ...
        retval = hal_pin_bit_newf(HAL_IN, &addr->index_polarity[num], comp_id, "stg.%d.index-polarity", num);
        if (retval != 0) {
            return retval;
        }
    }
...
static int export_dac(int num, stg_struct *addr) {
    ...
    retval = hal_pin_float_newf(HAL_IN, &addr->dac_value[num], comp_id, "stg.%d.dac-value", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for offset */
    retval = hal_param_float_newf(HAL_RW, &addr->dac_offset[num], comp_id, "stg.%d.dac-offset", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for gain */
    retval = hal_param_float_newf(HAL_RW, &addr->dac_gain[num], comp_id, "stg.%d.dac-gain", num);
    if (retval != 0) {
        return retval;
    }
...
static int export_adc(int num, stg_struct *addr) {
    ...
    /* export pin for voltage received by the board() */
    retval = hal_pin_float_newf(HAL_OUT, &addr->adc_value[num], comp_id, "stg.%d.adc-value", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for offset */
    retval = hal_param_float_newf(HAL_RW, &addr->adc_offset[num], comp_id, "stg.%d.adc-offset", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for gain */
    retval = hal_param_float_newf(HAL_RW, &addr->adc_gain[num], comp_id, "stg.%d.adc-gain", num);
    if (retval != 0) {
        return retval;
    }
    /* restore saved message level */
...
static int export_input_pin(int pinnum, io_pin *pin) {
    ...
    retval = hal_pin_bit_newf(HAL_OUT, &(pin->data), comp_id, "stg.in-%02d", pinnum);
    if (retval != 0) {
        return retval;
    }
    /* export additional pin for inverted input data */
    retval = hal_pin_bit_newf(HAL_OUT, &(pin->io.not ), comp_id, "stg.in-%02d-not", pinnum);
    /* initialize HAL pins */
    *(pin->data) = 0;
    *(pin->io.not ) = 1;
...
static int export_output_pin(int pinnum, io_pin *pin) {
    ...
    retval = hal_pin_bit_newf(HAL_IN, &(pin->data), comp_id, "stg.out-%02d", pinnum);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for polarity */
    retval = hal_param_bit_newf(HAL_RW, &(pin->io.invert), comp_id, "stg.out-%02d-invert", pinnum);
