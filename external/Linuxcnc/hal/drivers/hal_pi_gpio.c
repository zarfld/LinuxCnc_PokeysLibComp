...
static int setup_gpiomem_access(void) {
    if ((mem_fd = open("/dev/gpiomem", O_RDWR | O_SYNC)) < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "HAL_PI_GPIO: can't open /dev/gpiomem:  %d - %s\n"
...
int rtapi_app_main(void) {
        ...
        default: // This will need to change if there is a V3 pinout
            ...
            if (rev > 20) { // Rev 20 is Compute Module 4
                ...
                rtapi_print_msg(RTAPI_MSG_INFO,
                                "The Pi model %i is not known to "
    ...
    if (port_data == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL_PI_GPIO: ERROR: hal_malloc() failed\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    if (*endptr) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL_PI_GPIO: dir=%s - trailing garbage: '%s'\n", dir, endptr);
        return -1;
    }
    ...
    if (*endptr) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL_PI_GPIO: exclude=%s - trailing garbage: '%s'\n", exclude, endptr);
        return -1;
    }
    ...
    for (n = 0; n < npins; n++) {
        ...
        if (dir_map & RTAPI_BIT(n)) {
            ...
            if ((retval = hal_pin_bit_newf(HAL_IN, &port_data[n], comp_id, "hal_pi_gpio.pin-%02d-out", pinno)) < 0)
                break;
        } else {
            bcm2835_gpio_fsel(gpios[n], BCM2835_GPIO_FSEL_INPT);
            if ((retval = hal_pin_bit_newf(HAL_OUT, &port_data[n], comp_id, "hal_pi_gpio.pin-%02d-in", pinno)) < 0)
                break;
        }
    }
    if (retval < 0) {
