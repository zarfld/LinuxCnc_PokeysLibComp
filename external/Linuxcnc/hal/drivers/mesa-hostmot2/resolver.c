...
int hm2_resolver_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_resolvers > (md->instances * resolvers_per_instance)) {
        HM2_ERR("config.num_resolvers=%d, but only %d are available, not loading driver\n", hm2->config.num_resolvers, md->instances * resolvers_per_instance);
        return -EINVAL;
    }
    ...
    hm2->resolver.hal = (hm2_resolver_global_t *)hal_malloc(sizeof(hm2_resolver_global_t));
    if (hm2->resolver.hal == NULL) {
        HM2_ERR("out of memory!\n");
        r = -ENOMEM;
        goto fail0;
    }
    hm2->resolver.instance = (hm2_resolver_instance_t *)hal_malloc(hm2->resolver.num_resolvers * sizeof(hm2_resolver_instance_t));
    ...
    for (i = 0; i < hm2->resolver.num_instances; i++) {
        ...
        r = hm2_register_tram_read_region(hm2, hm2->resolver.status_addr, sizeof(rtapi_u32), &hm2->resolver.status_reg);
        r += hm2_register_tram_read_region(hm2, hm2->resolver.position_addr, (hm2->resolver.num_resolvers * sizeof(rtapi_u32)), &hm2->resolver.position_reg);
        r += hm2_register_tram_read_region(hm2, hm2->resolver.velocity_addr, (hm2->resolver.num_resolvers * sizeof(rtapi_u32)), (rtapi_u32 **)&hm2->resolver.velocity_reg);
        if (r < 0) {
...

    {
        ...
        rtapi_snprintf(name, sizeof(name), "%s.resolver.excitation-khz", hm2->llio->name);
        ret = hal_param_float_new(name, HAL_RW, &(hm2->resolver.hal->param.excitation_khz), hm2->llio->comp_id);
        if (ret < 0) {
        ...
        for (i = 0; i < hm2->resolver.num_resolvers; i++) {
            ...
            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.position", hm2->llio->name, i);
            ret = hal_pin_float_new(name, HAL_OUT, &(hm2->resolver.instance[i].hal.pin.position), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.angle", hm2->llio->name, i);
            ret = hal_pin_float_new(name, HAL_OUT, &(hm2->resolver.instance[i].hal.pin.angle), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.velocity", hm2->llio->name, i);
            ret = hal_pin_float_new(name, HAL_OUT, &(hm2->resolver.instance[i].hal.pin.velocity), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.velocity-rpm", hm2->llio->name, i);
            ret = hal_pin_float_new(name, HAL_OUT, &(hm2->resolver.instance[i].hal.pin.velocity_rpm), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.count", hm2->llio->name, i);
            ret = hal_pin_s32_new(name, HAL_OUT, &(hm2->resolver.instance[i].hal.pin.count), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.rawcounts", hm2->llio->name, i);
            ret = hal_pin_s32_new(name, HAL_OUT, &(hm2->resolver.instance[i].hal.pin.rawcounts), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.reset", hm2->llio->name, i);
            ret = hal_pin_bit_new(name, HAL_IN, &(hm2->resolver.instance[i].hal.pin.reset), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.index-enable", hm2->llio->name, i);
            ret = hal_pin_bit_new(name, HAL_IO, &(hm2->resolver.instance[i].hal.pin.index_enable), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.error", hm2->llio->name, i);
            ret = hal_pin_bit_new(name, HAL_OUT, &(hm2->resolver.instance[i].hal.pin.error), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.joint-pos-fb", hm2->llio->name, i);
            ret = hal_pin_float_new(name, HAL_IN, &(hm2->resolver.instance[i].hal.pin.joint_pos_fb), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }
            ...
            // parameters
            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.scale", hm2->llio->name, i);
            ret = hal_param_float_new(name, HAL_RW, &(hm2->resolver.instance[i].hal.param.scale), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.velocity-scale", hm2->llio->name, i);
            ret = hal_param_float_new(name, HAL_RW, &(hm2->resolver.instance[i].hal.param.vel_scale), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.index-divisor", hm2->llio->name, i);
            ret = hal_param_u32_new(name, HAL_RW, &(hm2->resolver.instance[i].hal.param.index_div), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            }

            rtapi_snprintf(name, sizeof(name), "%s.resolver.%02d.use-position-file", hm2->llio->name, i);
            ret = hal_param_bit_new(name, HAL_RW, &(hm2->resolver.instance[i].hal.param.use_abs), hm2->llio->comp_id);
            if (ret < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            }
    ...
    for (i = 0; i < hm2->resolver.num_resolvers; i++) {
        ...
        if ((res->old_reg > hm2->resolver.position_reg[i]) && (res->old_reg - hm2->resolver.position_reg[i] > 0x80000000)) {
            ...
            if (*res->hal.pin.index_enable) {
                ...
                if ((res->hal.param.index_div > 1 && r == 1) || (res->hal.param.index_div == 1 && r == 0)) {
        ...
        *res->hal.pin.position = (res->accum - res->offset) / 0x1P32 * res->hal.param.scale;
        *res->hal.pin.velocity = ((hm2->resolver.velocity_reg[i] / 0x1P32) * hm2->resolver.kHz * res->hal.param.vel_scale);
...
void hm2_resolver_write(hostmot2_t *hm2, long period) {
    ...
    switch (state) {
        ...
        case 10: // wait for command register clear before setting params
            hm2->llio->read(hm2->llio, hm2->resolver.command_addr, &buff, sizeof(rtapi_u32));
            if (buff) {
                timer += period;
                if (timer > 1e9) {
            ...
            hm2->llio->write(hm2->llio, hm2->resolver.data_addr, &data_val, sizeof(rtapi_u32));
            hm2->llio->write(hm2->llio, hm2->resolver.command_addr, &cmd_val, sizeof(rtapi_u32));
            state = 20;
            timer = 0;
            return;
...
void hm2_resolver_print_module(hostmot2_t *hm2) {
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n", hm2->resolver.clock_frequency, hm2_hz_to_mhz(hm2->resolver.clock_frequency));
