...
static void hm2_xy2mod_instance_write(hostmot2_t *hm2, int i) {
    ...
    if (*s->hal.pin.enable != 0) {
        ...
        *s->hal.pin.controly = (*s->hal.pin.controly & 7);
        ...
        hm2->xy2mod.mode_reg[i] = *s->hal.pin.controlx << 0 | *s->hal.pin.controly << 3 | *s->hal.pin.mode18bitx << 10 | *s->hal.pin.mode18bity << 11 | *s->hal.pin.commandmodex << 12 | *s->hal.pin.commandmodey << 13;
        ...
        *s->hal.pin.commandy = (*s->hal.pin.commandy & 0xFFFF);
        ...
        hm2->xy2mod.command_reg[i] = *s->hal.pin.commandx << 0 | *s->hal.pin.commandy << 16;
...
void hm2_xy2mod_allocate_pins(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        if ((hm2->pin[i].sec_tag != HM2_GTAG_XY2MOD) || (hm2->pin[i].sec_unit >= hm2->xy2mod.num_instances)) {
            continue;
        }
...
int hm2_xy2mod_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_xy2mods > md->instances) {
        HM2_ERR("config.num_xy2mods=%d, but only %d are available, not loading driver\n", hm2->config.num_xy2mods, md->instances);
        return -EINVAL;
    }
