...
static rpi5_port_t spi_ports[PORT_MAX] = {
    { .spix = { .width = 32, .miso_pull = SPI_PULL_MISO_DEF, .name = "SPI0/CE0", .transfer = spi_transfer }, .cemask = (1 << 0) }, { .spix = { .width = 32, .miso_pull = SPI_PULL_MISO_DEF, .name = "SPI0/CE1", .transfer = spi_transfer }, .cemask = (1 << 1) }, { .spix = { .width = 32, .miso_pull = SPI_PULL_MISO_DEF, .name = "SPI1/CE0", .transfer = spi_transfer }, .cemask = (1 << 0) },
    { .spix = { .width = 32, .miso_pull = SPI_PULL_MISO_DEF, .name = "SPI1/CE1", .transfer = spi_transfer }, .cemask = (1 << 1) }, { .spix = { .width = 32, .miso_pull = SPI_PULL_MISO_DEF, .name = "SPI1/CE2", .transfer = spi_transfer }, .cemask = (1 << 2) },
};
...
static int peripheral_map(uintptr_t membase, size_t memsize) {
    ...
    spi_ports[0].port = spi_ports[1].port = (dw_ssi_t *)((void *)((uintptr_t)peripheralmem + RP1_SPI0_OFFSET));
    spi_ports[2].port = spi_ports[3].port = spi_ports[4].port = (dw_ssi_t *)((void *)((uintptr_t)peripheralmem + RP1_SPI1_OFFSET));
