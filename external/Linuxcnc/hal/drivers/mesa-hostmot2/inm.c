...
int hm2_inm_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_inms > md->instances) {
        HM2_ERR("config.num_inms=%d, but only %d are available, not loading driver\n", hm2->config.num_inms, md->instances);
        return -EINVAL;
    }
...
void hm2_inm_force_write(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->inm.num_instances; i++) {
        ...
        hm2->inm.control_reg[i] = (1 << 5) +
                                  ...
                                  (divisor << 6) + (hm2->inm.instance[i].hal.param.fast_scans << 16) + (hm2->inm.instance[i].hal.param.slow_scans << 22);
    }
...
void hm2_inm_write(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->inm.num_instances; i++) {
        ...
        if ((divisor > 1023) | (scanrate == 0)) {
            divisor = 1023;
            hm2->inm.instance[i].hal.param.scan_rate = (hm2->inm.clock_frequency / 4) / (divisor + 1) / hm2->inm.instance[i].scanwidth;
        ...
        if (hm2->inm.instance[i].enc0_present) {
            hm2->inm.mpg_mode_reg[i] = hm2->inm.instance[i].hal.param.enc0_mode << 0;
        }
        if (hm2->inm.instance[i].enc1_present) {
            hm2->inm.mpg_mode_reg[i] |= hm2->inm.instance[i].hal.param.enc1_mode << 8;
        }
        if (hm2->inm.instance[i].enc2_present) {
            hm2->inm.mpg_mode_reg[i] |= hm2->inm.instance[i].hal.param.enc2_mode << 16;
        }
        if (hm2->inm.instance[i].enc3_present) {
            hm2->inm.mpg_mode_reg[i] |= hm2->inm.instance[i].hal.param.enc3_mode << 24;
        }
    ...
    for (i = 0; i < hm2->inm.num_instances; i++) {
        ...
        if (hm2->inm.instance[i].enc0_present) {
            ...
            if (*hm2->inm.instance[i].hal.pin.enc0_reset == 0) {
                ...
                *hm2->inm.instance[i].hal.pin.enc0_count = 0;
            }
        }
        ...
        if (hm2->inm.instance[i].enc1_present) {
            ...
            if (*hm2->inm.instance[i].hal.pin.enc1_reset == 0) {
                ...
                *hm2->inm.instance[i].hal.pin.enc1_count = 0;
            }
        }
        ...
        if (hm2->inm.instance[i].enc2_present) {
            ...
            if (*hm2->inm.instance[i].hal.pin.enc2_reset == 0) {
                ...
                *hm2->inm.instance[i].hal.pin.enc2_count = 0;
            }
        }
        ...
        if (hm2->inm.instance[i].enc3_present) {
            ...
            if (*hm2->inm.instance[i].hal.pin.enc3_reset == 0) {
                ...
                *hm2->inm.instance[i].hal.pin.enc3_count = 0;
            }
        }
    }
}
