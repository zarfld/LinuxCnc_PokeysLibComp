...
int hm2_outm_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_outms > md->instances) {
        HM2_ERR("config.num_outms=%d, but only %d are available, not loading driver\n", hm2->config.num_outms, md->instances);
        return -EINVAL;
    }
    ...
    // the ones that have Pin Descriptors below.
    {
        ...
        for (inst = 0; inst < hm2->outm.num_instances; inst++) {
            unsigned out;
            for (out = 0; out < sizeof(hm2->outm.instance[0].hal.pin.out) / sizeof(hm2->outm.instance[0].hal.pin.out[0]); out++) {
