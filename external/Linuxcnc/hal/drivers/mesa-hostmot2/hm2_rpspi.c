...
static inline void spi1_xfer_setup(hm2_rpspi_t *hm2, bool rd) {
    ...
    reg_wr(&aux->spi0_cntl0, AUX_SPI_CNTL0_SPEED(spi1_clkdiv_calc(hm2->spiclkbase, rd ? hm2->spiclkrater : hm2->spiclkratew)) | AUX_SPI_CNTL0_ENABLE | AUX_SPI_CNTL0_MSB_OUT | AUX_SPI_CNTL0_OUT_RISING | AUX_SPI_CNTL0_SHIFT_LENGTH(16) | hm2->spice);
...
static int32_t check_cookie(hm2_rpspi_t *board) {
    ...
    rtapi_print_msg(RPSPI_ERR,
                    "hm2_rpspi: SPI%d/CE%d Invalid cookie, read: %08x %08x %08x,"
                    " expected: %08x %08x %08x\n",
                    board->spidevid, board->spiceid, cookie[0], cookie[1], cookie[2], xcookie[0], xcookie[1], xcookie[2]);
    ...
    if ((!co && spi_pull_miso == SPI_PULL_DOWN) || (ca == 0xffffffff && spi_pull_miso == SPI_PULL_UP)) {
        rtapi_print_msg(RPSPI_ERR,
                        "hm2_rpspi: SPI%d/CE%d No drive seen on MISO line (kept at pull-%s level)."
        ...
        rtapi_print_msg(RPSPI_ERR,
                        "hm2_rpspi: SPI%d/CE%d MISO line stuck at %s level."
        ...
        if (!ones) {
            ...
            rtapi_print_msg(RPSPI_ERR,
                            "hm2_rpspi: SPI%d/CE%d MISO input is bit-shifted by one bit."
                            " SPI read clock frequency probably too high.\n",
                            board->spidevid, board->spiceid);
        } else {
            // More bits are wrong, erratic behaviour
            rtapi_print_msg(RPSPI_ERR,
                            "hm2_rpspi: SPI%d/CE%d MISO input does not match any expected bit-pattern (>= %u bit difference)."
...
static void peripheral_setup(void) {
    ...
    if (spi_pull_miso < 0 || spi_pull_miso > 2)
        spi_pull_miso = 0;
    if (spi_pull_mosi < 0 || spi_pull_mosi > 2)
        spi_pull_mosi = 0;
    if (spi_pull_sclk < 0 || spi_pull_sclk > 2)
        spi_pull_sclk = 0;
...
static int hm2_rpspi_setup(void) {
    ...
    for (j = i = 0; i < RPSPI_MAX_BOARDS; i++) {
        ...
        if (!iddev) {
            ...
            switch (idce) {
                case 0:
                    boards[j].spice = AUX_SPI_CNTL0_CS_1 | AUX_SPI_CNTL0_CS_2;
                    break; // Set SPI1 CE_0
                case 1:
                    boards[j].spice = AUX_SPI_CNTL0_CS_0 | AUX_SPI_CNTL0_CS_2;
                    break; // Set SPI1 CE_1
                case 2:
                    boards[j].spice = AUX_SPI_CNTL0_CS_0 | AUX_SPI_CNTL0_CS_1;
                    break; // Set SPI1 CE_2
            }
