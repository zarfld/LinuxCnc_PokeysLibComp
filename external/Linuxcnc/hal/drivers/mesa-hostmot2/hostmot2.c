...
static void hm2_read(void *void_hm2, long period) {
    ...
    if (!hm2->llio->read_requested)
        hm2_read_request(void_hm2, period);
        ...
        *hm2 = rtapi_list_entry(ptr, hostmot2_t, list);
        if ((*hm2)->bspi.num_instances > 0) {
            for (i = 0; i < (*hm2)->bspi.num_instances; i++) {
                if (!strcmp((*hm2)->bspi.instance[i].name, name)) {
                    return i;
                }
            }
        }
        ...
        *hm2 = rtapi_list_entry(ptr, hostmot2_t, list);
        if ((*hm2)->uart.num_instances > 0) {
            for (i = 0; i < (*hm2)->uart.num_instances; i++) {
                if (!strcmp((*hm2)->uart.instance[i].name, name)) {
                    return i;
                }
            }
        }
        ...
        *hm2 = rtapi_list_entry(ptr, hostmot2_t, list);
        if ((*hm2)->pktuart.num_instances > 0) {
            for (i = 0; i < (*hm2)->pktuart.num_instances; i++) {
                if (!strcmp((*hm2)->pktuart.instance[i].name, name)) {
                    return i;
                }
            }
        }
...
const char *hm2_get_general_function_name(int gtag) {
    switch (gtag) {
        ...
        case HM2_GTAG_RCPWMGEN:
            return "RCPWMGen";
        case HM2_GTAG_TRANSLATIONRAM:
            return "TranslationRAM";
        case HM2_GTAG_TPPWM:
            return "ThreePhasePWM";
        case HM2_GTAG_LED:
            return "LED";
        case HM2_GTAG_MUXED_ENCODER:
            return "Muxed Encoder";
        case HM2_GTAG_MUXED_ENCODER_SEL:
            return "Muxed Encoder Select";
        case HM2_GTAG_SMARTSERIALB:
        case HM2_GTAG_SMARTSERIAL:
            return "Smart Serial Interface";
        case HM2_GTAG_BSPI:
            return "Buffered SPI Interface";
        case HM2_GTAG_UART_RX:
            return "UART Receive Channel";
        case HM2_GTAG_UART_TX:
            return "UART Transmit Channel";
        case HM2_GTAG_PKTUART_RX:
            return "PktUART Receive Channel";
        case HM2_GTAG_PKTUART_TX:
            return "PktUART Transmit Channel";
        case HM2_GTAG_HM2DPLL:
            return "Hostmot2 DPLL";
        case HM2_GTAG_INMUX:
            return "InMux Input Mux";
        case HM2_GTAG_INM:
            return "InM Input Module";
        case HM2_GTAG_OUTM:
            return "OutM Output Module";
        case HM2_GTAG_XY2MOD:
            return "xy2mod Galvo interface";
        case HM2_GTAG_DPAINTER:
            return "Data Painter";
        case HM2_GTAG_SSR:
            return "SSR";
...
static int hm2_parse_config_string(hostmot2_t *hm2, char *config_string) {
    ...
    for (i = 0; i < 4; i++)
        for (j = 0; j < 8; j++)
            hm2->config.sserial_modes[i][j] = '0';
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (strncmp(token, "num_encoders=", 13) == 0) {
            ...
            token += 9;
            if (hm2_fabs_parse(hm2, token, HM2_GTAG_SSI))
                goto fail;

        } else if (strncmp(token, "biss_chan_", 10) == 0) {
            token += 10;
            if (hm2_fabs_parse(hm2, token, HM2_GTAG_BISS))
                goto fail;

        } else if (strncmp(token, "fanuc_chan_", 11) == 0) {
            token += 11;
            if (hm2_fabs_parse(hm2, token, HM2_GTAG_FABS))
                goto fail;
            ...
            if (i < 0 || i > 3 || *token != '=') {
                HM2_ERR("sserial_port tag must be in the form "
                        ""
                        "sserial_port_N=0123xx23"
                        " where N may be 0 to 3\n");
                goto fail;
            }
    ...
    HM2_DBG("    sserial_port_0=%8.8s\n"
            ...
            "                    sserial_port_2=%8.8s\n"
            "                    sserial_port_3=%8.8s\n",
            hm2->config.sserial_modes[0], hm2->config.sserial_modes[1], hm2->config.sserial_modes[2], hm2->config.sserial_modes[3]);
...
static void hm2_print_idrom(hostmot2_t *hm2) {
    ...
    HM2_PRINT("    Board Name: %c%c%c%c%c%c%c%c\n", hm2->idrom.board_name[0], hm2->idrom.board_name[1], hm2->idrom.board_name[2], hm2->idrom.board_name[3], hm2->idrom.board_name[4], hm2->idrom.board_name[5], hm2->idrom.board_name[6], hm2->idrom.board_name[7]);
...
static int hm2_read_idrom(hostmot2_t *hm2) {
    ...
    if ((hm2->idrom.idrom_type != 2) && (hm2->idrom.idrom_type != 3)) {
    ...
    if (hm2->idrom.io_width != (hm2->idrom.io_ports * hm2->idrom.port_width)) {
        HM2_ERR("IDROM IOWidth is %d, but IDROM IOPorts is %d and IDROM PortWidth is %d (inconsistent firmware), aborting driver load\n", hm2->idrom.io_width, hm2->idrom.io_ports, hm2->idrom.port_width);
        return -EINVAL;
    }

    if (hm2->idrom.io_ports != hm2->llio->num_ioport_connectors) {
        HM2_ERR("IDROM IOPorts is %d but llio num_ioport_connectors is %d, driver and firmware are inconsistent, aborting driver load\n", hm2->idrom.io_ports, hm2->llio->num_ioport_connectors);
        return -EINVAL;
    }

    if (hm2->idrom.io_width > HM2_MAX_PIN_DESCRIPTORS) {
        HM2_ERR("IDROM IOWidth is %d but max is %d, aborting driver load\n", hm2->idrom.io_width, HM2_MAX_PIN_DESCRIPTORS);
        return -EINVAL;
    }

    if (hm2->idrom.clock_low < 1e6) {
        HM2_ERR("IDROM ClockLow is %d, that's too low, aborting driver load\n", hm2->idrom.clock_low);
        return -EINVAL;
    }

    if (hm2->idrom.clock_high < 1e6) {
        HM2_ERR("IDROM ClockHigh is %d, that's too low, aborting driver load\n", hm2->idrom.clock_high);
        return -EINVAL;
    }
...
static int hm2_read_module_descriptors(hostmot2_t *hm2) {
    ...
    for (hm2->num_mds = 0; hm2->num_mds < HM2_MAX_MODULE_DESCRIPTORS; hm2->num_mds++, addr += 12) {
        rtapi_u32 d[3];
...
int hm2_md_is_consistent_or_complain(hostmot2_t *hm2, int md_index, rtapi_u8 version, rtapi_u8 num_registers, rtapi_u32 instance_stride, rtapi_u32 multiple_registers) {
    hm2_module_descriptor_t *md = &hm2->md[md_index];

    if (hm2_md_is_consistent(hm2, md_index, version, num_registers, instance_stride, multiple_registers))
        return 1;

    HM2_ERR("inconsistent Module Descriptor for %s, not loading driver\n", hm2_get_general_function_name(md->gtag));

    HM2_ERR("    Version = %d, expected %d\n", md->version, version);

    HM2_ERR("    NumRegisters = %d, expected %d\n", md->num_registers, num_registers);

    HM2_ERR("    InstanceStride = 0x%08X, expected 0x%08X\n", md->instance_stride, instance_stride);

    HM2_ERR("    MultipleRegisters = 0x%08X, expected 0x%08X\n", md->multiple_registers, multiple_registers);

    return 0;
}

int hm2_md_is_consistent(hostmot2_t *hm2, int md_index, rtapi_u8 version, rtapi_u8 num_registers, rtapi_u32 instance_stride, rtapi_u32 multiple_registers) {
    hm2_module_descriptor_t *md = &hm2->md[md_index];

    if ((md->num_registers == num_registers) && (md->version == version) && (md->instance_stride == instance_stride) && (md->multiple_registers == multiple_registers)) {
        return 1;
    }
...
static int hm2_parse_module_descriptors(hostmot2_t *hm2) {
    ...
    for (md_index = 0; md_index < hm2->num_mds; md_index++) {
        ...
        if (md_accepted >= 0) {
            HM2_INFO("MD %d: %dx %s v%d: accepted, using %d\n", md_index, md->instances, hm2_get_general_function_name(md->gtag), md->version, md_accepted);
        } else {
    ...
    for (md_index = 0; md_index < hm2->num_mds; md_index++) {
        ...
        switch (md->gtag) {
            ...
            default:
                HM2_WARN("MD %d: %dx %s v%d: ignored\n", md_index, md->instances, hm2_get_general_function_name(md->gtag), md->version);
                continue;
        }
...
static int dummy_queue_write(hm2_lowlevel_io_t *this, rtapi_u32 addr, const void *buffer, int size) {
    if (size >= 0)
        return this->write(this, addr, buffer, size);
    return 1; // success
}

static int dummy_queue_read(hm2_lowlevel_io_t *this, rtapi_u32 addr, void *buffer, int size) {
    if (size >= 0)
        return this->read(this, addr, buffer, size);
    return 1; // success
}
...
int hm2_register(hm2_lowlevel_io_t *llio, char *config_string) {
...

    {
        for (unsigned port = 0; port < llio->num_ioport_connectors; port++) {
            ...
            for (i = 0; i < HAL_NAME_LEN + 1; i++) {
                if (llio->ioport_connector_name[port][i] == '\0')
                    break;
    ...
    if ((llio->program_fpga != NULL) && (hm2->config.firmware != NULL)) {
        ...
        if (llio->fpga_part_number == NULL) {
            ...
            if (strcmp(llio->fpga_part_number, (char *)bitfile.b.data) != 0) {
                HM2_ERR("board has FPGA '%s', but the firmware in %s is for FPGA '%s'\n", llio->fpga_part_number, hm2->config.firmware, bitfile.b.data);
                rtapi_release_firmware(fw);
                r = -EINVAL;
                goto fail0;
            }
