...
static const char *hm2_get_pin_secondary_name(hm2_pin_t *pin) {
    ...
    switch (pin->sec_tag) {
        ...
        case HM2_GTAG_MUXED_ENCODER:
            switch (sec_pin) {
                ...
                case 3:
                    return "Muxed Index";
                case 4:
                    return "Muxed IndexMask";
                case 5:
                    return "Muxed Probe";
                case 6:
                    return "Muxed Shared Index";
            }
            break;
        ...
        case HM2_GTAG_PWMGEN:
            ...
            switch (sec_pin) {
                case 1:
                    return "Out0 (PWM or Up)";
                case 2:
                    return "Out1 (Dir or Down)";
                case 3:
                    return "Not-Enable";
            }
            break;
        ...
        case HM2_GTAG_PKTUART_TX:
            switch (sec_pin) {
                case 0x1:
                    return "TX Data";
                case 0x2:
                    return "Drv Enable low";
                case 0x3:
                    return "Drv Enable high";
            }
            break;
        ...
        case HM2_GTAG_TPPWM:
            switch (sec_pin) {
                ...
                case 3:
                    return "phase-C";
                case 4:
                    return "phase-A-inverted";
                case 5:
                    return "phase-B-inverted";
                case 6:
                    return "phase-C-inverted";
                case 7:
                    return "enable";
                case 8:
                    return "fault";
...
static void hm2_print_pin_descriptors(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        HM2_PRINT("    pin %d:\n", i);
        HM2_PRINT("        Primary Tag: 0x%02X (%s)\n", hm2->pin[i].primary_tag, hm2_get_general_function_name(hm2->pin[i].primary_tag));
        if (hm2->pin[i].sec_tag != 0) {
            HM2_PRINT("        Secondary Tag: 0x%02X (%s)\n", hm2->pin[i].sec_tag, hm2_get_general_function_name(hm2->pin[i].sec_tag));
            HM2_PRINT("        Secondary Unit: 0x%02X\n", hm2->pin[i].sec_unit);
            HM2_PRINT("        Secondary Pin: 0x%02X (%s, %s)\n", hm2->pin[i].sec_pin, hm2_get_pin_secondary_name(&hm2->pin[i]), ((hm2->pin[i].sec_pin & 0x80) ? "Output" : "Input"));
        }
    }
}
...
int hm2_read_pin_descriptors(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        ...
        if (pin->primary_tag == 0) {
            ...
            HM2_ERR("pin %d primary tag is 0 (end-of-list sentinel), expected %d pins!\n", i, hm2->num_pins);
            return -EINVAL;
        }

        if (pin->primary_tag != HM2_GTAG_IOPORT) {
            HM2_ERR("pin %d primary tag is %d (%s), not IOPort!\n", i, pin->primary_tag, hm2_get_general_function_name(pin->primary_tag));
            return -EINVAL;
        }
...
void hm2_set_pin_source(hostmot2_t *hm2, int pin_number, int source) {
    ...
    if ((pin_number < 0) || (pin_number >= hm2->num_pins) || (hm2->ioport.num_instances <= 0)) {
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        ...
        if (pin->gtag == pin->sec_tag) {
            if (pin->sec_unit & 0x80) // global pins have a 0x80 secondary unit #
                HM2_PRINT("    IO Pin %03d (%s): %s (all), pin %s (%s)\n", i, connector_pin_name, hm2_get_general_function_name(pin->gtag), hm2_get_pin_secondary_name(&hm2->pin[i]), ((pin->sec_pin & 0x80) ? "Output" : "Input"));
            else
                HM2_PRINT("    IO Pin %03d (%s): %s #%d, pin %s (%s)\n", i, connector_pin_name, hm2_get_general_function_name(pin->gtag), pin->sec_unit, hm2_get_pin_secondary_name(pin), ((pin->sec_pin & 0x80) ? "Output" : "Input"));
        } else {
            HM2_PRINT("    IO Pin %03d (%s): %s\n", i, connector_pin_name, hm2_get_general_function_name(pin->gtag));
        }
    }
}
...
static void hm2_pins_allocate_all(hostmot2_t *hm2, int gtag, int num_instances, bool immediate) {
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        if ((hm2->pin[i].sec_tag == gtag) && ((hm2->pin[i].sec_unit < num_instances) || ((hm2->pin[i].sec_unit & 0x80) && (num_instances > 0)))) {
