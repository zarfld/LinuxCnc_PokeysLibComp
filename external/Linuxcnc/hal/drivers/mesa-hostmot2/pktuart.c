...
int hm2_pktuart_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (md->gtag == HM2_GTAG_PKTUART_TX && !hm2_md_is_consistent(hm2, md_index, 0, 4, 4, 0x000F) && !hm2_md_is_consistent(hm2, md_index, 1, 4, 4, 0x000F) && !hm2_md_is_consistent(hm2, md_index, 2, 4, 4, 0x000F)) {
        HM2_ERR("Unsupported or inconsistent PktUART TX module (version %i)"
                "not loading driver \n",
                md->version);
        return -EINVAL;
    }
    if (md->gtag == HM2_GTAG_PKTUART_RX && !hm2_md_is_consistent(hm2, md_index, 0, 4, 4, 0x000F) && !hm2_md_is_consistent(hm2, md_index, 1, 4, 4, 0x000F) && !hm2_md_is_consistent(hm2, md_index, 2, 4, 4, 0x000F)) {
    ...
    if (hm2->config.num_pktuarts > md->instances) {
        HM2_ERR("config defines %d pktuarts, but only %d are available, "
                "not loading driver\n",
                hm2->config.num_pktuarts, md->instances);
        return -EINVAL;
    }
    ...
    if (hm2->pktuart.num_instances == 0) {
        ...
        hm2->pktuart.instance = (hm2_pktuart_instance_t *)hal_malloc(hm2->pktuart.num_instances * sizeof(hm2_pktuart_instance_t));
    ...
    if (md->gtag == HM2_GTAG_PKTUART_RX) {
        ...
        r = hm2_register_tram_read_region(hm2, md->base_address + 3 * md->register_stride, (hm2->pktuart.num_instances * sizeof(rtapi_u32)), &hm2->pktuart.rx_status_reg);
        if (r < 0) {
        ...
        r = hm2_register_tram_read_region(hm2, md->base_address + 3 * md->register_stride, (hm2->pktuart.num_instances * sizeof(rtapi_u32)), &hm2->pktuart.tx_status_reg);
        if (r < 0) {
    ...
    *num_frames = 0;
    ...
    for (i = 0; i < nframes; i++) {
        ...
        while (c < count - 3) {
            ...
            r = hm2->llio->queue_write(hm2->llio, hm2->pktuart.instance[inst].tx_addr, &buff, sizeof(rtapi_u32));
            if (r < 0) {
        ...
        switch (count - c) {
            ...
            case 2:
                ...
                break;
            case 3:
                buff = (data[c] + (data[c + 1] << 8) + (data[c + 2] << 16));
                r = hm2->llio->queue_write(hm2->llio, hm2->pktuart.instance[inst].tx_addr, &buff, sizeof(rtapi_u32));
                if (r < 0) {
                    HM2_ERR("%s send: hm2->llio->queue_write failure\n", name);
                    return -1;
    ...
    for (i = 0; i < nframes; i++) {
        ...
        r = hm2->llio->queue_write(hm2->llio, hm2->pktuart.instance[inst].tx_fifo_count_addr, &buff, sizeof(rtapi_u32));
        // Check for Send Count FIFO error
        r = hm2->llio->queue_read(hm2->llio, hm2->pktuart.instance[inst].tx_mode_addr, &buff, sizeof(rtapi_u32));
        if ((buff >> 4) & 0x01) {
...
int hm2_pktuart_read(char *name, unsigned char data[], rtapi_u8 *num_frames, rtapi_u16 *max_frame_length, rtapi_u16 frame_sizes[]) {
    ...
    *num_frames = 0;
    ...
    while (i < countp) {
        ...
        r = hm2->llio->read(hm2->llio, hm2->pktuart.instance[inst].rx_fifo_count_addr, &buff, sizeof(rtapi_u32));
        ...
        while (c < countb - 3) {
            r = hm2->llio->read(hm2->llio, hm2->pktuart.instance[inst].rx_addr, &buff, sizeof(rtapi_u32));
...
int hm2_pktuart_queue_get_frame_sizes(char *name, rtapi_u32 fsizes[]) {
    ...
    for (j = 0; j < (int)((hm2->pktuart.rx_status_reg[inst] >> 16) & 0x1F); j++) {
        ...
        r = hm2->llio->queue_read(hm2->llio, hm2->pktuart.instance[inst].rx_fifo_count_addr, &fsizes[j], sizeof(rtapi_u32));
        if (r < 0) {
...
int hm2_pktuart_queue_read_data(char *name, rtapi_u32 data[], int bytes) {
    ...
    for (i = 0; i < ((bytes % 4 == 0) ? bytes / 4 : bytes / 4 + 1); i++) {
        r = hm2->llio->queue_read(hm2->llio, hm2->pktuart.instance[inst].rx_addr, &data[i], sizeof(rtapi_u32));
        if (r < 0) {
...
void hm2_pktuart_print_module(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->pktuart.num_instances; i++) {
        HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n", hm2->pktuart.instance[i].clock_freq, hm2_hz_to_mhz(hm2->pktuart.instance[i].clock_freq));
