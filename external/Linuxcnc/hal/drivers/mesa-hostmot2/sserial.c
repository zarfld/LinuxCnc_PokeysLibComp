...
int hm2_sserial_read_nvram_word(hostmot2_t *hm2, hm2_sserial_remote_t *chan, void *data, int addr, int length) {
    rtapi_u32 buff;
    buff = 0xEC000000;
    ...
    switch (length) {
        case 1:
            buff = READ_REM_BYTE_CMD + addr;
            break;
        case 2:
            buff = READ_REM_WORD_CMD + addr;
            break;
        case 4:
            buff = READ_REM_LONG_CMD + addr;
            break;
        case 8:
            buff = READ_REM_DOUBLE_CMD + addr;
            break;
        default:
...
int check_set_baudrate(hostmot2_t *hm2, hm2_sserial_instance_t *inst) {
    ...
    buff = 0x800;
    HM2WRITE(inst->command_reg_addr, buff); // stop all

    return 0;
}

int hm2_sserial_stopstart(hostmot2_t *hm2, hm2_module_descriptor_t *md, hm2_sserial_instance_t *inst, rtapi_u32 start_mode) {
    rtapi_u32 buff, addr;
    int i = inst->index;
    int c;
    ...
    for (c = 0; c < inst->num_channels; c++) {
        if (hm2->config.sserial_modes[i][c] != 'x') {
            ...
            addr = md->base_address + 2 * md->register_stride + i * md->instance_stride + c * sizeof(rtapi_u32);
...
int hm2_sserial_get_param_value(hostmot2_t *hm2, hm2_sserial_remote_t *chan, int index, int set_hal) {
    ...
    if (index >= chan->num_globals || index < 0)
        return -1;
    ...
    switch (chan->globals[index].DataType) {
        ...
        case LBP_BITS:
            break;
        case LBP_UNSIGNED:
            r = hm2_sserial_get_bytes(hm2, chan, (void *)&(p->u32_written), g->ParmAddr, g->DataLength / 8);
            if (r < 0) {
                HM2_ERR("SSerial Parameter read error\n");
                return -EINVAL;
            }
            if (set_hal)
            ...
            break;
        case LBP_SIGNED:
            r = hm2_sserial_get_bytes(hm2, chan, (void *)&(p->s32_written), g->ParmAddr, g->DataLength / 8);
            if (set_hal)
                p->s32_param = p->s32_written;
            HM2_DBG("LBP_SIGNED %i %i \n", p->s32_param, p->s32_written);
            break;
        case LBP_NONVOL_UNSIGNED:
            r = hm2_sserial_read_nvram_word(hm2, chan, (void *)&(p->u32_written), g->ParmAddr, g->DataLength / 8);
            if (set_hal)
                p->u32_param = p->u32_written;
            HM2_DBG("LBP_NONVOL_UNSIGNED %i %i \n", p->u32_param, p->u32_written);
            break;
        case LBP_NONVOL_SIGNED:
            r = hm2_sserial_read_nvram_word(hm2, chan, (void *)&(p->s32_written), g->ParmAddr, g->DataLength / 8);
            if (set_hal)
                p->s32_param = p->s32_written;
        case LBP_STREAM:
...
int hm2_sserial_create_params(hostmot2_t *hm2, hm2_sserial_remote_t *chan) {
    ...
    for (i = 0; i < chan->num_globals; i++) {
        ...
        switch (chan->params[i].type) {
            ...
            case LBP_UNSIGNED:
            case LBP_NONVOL_UNSIGNED:
                r = hal_param_u32_newf(hal_dir, &(chan->params[i].u32_param), hm2->llio->comp_id, "%s.%s", chan->name, global.NameString);
                if (r < 0) {
            ...
            case LBP_SIGNED:
            case LBP_NONVOL_SIGNED:
                r = hal_param_s32_newf(hal_dir, &(chan->params[i].s32_param), hm2->llio->comp_id, "%s.%s", chan->name, global.NameString);
                if (r < 0) {
                    HM2_ERR("Out of memory\n");
                    return -ENOMEM;
                }
            ...
            case LBP_NONVOL_FLOAT:
                r = hal_param_float_newf(hal_dir, &(chan->params[i].float_param), hm2->llio->comp_id, "%s.%s", chan->name, global.NameString);
                if (r < 0) {
                    HM2_ERR("Out of memory\n");
                    return -ENOMEM;
                }
...
int hm2_sserial_get_globals_list(hostmot2_t *hm2, hm2_sserial_remote_t *chan) {
    ...
    if (gtoc == 0) {
        if (hm2->sserial.baudrate == 115200) {
            ...
            HM2_ERR("No GTOC in sserial read globals\n");
            return -1;
        }
    } else {
        do {
            ...
            for (i = 0; i <= chan->num_confs; i++) {
                if (chan->confs[i].ParmAddr == data.ParmAddr) {
                    i = 1000;
                }
            }
            if (data.RecordType == LBP_DATA && i < 1000) {
                ...
                chan->num_globals++;
                chan->globals = (hm2_sserial_data_t *)rtapi_krealloc(chan->globals, chan->num_globals * sizeof(hm2_sserial_data_t), RTAPI_GFP_KERNEL);
                ...
                rtapi_print("Board %s %s Mode %i = %s\n", chan->name, type, mode.ModeIndex, mode.NameString);
            }
        } while (addr > 0);
    }
...
int hm2_sserial_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->sserial.num_instances != 0) {
        HM2_ERR("found duplicate Module Descriptor for %s (inconsistent firmwar"
                "e), not loading driver\n",
                hm2_get_general_function_name(md->gtag));
        return -EINVAL;
    }
    if (hm2->config.num_sserials > md->instances) {
        HM2_ERR("num_sserials references %d instances, but only %d are available"
                ", not loading driver\n",
                hm2->config.num_sserials, md->instances);
        return -EINVAL;
    }
    ...
    hm2->sserial.instance = (hm2_sserial_instance_t *)hal_malloc(hm2->sserial.num_instances * sizeof(hm2_sserial_instance_t));
    ...
    for (port = 0; port < hm2->ioport.num_instances; port++) {
        ...
        for (unsigned port_pin = 0; port_pin < hm2->idrom.port_width; port_pin++) {
            pin++;
            if (hm2->pin[pin].sec_tag == HM2_GTAG_SMARTSERIAL || hm2->pin[pin].sec_tag == HM2_GTAG_SMARTSERIALB) {
                ...
                HM2_DBG("port %i sec unit = %i, sec pin = %i mode=%c\n", port, hm2->pin[pin].sec_unit, hm2->pin[pin].sec_pin & 0x0F, hm2->config.sserial_modes[hm2->pin[pin].sec_unit][(hm2->pin[pin].sec_pin & 0x0F) - 1]);
                if (hm2->config.sserial_modes[hm2->pin[pin].sec_unit][(hm2->pin[pin].sec_pin & 0x0F) - 1] != 'x') {
                    src_reg |= (1 << port_pin);
        ...
        hm2->llio->write(hm2->llio, hm2->ioport.ddr_addr + 4 * port, &ddr_reg, sizeof(rtapi_u32));
        hm2->llio->write(hm2->llio, hm2->ioport.alt_source_addr + 4 * port, &src_reg, sizeof(rtapi_u32));
    }
    ...
    for (i = 0; i < md->instances; i++) {
        ...
        for (c = 0; c < inst->num_channels; c++) {
            ...
            addr0 = md->base_address + 3 * md->register_stride + i * md->instance_stride + c * sizeof(rtapi_u32);
            HM2READ(addr0, user0);
            HM2_DBG("Inst %i Chan %i Addr %x User0 = %x\n", i, c, addr0, user0);

            addr1 = md->base_address + 4 * md->register_stride + i * md->instance_stride + c * sizeof(rtapi_u32);
            HM2READ(addr1, user1);
            HM2_DBG("Inst %i Chan %i Addr %x User1 = %x\n", i, c, addr1, user1);

            addr2 = md->base_address + 5 * md->register_stride + i * md->instance_stride + c * sizeof(rtapi_u32);
            HM2READ(addr2, user2);
            HM2_DBG("Inst %i Chan %i Addr %x User2 = %x\n", i, c, addr2, user2);

            if (hm2->sserial.baudrate == 115200 && hm2->config.sserial_modes[i][c] != 'x') { // setup mode
                ...
                if ((user1 & 0xFF00) == 0x4900) { // XiXXboard
                    ...
                    int crc_addr = getlocal8(hm2, inst, SSLBPCHANNELSTARTLOC) + (c * lbpstride) + 30;
            ...
            else if (user1 == 0 || (inst->tag & 1 << c) == 0 || hm2->config.sserial_modes[i][c] == 'x') {
                for (pin = 0; pin < hm2->num_pins; pin++) {
                    if (hm2->pin[pin].sec_tag == HM2_GTAG_SMARTSERIAL || hm2->pin[pin].sec_tag == HM2_GTAG_SMARTSERIALB) {
                        if ((hm2->pin[pin].sec_pin & 0x0F) - 1 == c && hm2->pin[pin].sec_unit == i) {
        ...
        if (inst->num_remotes > 0) {
            ...
            if ((r = hm2_register_tram_write_region(hm2, inst->command_reg_addr, sizeof(rtapi_u32), &inst->command_reg_write)) < 0) {
...
int hm2_sserial_setup_channel(hostmot2_t *hm2, hm2_sserial_instance_t *inst, int index) {
    ...
    r = hal_pin_s32_newf(HAL_OUT, &(inst->debug), hm2->llio->comp_id, "%s.%i.debug", hm2->llio->name, index);
    if (r < 0) {
        HM2_ERR("error adding pin %s.sserial.%1d.run. aborting\n", hm2->llio->name, index);
        return -EINVAL;
    }

    r = hal_pin_bit_newf(HAL_IN, &(inst->run), hm2->llio->comp_id, "%s.sserial.port-%1d.run", hm2->llio->name, index);
    if (r < 0) {
        HM2_ERR("error adding pin %s.sserial.%1d.run. aborting\n", hm2->llio->name, index);
        return -EINVAL;
    }
    *inst->run = true;
    ...
    r = hal_pin_u32_newf(HAL_OUT, &(inst->state), hm2->llio->comp_id, "%s.sserial.port-%1d.port_state", hm2->llio->name, index);
    if (r < 0) {
        HM2_ERR("error adding pin %s.sserial.%1d.port_state. aborting\n", hm2->llio->name, index);
        return -EINVAL;
    }
    r = hal_pin_u32_newf(HAL_OUT, &(inst->state2), hm2->llio->comp_id, "%s.sserial.port-%1d.port_state2", hm2->llio->name, index);
    if (r < 0) {
        HM2_ERR("error adding pin %s.sserial.%1d.port_state. aborting\n", hm2->llio->name, index);
        return -EINVAL;
    }
    r = hal_pin_u32_newf(HAL_OUT, &(inst->state3), hm2->llio->comp_id, "%s.sserial.port-%1d.port_state3", hm2->llio->name, index);
    if (r < 0) {
        HM2_ERR("error adding pin %s.sserial.%1d.port_state. aborting\n", hm2->llio->name, index);
        return -EINVAL;
    }

    r = hal_pin_u32_newf(HAL_OUT, &(inst->fault_count), hm2->llio->comp_id, "%s.sserial.port-%1d.fault-count", hm2->llio->name, index);
    if (r < 0) {
        HM2_ERR("error adding pin %s.sserial.%1d.fault-count. aborting\n", hm2->llio->name, index);
        return -EINVAL;
    }
    r = hal_param_u32_newf(HAL_RW, &(inst->fault_inc), hm2->llio->comp_id, "%s.sserial.port-%1d.fault-inc", hm2->llio->name, index);
    if (r < 0) {
    ...
    r = hal_param_u32_newf(HAL_RW, &(inst->fault_dec), hm2->llio->comp_id, "%s.sserial.port-%1d.fault-dec", hm2->llio->name, index);
    if (r < 0) {
    ...
    r = hal_param_u32_newf(HAL_RW, &(inst->fault_lim), hm2->llio->comp_id, "%s.sserial.port-%1d.fault-lim", hm2->llio->name, index);
    if (r < 0) {
    ...
    r = hm2_register_tram_read_region(hm2, inst->command_reg_addr, sizeof(rtapi_u32), &inst->command_reg_read);
    if (r < 0) {
    ...
    r = hm2_register_tram_read_region(hm2, inst->data_reg_addr, sizeof(rtapi_u32), &inst->data_reg_read);
    if (r < 0) {
        HM2_ERR("error registering tram read region for sserial "
                "command register (%d)\n",
                index);
...
int hm2_sserial_setup_remotes(hostmot2_t *hm2, hm2_sserial_instance_t *inst, hm2_module_descriptor_t *md) {
    ...
    inst->remotes = (hm2_sserial_remote_t *)rtapi_kzalloc(inst->num_remotes * sizeof(hm2_sserial_remote_t), RTAPI_GFP_KERNEL);
    ...
    for (c = 0; c < inst->num_channels; c++) {
        if (inst->tag & (1 << c)) {
            ...
            chan->reg_cs_addr = md->base_address + 2 * md->register_stride + inst->index * md->instance_stride + c * sizeof(rtapi_u32);
            HM2_DBG("reg_cs_addr = %x\n", chan->reg_cs_addr);
            // Assume that all process data registers are in use for now.
            for (i = 0; i < HM2_SSERIAL_NUMREGS; i++) {
                chan->rw_addr[i] = md->base_address + (3 + i) * md->register_stride + inst->index * md->instance_stride + c * sizeof(rtapi_u32);
                HM2_DBG("rw_addr[%i] = %x\n", i, chan->rw_addr[i]);
            }

            // Get the board ID and name before it is over-written by DoIts
            hm2->llio->read(hm2->llio, chan->rw_addr[0], &buff, sizeof(rtapi_u32));
            ...
            if (hm2->use_serial_numbers) {
                rtapi_snprintf(chan->name, sizeof(chan->name), "hm2_%2s.%04x", chan->raw_name, (chan->serialnumber & 0xffff));
            } else {
                rtapi_snprintf(chan->name, sizeof(chan->name), "%s.%2s.%d.%d", hm2->llio->name, chan->raw_name, inst->index, c);
            }
...
int hm2_sserial_read_configs(hostmot2_t *hm2, hm2_sserial_remote_t *chan) {
    ...
    do {
        ...
        if (rectype == LBP_DATA) {
            c = chan->num_confs++;
            chan->confs = (hm2_sserial_data_t *)rtapi_krealloc(chan->confs, chan->num_confs * sizeof(hm2_sserial_data_t), RTAPI_GFP_KERNEL);
            addr = hm2_sserial_get_bytes(hm2, chan, &chan->confs[c], addr, 14);
            if (addr < 0) {
                return -EINVAL;
            }
            addr = hm2_sserial_get_bytes(hm2, chan, &(chan->confs[c].UnitString), addr, -1);
            if (addr < 0) {
                return -EINVAL;
            }
            addr = hm2_sserial_get_bytes(hm2, chan, &(chan->confs[c].NameString), addr, -1);
            if (addr < 0) {
                return -EINVAL;
            }
            ...
            chan->num_modes++;
            m = chan->num_modes - 1;
            chan->modes = (hm2_sserial_mode_t *)rtapi_krealloc(chan->modes, chan->num_modes * sizeof(hm2_sserial_mode_t), RTAPI_GFP_KERNEL);
            addr = hm2_sserial_get_bytes(hm2, chan, &chan->modes[m], addr, 4);
            if (addr < 0) {
                return -EINVAL;
            }
            addr = hm2_sserial_get_bytes(hm2, chan, &(chan->modes[m].NameString), addr, -1);
            if (addr < 0) {
                return -EINVAL;
            }
        }
...
int hm2_sserial_create_pins(hostmot2_t *hm2, hm2_sserial_remote_t *chan) {
    ...
    int data_dir;
    chan->pins = (hm2_sserial_pins_t *)hal_malloc(chan->num_confs * sizeof(hm2_sserial_pins_t));

    chan->num_read_bits = 0;
    chan->num_write_bits = 0;
    ...
    for (i = 0; i < chan->num_confs; i++) {
        ...
        if (chan->num_read_regs > HM2_SSERIAL_NUMREGS || chan->num_write_regs > HM2_SSERIAL_NUMREGS) {
        ...
        switch (chan->confs[i].DataType) {
            case LBP_PAD:
                break;
            case LBP_BITS:
                chan->pins[i].bit_pins = (hal_bit_t **)hal_malloc(chan->confs[i].DataLength * sizeof(hal_bit_t *));
                chan->pins[i].bit_pins_not = (hal_bit_t **)hal_malloc(chan->confs[i].DataLength * sizeof(hal_bit_t *));
                chan->pins[i].invert = (hal_bit_t *)hal_malloc(chan->confs[i].DataLength * sizeof(hal_bit_t));
                for (j = 0; j < chan->confs[i].DataLength; j++) {

                    rtapi_snprintf(name, sizeof(name), "%s.%s-%02d", chan->name, chan->confs[i].NameString, j);
                    r = hal_pin_bit_new(name, data_dir, &(chan->pins[i].bit_pins[j]), hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        return r;
                    }
                    if (data_dir == HAL_OUT) {
                        rtapi_snprintf(name, sizeof(name), "%s.%s-%02d-not", chan->name, chan->confs[i].NameString, j);
                        r = hal_pin_bit_new(name, data_dir, &(chan->pins[i].bit_pins_not[j]), hm2->llio->comp_id);
                        if (r < 0) {
                            HM2_ERR("error adding pin '%s', aborting\n", name);
                            return r;
                        }
                    ...
                    if (data_dir == HAL_IN) {
                        rtapi_snprintf(name, sizeof(name), "%s.%s-%02d-invert", chan->name, chan->confs[i].NameString, j);
                        r = hal_param_bit_new(name, HAL_RW, &(chan->pins[i].invert[j]), hm2->llio->comp_id);
                        if (r < 0) {
                            HM2_ERR("error adding pin '%s', aborting\n", name);
                            return r;
                        }
            ...
            case LBP_UNSIGNED:
            case LBP_SIGNED:
                rtapi_snprintf(name, sizeof(name), "%s.%s", chan->name, chan->confs[i].NameString);
                r = hal_pin_float_new(name, data_dir, &(chan->pins[i].float_pin), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return r;
                }
                rtapi_snprintf(name, sizeof(name), "%s.%s-scalemax", chan->name, chan->confs[i].NameString);
                r = hal_param_float_new(name, HAL_RW, &(chan->pins[i].fullscale), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return r;
                }
                ...
                rtapi_snprintf(name, sizeof(name), "%s.%s-maxlim", chan->name, chan->confs[i].NameString);
                r = hal_param_float_new(name, HAL_RW, &(chan->pins[i].maxlim), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return r;
                }
                chan->pins[i].maxlim = chan->confs[i].ParmMax;
                rtapi_snprintf(name, sizeof(name), "%s.%s-minlim", chan->name, chan->confs[i].NameString);
                r = hal_param_float_new(name, HAL_RW, &(chan->pins[i].minlim), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return r;
                }
            ...
            case LBP_STREAM:
                rtapi_snprintf(name, sizeof(name), "%s.%s", chan->name, chan->confs[i].NameString);
                r = hal_pin_u32_new(name, data_dir, &(chan->pins[i].u32_pin), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return r;
                }
            ...
            case LBP_BOOLEAN:
                rtapi_snprintf(name, sizeof(name), "%s.%s", chan->name, chan->confs[i].NameString);
                r = hal_pin_bit_new(name, data_dir, &(chan->pins[i].boolean), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return r;
                }
                if (data_dir == HAL_OUT) {
                    rtapi_snprintf(name, sizeof(name), "%s.%s-not", chan->name, chan->confs[i].NameString);
                    r = hal_pin_bit_new(name, data_dir, &(chan->pins[i].boolean2), hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        return r;
                    }
                ...
                if (data_dir == HAL_IN) {
                    ...
                    rtapi_snprintf(name, sizeof(name), "%s.%s-invert", chan->name, chan->confs[i].NameString);
                    r = hal_param_bit_new(name, HAL_RW, chan->pins[i].invert, hm2->llio->comp_id);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        return r;
                    }
            ...
            case LBP_ENCODER_H:
                ...
                rtapi_snprintf(name, sizeof(name), "%s.%s.count", chan->name, chan->confs[i].NameString);
                r = hal_pin_s32_new(name, HAL_OUT, &(chan->pins[i].s32_pin), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return -EINVAL;
                }
                rtapi_snprintf(name, sizeof(name), "%s.%s.rawcounts", chan->name, chan->confs[i].NameString);
                r = hal_pin_s32_new(name, HAL_OUT, &(chan->pins[i].s32_pin2), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return -EINVAL;
                }
                rtapi_snprintf(name, sizeof(name), "%s.%s.position", chan->name, chan->confs[i].NameString);
                r = hal_pin_float_new(name, HAL_OUT, &(chan->pins[i].float_pin), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return -EINVAL;
                }
                rtapi_snprintf(name, sizeof(name), "%s.%s.index-enable", chan->name, chan->confs[i].NameString);
                r = hal_pin_bit_new(name, HAL_IO, &(chan->pins[i].boolean), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return -EINVAL;
                }

                rtapi_snprintf(name, sizeof(name), "%s.%s.reset", chan->name, chan->confs[i].NameString);
                r = hal_pin_bit_new(name, HAL_IO, &(chan->pins[i].boolean2), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return -EINVAL;
                }
                rtapi_snprintf(name, sizeof(name), "%s.%s.scale", chan->name, chan->confs[i].NameString);
                r = hal_param_float_new(name, HAL_RW, &(chan->pins[i].fullscale), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return -EINVAL;
                ...
                rtapi_snprintf(name, sizeof(name), "%s.%s.counts-per-rev", chan->name, chan->confs[i].NameString);
                r = hal_param_u32_new(name, HAL_RW, &(chan->pins[i].u32_param), hm2->llio->comp_id);
                if (r < 0) {
                    HM2_ERR("error adding pin '%s', aborting\n", name);
                    return -EINVAL;
                }
            ...
            case LBP_FLOAT:
                ...
                break;
            default:
                HM2_ERR("Unhandled sserial data type (%i) Name %s Units %s\n", chan->confs[i].DataType, chan->confs[i].NameString, chan->confs[i].UnitString);
        }
    }
    return 0;
}
...
int hm2_sserial_register_tram(hostmot2_t *hm2, hm2_sserial_remote_t *chan) {
    ...
    HM2_DBG("%s read-bits = %i, write-bits = %i\n"
            ...
            chan->name, chan->num_read_bits, chan->num_write_bits, chan->num_read_regs, chan->num_write_regs);

    r = hm2_register_tram_read_region(hm2, chan->reg_cs_addr, sizeof(rtapi_u32), &chan->reg_cs_read);
    if (r < 0) {
    ...
    for (i = 0; i < chan->num_read_regs; i++) {
        r = hm2_register_tram_read_region(hm2, chan->rw_addr[i], sizeof(rtapi_u32), &chan->read[i]);
        if (r < 0) {
            HM2_ERR("error registering tram read region for sserial "
    ...
    r = hm2_register_tram_write_region(hm2, chan->reg_cs_addr, sizeof(rtapi_u32), &(chan->reg_cs_write));
    if (r < 0) {
    ...
    for (i = 0; i < chan->num_write_regs; i++) {
        r = hm2_register_tram_write_region(hm2, chan->rw_addr[i], sizeof(rtapi_u32), &chan->write[i]);
        if (r < 0) {
            HM2_ERR("error registering tram read region for sserial "
                    "interface %i register (%d)\n",
                    i, r);
            ...
            switch (*inst->state3) {
                ...
                case 0:
                    ...
                    switch (p->type) {
                        case LBP_SIGNED:
                        case LBP_NONVOL_SIGNED:
                            if (p->s32_param != p->s32_written)
                                break;
                            *inst->state2 = 2; // increment indices
                            return *inst->state2;
                        case LBP_UNSIGNED:
                        case LBP_NONVOL_UNSIGNED:
                            if (p->u32_param != p->u32_written)
                                break;
                            *inst->state2 = 2; // increment indices
                            return *inst->state2;
                        case LBP_FLOAT:
                        case LBP_NONVOL_FLOAT:
                            ...
                            switch (g->DataLength) {
                                ...
                                case 8:
                                    shift = (52 - 4);
                                    break; // 1.3.4 minifloat, if we ever add them
                                case 16:
                                    shift = (52 - 10);
                                    break;
                                case 32:
                            ...
                            if (labs((p->s64_param - p->s64_written) >> shift) > 2)
                                break;
                            *inst->state2 = 2; // increment indices
                            return *inst->state2;
                        default:
                ...
                case 1:
                    ...
                    if (ret > 0)
                        break;
                    if (ret < 0)
                        *inst->state3 = 100; // quit and tidy up
                    *inst->state3 = 2;
                    inst->timer = 20000000;
                ...
                case 2: // Unlock Nonvol access
                    ...
                    if (ret < 0)
                        *inst->state3 = 100; // quit and tidy up
                    if (p->type == LBP_NONVOL_FLOAT || p->type == LBP_NONVOL_UNSIGNED || p->type == LBP_NONVOL_SIGNED) {
...
void hm2_sserial_write_pins(hostmot2_t *hm2, hm2_sserial_instance_t *inst) {
    ...
    if (*inst->fault_count > inst->fault_lim) {
        ...
        HM2_ERR("Smart Serial Comms Error: "
                ...
                inst->fault_dec, inst->fault_inc, inst->fault_lim);
    ...
    for (r = 0; r < inst->num_remotes; r++) {
        ...
        if (*inst->data_reg_read & (1 << chan->index))
            continue;
                ...
                switch (conf->DataType) {
                    ...
                    case LBP_BITS:
                        ...
                        for (b = 0; b < conf->DataLength; b++) {
                            buff |= ((rtapi_u64)(*pin->bit_pins[b] != 0) << b) ^ ((rtapi_u64)(pin->invert[b] != 0) << b);
                        }
                        break;
                    case LBP_UNSIGNED:
                        val = *pin->float_pin;
                        ...
                        if (val < pin->minlim)
                            val = pin->minlim;
                        buff = (rtapi_u64)((val / pin->fullscale) * (~0ull >> (64 - conf->DataLength)));
                        break;
                    case LBP_SIGNED:
                        ...
                        if (val < pin->minlim)
                            val = pin->minlim;
                        buff = (((rtapi_s32)(val / pin->fullscale * 2147483647)) >> (32 - conf->DataLength)) & (~0ull >> (64 - conf->DataLength));
                        break;
                    case LBP_STREAM:
                    ...
                    case LBP_FLOAT:
                        ...
                        break;
                    default:
                        HM2_ERR("Unsupported output datatype 0x%02X (name: "
                                "%s"
                                ")\n",
    ...
    for (i = 0; i < hm2->sserial.num_instances; i++) {
        ...
        switch ((*inst->state) & 0xFF) {
            ...
            case 0: // Idle
                ...
                if (hm2_sserial_update_params(hm2, inst, period) > 0)
                    break;
            ...
            case 2: // just transitioning to running
                if (hm2_sserial_wait(hm2, inst, period) > 0)
                    break;
                *inst->state = 3;
                break;
        ...
        if (!(conf->DataDir & 0x80)) {
            ...
            switch (conf->DataType) {
                ...
                case LBP_UNSIGNED:
                    ...
                    *pin->float_pin = (buff * pin->fullscale) / ((1 << conf->DataLength) - 1);
                    break;
                case LBP_SIGNED:
                    buff32 = (buff & 0xFFFFFFFFL) << (32 - conf->DataLength);
                    *pin->float_pin = (buff32 / 2147483647.0) * pin->fullscale;
                    break;
                case LBP_STREAM:
                ...
                case LBP_ENCODER: {
                    ...
                    if (*pin->boolean && ppr > 0) { // index-enable set
                        ...
                        if ((unsigned)abs(rem1 - rem2) > ppr / 2 || (rem1 >= 0 && rem2 < 0) || (rem1 < 0 && rem2 >= 0)) {
                ...
                default:
                    HM2_ERR_NO_LL("Unsupported input datatype 0x%02X (name: "
                                  "%s"
                                  ")\n",
                                  conf->DataType, conf->NameString);
...
void hm2_sserial_process_tram_read(hostmot2_t *hm2, long period) {
    ...
    for (i = 0; i < hm2->sserial.num_instances; i++) {
        ...
        inst->ever_read = true;
        if (*inst->state != 3)
            continue; // Only work on running instances
    ...
    for (i = 0; i < hm2->sserial.num_instances; i++) {
        ...
        for (r = 0; r < hm2->sserial.instance[i].num_remotes; r++) {
            ...
            for (c = 0; c < hm2->sserial.instance[i].remotes[r].num_confs; c++) {
                ...
                HM2_PRINT("                   ParmMax %0i.%02i\n", (int)conf.ParmMax, (int)((conf.ParmMax - (int)conf.ParmMax) * 100.0));
                HM2_PRINT("                   ParmMin %0i.%02i\n", (int)conf.ParmMin, (int)((conf.ParmMin - (int)conf.ParmMin) * 100.0));
...
static const char *err_list[32] = { "CRC error",
                                    "Invalid cookie",
                                    "Overrun",
                                    "Timeout",
                                    "Extra character",
