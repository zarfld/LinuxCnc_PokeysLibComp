...
static void hm2_absenc_trigger(void *void_hm2, long period) {
    ...
    if (hm2->absenc.ssi_global_start_addr) {
        hm2->llio->write(hm2->llio, hm2->absenc.ssi_global_start_addr, &buff, sizeof(rtapi_u32));
    }
    if (hm2->absenc.fabs_global_start_addr) {
        hm2->llio->write(hm2->llio, hm2->absenc.fabs_global_start_addr, &buff, sizeof(rtapi_u32));
    }
    if (hm2->absenc.biss_global_start_addr) {
        hm2->llio->write(hm2->llio, hm2->absenc.biss_global_start_addr, &buff, sizeof(rtapi_u32));
    }
}
...
int hm2_absenc_register_tram(hostmot2_t *hm2) {
    ...
    if (hm2->absenc.ssi_global_start_addr) {
        if (hm2_register_tram_read_region(hm2, hm2->absenc.ssi_global_start_addr, sizeof(rtapi_u32), &(hm2->absenc.ssi_busy_flags)) < 0) {
            HM2_ERR("error registering tram read region for SSI flags\n");
            return -EINVAL;
        }
    }
    if (hm2->absenc.biss_global_start_addr) {
        if (hm2_register_tram_read_region(hm2, hm2->absenc.biss_global_start_addr, sizeof(rtapi_u32), &(hm2->absenc.biss_busy_flags)) < 0) {
            HM2_ERR("error registering tram read region for BiSS flags\n");
            return -EINVAL;
        }
    }
    if (hm2->absenc.fabs_global_start_addr) {
        if (hm2_register_tram_read_region(hm2, hm2->absenc.fabs_global_start_addr, sizeof(rtapi_u32), &(hm2->absenc.fabs_busy_flags)) < 0) {
            HM2_ERR("error registering tram read region for BiSS flags\n");
            return -EINVAL;
        }
    }
    ...
    for (i = 0; i < hm2->absenc.num_chans; i++) {
        ...
        switch (chan->myinst) {
            case HM2_GTAG_FABS:
                r += hm2_register_tram_read_region(hm2, chan->rw_addr[2], sizeof(rtapi_u32), &chan->read[2]);
                /* Fallthrough */
            case HM2_GTAG_SSI:
                r += hm2_register_tram_read_region(hm2, chan->rw_addr[1], sizeof(rtapi_u32), &chan->read[1]);
                r += hm2_register_tram_read_region(hm2, chan->rw_addr[0], sizeof(rtapi_u32), &chan->read[0]);
                break;
            case HM2_GTAG_BISS:
                // BiSS is different, it reads multiple times from the same address
                r += hm2_register_tram_read_region(hm2, chan->rw_addr[0], sizeof(rtapi_u32), &chan->read[0]);
                if (chan->num_read_bits > 32) {
                    r += hm2_register_tram_read_region(hm2, chan->rw_addr[0], sizeof(rtapi_u32), &chan->read[1]);
                }
                if (chan->num_read_bits > 64) {
                    r += hm2_register_tram_read_region(hm2, chan->rw_addr[0], sizeof(rtapi_u32), &chan->read[2]);
                }
    ...
    if (hm2->config.num_dplls == 0) {
        hal_export_functf(hm2_absenc_trigger, hm2, 0, 0, hm2->llio->comp_id, "%s.trigger-encoders", hm2->llio->name);
        funct_flag = true;
    }
...
int hm2_absenc_setup_ssi(hostmot2_t *hm2, hm2_sserial_remote_t *chan, hm2_module_descriptor_t *md) {

    if (hm2_sserial_create_pins(hm2, chan))
        return -EINVAL;
    ...
    chan->rw_addr[0] = md->base_address + (0 * md->register_stride) + chan->index * md->instance_stride;
    chan->rw_addr[1] = md->base_address + (1 * md->register_stride) + chan->index * md->instance_stride;
    chan->reg_cs_addr = md->base_address + (2 * md->register_stride) + chan->index * md->instance_stride;
...
int hm2_absenc_setup_biss(hostmot2_t *hm2, hm2_sserial_remote_t *chan, hm2_module_descriptor_t *md) {
    ...
    chan->rw_addr[0] = md->base_address + (0 * md->register_stride) + chan->index * md->instance_stride;
    chan->rw_addr[1] = md->base_address + (1 * md->register_stride) + chan->index * md->instance_stride;
    chan->rw_addr[2] = md->base_address + (2 * md->register_stride) + chan->index * md->instance_stride;
    hm2->absenc.biss_global_start_addr = md->base_address + (3 * md->register_stride);
    chan->data_written[0] = 0;
...
int hm2_absenc_setup_fabs(hostmot2_t *hm2, hm2_sserial_remote_t *chan, hm2_module_descriptor_t *md) {
    ...
    chan->rw_addr[1] = md->base_address + (1 * md->register_stride) + chan->index * md->instance_stride;
    chan->rw_addr[2] = md->base_address + (2 * md->register_stride) + chan->index * md->instance_stride;
    chan->reg_cs_addr = md->base_address + (3 * md->register_stride) + chan->index * md->instance_stride;
    chan->data_reg_addr = md->base_address + (4 * md->register_stride) + chan->index * md->instance_stride;
    hm2->absenc.fabs_global_start_addr = md->base_address + (5 * md->register_stride);
    chan->data_written[0] = 0;

    if (hal_param_u32_newf(HAL_RW, &(chan->params->u32_param), hm2->llio->comp_id, "%s.filter", chan->name)) {
    ...
    for (index = 0; index < md->instances; index++) {
        ...
        rtapi_list_for_each(ptr, &hm2->config.absenc_formats) {
            ...
            if (def->index > md->instances && def->gtag == md->gtag) {
                HM2_ERR("You have defined a configuration string for %s number"
                        ...
                        hm2_get_general_function_name(md->gtag), def->index, md->instances, md->instances > 1 ? "are" : "is");
                goto fail1;
            }
            if (index == def->index && md->gtag == def->gtag) {
                has_format = true;
                hm2->absenc.num_chans += 1;
                hm2->absenc.chans = rtapi_krealloc(hm2->absenc.chans, hm2->absenc.num_chans * sizeof(hm2_sserial_remote_t), RTAPI_GFP_KERNEL);
                chan = &hm2->absenc.chans[hm2->absenc.num_chans - 1];
                memset(chan, 0, sizeof(hm2_sserial_remote_t));
                chan->index = index;
                chan->myinst = md->gtag;

                if (hm2_absenc_parse_format(chan, def))
                    goto fail1;

                switch (md->gtag) {
                    case HM2_GTAG_SSI:
                        rtapi_snprintf(chan->name, sizeof(chan->name), "%s.ssi.%02d", hm2->llio->name, index);
                        if (hm2_absenc_setup_ssi(hm2, chan, md))
                            goto fail1;
                        break;
                    case HM2_GTAG_BISS:
                        rtapi_snprintf(chan->name, sizeof(chan->name), "%s.biss.%02d", hm2->llio->name, index);
                        if (hm2_absenc_setup_biss(hm2, chan, md))
                            goto fail1;
                        break;
                    case HM2_GTAG_FABS:
                        rtapi_snprintf(chan->name, sizeof(chan->name), "%s.fanuc.%02d", hm2->llio->name, index);
                        if (hm2_absenc_setup_fabs(hm2, chan, md))
                            goto fail1;
                        break;
                    default:
                ...
                if (hal_pin_bit_newf(HAL_OUT, &(chan->params->error), hm2->llio->comp_id, "%s.data-invalid", chan->name)) {
                    HM2_ERR("error adding %s over-run pin, aborting\n", chan->name);
                    return -EINVAL;
                }
                // And Params
                if (hal_param_float_newf(HAL_RW, &(chan->params->float_param), hm2->llio->comp_id, "%s.frequency-khz", chan->name)) {
                    HM2_ERR("error adding frequency param for %s, aborting\n", chan->name);
                    return -EINVAL;
                }
                if (hal_param_u32_newf(HAL_RW, &(chan->params->timer_num), hm2->llio->comp_id, "%s.timer-number", chan->name)) {
        ...
        if (has_format == false) {
            ...
            for (pin = 0; pin < hm2->num_pins; pin++) {
                if (hm2->pin[pin].sec_tag == md->gtag && hm2->pin[pin].sec_unit == index) {
...
void hm2_absenc_process_tram_read(hostmot2_t *hm2, long period) {
    ...
    for (i = 0; i < hm2->absenc.num_chans; i++) {
        ...
        if ((chan->myinst == HM2_GTAG_FABS) && (*chan->read[2] & 0x80000000)) {
        ...
        if (((chan->myinst == HM2_GTAG_SSI) && (*hm2->absenc.ssi_busy_flags & (1 << chan->index))) || ((chan->myinst == HM2_GTAG_BISS) && (*hm2->absenc.biss_busy_flags & (1 << chan->index))) || ((chan->myinst == HM2_GTAG_FABS) && (*hm2->absenc.fabs_busy_flags & (1 << chan->index)))) {
            ...
            if (err_count[i] > 5000 && err_tag[i] == 0) {
                HM2_ERR("Data transmission not complete on channel %s read."
                        ...
                        chan->name, (chan->params->timer_num == 0) ? "the trigger function" : "the hm2dpll timer");
                err_tag[i] = 1;
            }
        }
...
void hm2_absenc_write(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->absenc.num_chans; i++) {
        ...
        switch (chan->myinst) {
            case HM2_GTAG_SSI:
                if (chan->params->timer_num > 4)
                    chan->params->timer_num = 4;
                buff = ((rtapi_u32)(0x10000 * (chan->params->float_param * 1000 / hm2->absenc.clock_frequency))) << 16 | chan->params->timer_num << 12 | (chan->params->timer_num == 0) << 8 | (chan->params->timer_num != 0) << 9 | chan->num_read_bits;
            ...
            case HM2_GTAG_BISS:
                if (chan->params->timer_num > 4)
                    chan->params->timer_num = 4;
                dds = ((rtapi_u32)(0x10000 * (chan->params->float_param * 1000 / hm2->absenc.clock_frequency)));
                filt = 0x8000 / dds; // RX data filter set to 1/2 a clock period
                if (filt > 63) {
                    filt = 63;
                } // bound so we dont splatter into adjacent fields
                buff = dds << 16 | filt << 10 | chan->num_read_bits;
                if (buff != chan->data_written[0]) {
                    HM2_PRINT("BISS DDS set to %d\n", dds);
                    HM2_PRINT("BISS Filter set to %d\n", filt);
                    hm2->llio->write(hm2->llio, chan->rw_addr[1], &buff, sizeof(rtapi_u32));
                    chan->data_written[0] = buff;
                }
                buff2 = chan->params->timer_num << 12 | (chan->params->timer_num == 0) << 8 | (chan->params->timer_num != 0) << 9;
                if (buff2 != chan->data_written[1]) {
                    hm2->llio->write(hm2->llio, chan->rw_addr[2], &buff2, sizeof(rtapi_u32));
            ...
            case HM2_GTAG_FABS:
                if (chan->params->timer_num > 4)
                    chan->params->timer_num = 4;
                if (chan->params->u32_param > 15)
                    chan->params->u32_param = 15;
                buff3 = chan->num_read_bits << 24 | (rtapi_u32)(8.0e-6 * hm2->absenc.clock_frequency) << 14;
                buff2 = chan->params->u32_param << 28 | ((rtapi_u32)(0x100000 * (chan->params->float_param * 1000 / hm2->absenc.clock_frequency)));
                buff = chan->params->timer_num << 12 | (chan->params->timer_num == 0) << 8 | (chan->params->timer_num != 0) << 9 | (buff3 != chan->data_written[2] || buff2 != chan->data_written[1]) << 7;
                if (buff != chan->data_written[0]) {
                ...
                if (buff2 != chan->data_written[1]) {
                    hm2->llio->write(hm2->llio, chan->data_reg_addr, &buff, sizeof(rtapi_u32));
                    chan->data_written[1] = buff2;
                }
                if (buff3 != chan->data_written[2]) {
                    hm2->llio->write(hm2->llio, chan->rw_addr[2], &buff, sizeof(rtapi_u32));
...
void hm2_absenc_print_module(hostmot2_t *hm2) {
    ...
    HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n", hm2->absenc.clock_frequency, hm2_hz_to_mhz(hm2->absenc.clock_frequency));
