...
int hm2_uart_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_uarts > md->instances) {
        HM2_ERR("config defines %d uarts, but only %d are available, "
                "not loading driver\n",
                hm2->config.num_uarts, md->instances);
        return -EINVAL;
    }
    ...
    if (hm2->uart.num_instances == 0) {
        ...
        hm2->uart.instance = (hm2_uart_instance_t *)hal_malloc(hm2->uart.num_instances * sizeof(hm2_uart_instance_t));
...
int hm2_uart_send(char *name, unsigned char data[], int count) {
    ...
    while (c < count - 3) {
        ...
        r = hm2->llio->write(hm2->llio, hm2->uart.instance[inst].tx4_addr, &buff, sizeof(rtapi_u32));
        if (r < 0) {
    ...
    switch (count - c) {
        ...
        case 1:
            buff = data[c];
            r = hm2->llio->write(hm2->llio, hm2->uart.instance[inst].tx1_addr, &buff, sizeof(rtapi_u32));
            if (r < 0) {
                HM2_ERR("UART WRITE: hm2->llio->write failure %s\n", name);
                return r;
            } else {
        ...
        case 2:
            ...
            r = hm2->llio->write(hm2->llio, hm2->uart.instance[inst].tx2_addr, &buff, sizeof(rtapi_u32));
            if (r < 0) {
        ...
        case 3:
            ...
            r = hm2->llio->write(hm2->llio, hm2->uart.instance[inst].tx3_addr, &buff, sizeof(rtapi_u32));
            if (r < 0) {
                HM2_ERR("UART WRITE: hm2->llio->write failure %s\n", name);
                return r;
            } else {
...
int hm2_uart_read(char *name, unsigned char data[]) {
    ...
    r = hm2->llio->read(hm2->llio, hm2->uart.instance[inst].rx_fifo_count_addr, &buff, sizeof(rtapi_u32));

    count = buff & 0x1F;
    c = 0;
    while (c < count - 3 && c < 16) {
        r = hm2->llio->read(hm2->llio, hm2->uart.instance[inst].rx4_addr, &buff, sizeof(rtapi_u32));
    ...
    switch (count - c) {
        case 0:
            return c;
        case 1:
            r = hm2->llio->read(hm2->llio, hm2->uart.instance[inst].rx1_addr, &buff, sizeof(rtapi_u32));
            if (r < 0) {
            ...
            return c + 1;
        case 2:
            r = hm2->llio->read(hm2->llio, hm2->uart.instance[inst].rx2_addr, &buff, sizeof(rtapi_u32));
            if (r < 0) {
                HM2_ERR("UART READ: hm2->llio->read failure %s\n", name);
                return r;
            }
            ...
            return c + 2;
        case 3:
            r = hm2->llio->read(hm2->llio, hm2->uart.instance[inst].rx3_addr, &buff, sizeof(rtapi_u32));
            if (r < 0) {
                HM2_ERR("UART READ: hm2->llio->read failure %s\n", name);
                return r;
            }
...
void hm2_uart_print_module(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->uart.num_instances; i++) {
        HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n", hm2->uart.instance[i].clock_freq, hm2_hz_to_mhz(hm2->uart.instance[i].clock_freq));
