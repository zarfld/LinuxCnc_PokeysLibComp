...
static void hm2_encoder_update_control_register(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->encoder.num_instances; i++) {
        ...
        do_flag(&hm2->encoder.control_reg[i], index_enable, HM2_ENCODER_LATCH_ON_INDEX);

        do_flag(&hm2->encoder.control_reg[i], !index_enable && latch_enable, HM2_ENCODER_LATCH_ON_PROBE);

        do_flag(&hm2->encoder.control_reg[i], index_enable || latch_enable, HM2_ENCODER_INDEX_JUSTONCE);

        do_flag(&hm2->encoder.control_reg[i], latch_polarity, HM2_ENCODER_PROBE_POLARITY);

        do_flag(&hm2->encoder.control_reg[i], e->hal.param.index_invert, HM2_ENCODER_INDEX_POLARITY);

        do_flag(&hm2->encoder.control_reg[i], e->hal.param.index_mask, HM2_ENCODER_INDEX_MASK);

        do_flag(&hm2->encoder.control_reg[i], e->hal.param.index_mask_invert, HM2_ENCODER_INDEX_MASK_POLARITY);

        do_flag(&hm2->encoder.control_reg[i], e->hal.param.counter_mode, HM2_ENCODER_COUNTER_MODE);

        do_flag(&hm2->encoder.control_reg[i], e->hal.param.filter, HM2_ENCODER_FILTER);
        do_flag(&hm2->encoder.control_reg[i], e->reset_quadrature_error, HM2_ENCODER_QUADRATURE_ERROR);
    }
}
...
static void hm2_encoder_set_dpll_timer_if_present(hostmot2_t *hm2) {
    ...
    hm2->llio->write(hm2->llio, hm2->encoder.dpll_timer_num_addr, &data, sizeof(data));
...
void hm2_encoder_write(hostmot2_t *hm2) {
    ...
    if (hm2->encoder.dpll_timer_num_addr) {
        ...
        if (dpll_timer_num == -1)
            ...
            hm2->encoder.desired_dpll_timer_reg = (dpll_timer_num << 12) | (1 << 15);
        if (hm2->encoder.desired_dpll_timer_reg != hm2->encoder.written_dpll_timer_reg)
            goto force_write;
    }
...
void hm2_encoder_force_write(hostmot2_t *hm2) {
    ...
    hm2->llio->write(hm2->llio, hm2->encoder.latch_control_addr, hm2->encoder.control_reg, (hm2->encoder.num_instances * sizeof(rtapi_u32)));

    for (i = 0; i < hm2->encoder.num_instances; i++) {
        hm2->encoder.instance[i].prev_control = hm2->encoder.control_reg[i];
    }

    hm2->llio->write(hm2->llio, hm2->encoder.timestamp_div_addr, &hm2->encoder.timestamp_div_reg, sizeof(rtapi_u32));
...
int hm2_encoder_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_encoders > md->instances) {
        HM2_ERR("config.num_encoders=%d, but only %d are available, not loading driver\n", hm2->config.num_encoders, md->instances);
        return -EINVAL;
    }
...
static void hm2_encoder_instance_process_tram_read(hostmot2_t *hm2, int instance) {
    ...
    switch (e->state) {
        ...
        case HM2_ENCODER_STOPPED: {
            ...
            reg_count = hm2_encoder_get_reg_count(hm2, instance);
            if (reg_count == e->prev_reg_count && !(e->prev_control & (HM2_ENCODER_LATCH_ON_INDEX | HM2_ENCODER_LATCH_ON_PROBE))) {
        ...
        case HM2_ENCODER_MOVING: {
            ...
            if (reg_count == e->prev_reg_count) {
                ...
                *e->hal.pin.position_interpolated = *e->hal.pin.position;
                ...
                if ((((*e->hal.pin.rawcounts - e->prev_event_rawcounts) == 1) && (e->prev_dS_counts < 0)) || (((*e->hal.pin.rawcounts - e->prev_event_rawcounts) == -1) && (e->prev_dS_counts > 0))) {
                    *e->hal.pin.velocity = 0.0;
                    *e->hal.pin.velocity_rpm = 0.0;
                    *e->hal.pin.position_interpolated = *e->hal.pin.position;
