...
void hm2_tp_pwmgen_handle_pwm_frequency(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->tp_pwmgen.num_instances; i++) {
        ...
        if (deadtime > 511) {
            deadtime = 511;
            hm2->tp_pwmgen.instance[i].hal.param.deadzone = ((double)deadtime * 2.0 * 65536.0 * 1.0e9) / ((double)hm2->tp_pwmgen.clock_frequency * dds);
        ...
        hm2->tp_pwmgen.setup_reg[i] = (((int)(hm2->tp_pwmgen.instance[i].hal.param.sampletime * 1023) << 16) + ((hm2->tp_pwmgen.instance[i].hal.param.faultpolarity != 0) << 15) + (deadtime));
    }
}
...
void hm2_tp_pwmgen_write(hostmot2_t *hm2) {
    ...
    // check pwm frequency
    if (hm2->tp_pwmgen.hal->param.pwm_frequency != hm2->tp_pwmgen.written_pwm_frequency)
        goto force_write;

    // update enable register?
    for (i = 0; i < hm2->tp_pwmgen.num_instances; i++) {
        if ((hm2->tp_pwmgen.instance[i].hal.param.deadzone != hm2->tp_pwmgen.instance[i].written_deadzone) || (hm2->tp_pwmgen.instance[i].hal.param.sampletime != hm2->tp_pwmgen.instance[i].written_sampletime) || (hm2->tp_pwmgen.instance[i].hal.param.faultpolarity != hm2->tp_pwmgen.instance[i].written_faultpolarity))
            goto force_write;
...
int hm2_tp_pwmgen_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_tp_pwmgens > md->instances) {
        HM2_ERR("config.num_3pwmgens=%d, but only %d are available, not loading driver\n", hm2->config.num_tp_pwmgens, md->instances);
        return -EINVAL;
    }
    ...
    // FIXME: r hides the r in enclosing function, and it returns the wrong thing
    {
        ...
        r = hal_param_u32_newf(HAL_RW, &(hm2->tp_pwmgen.hal->param.pwm_frequency), hm2->llio->comp_id, "%s.3pwmgen.frequency", hm2->llio->name);
        if (r < 0) {
...
void hm2_tp_pwmgen_prepare_tram_write(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->tp_pwmgen.num_instances; i++) {
        ...
        if (scaled_Avalue > 1.0)
            scaled_Avalue = 1.0;
        else if (scaled_Avalue < -1.0)
            scaled_Avalue = -1.0;
        if (scaled_Bvalue > 1.0)
            scaled_Bvalue = 1.0;
        else if (scaled_Avalue < -1.0)
            scaled_Avalue = -1.0;
        if (scaled_Cvalue > 1.0)
            scaled_Cvalue = 1.0;
        else if (scaled_Avalue < -1.0)
            scaled_Avalue = -1.0;
        ...
        hm2->tp_pwmgen.pwm_value_reg[i] = (((long)(scaled_Cvalue * (511.0) + 512.0) << 20) + ((long)(scaled_Bvalue * (511.0) + 512.0) << 10) + ((long)(scaled_Avalue * (511.0) + 512.0) << 0));
    }
}
