...
int hm2_dpll_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_dplls > md->instances) {
        ...
        hm2->dpll.num_instances = hm2->config.num_dplls;
    ...
    r = hal_pin_float_newf(HAL_IN, &(hm2->dpll.pins->time1_us), hm2->llio->comp_id, "%s.dpll.01.timer-us", hm2->llio->name);
    r += hal_pin_float_newf(HAL_IN, &(hm2->dpll.pins->time2_us), hm2->llio->comp_id, "%s.dpll.02.timer-us", hm2->llio->name);
    r += hal_pin_float_newf(HAL_IN, &(hm2->dpll.pins->time3_us), hm2->llio->comp_id, "%s.dpll.03.timer-us", hm2->llio->name);
    r += hal_pin_float_newf(HAL_IN, &(hm2->dpll.pins->time4_us), hm2->llio->comp_id, "%s.dpll.04.timer-us", hm2->llio->name);
    r += hal_pin_float_newf(HAL_IN, &(hm2->dpll.pins->base_freq), hm2->llio->comp_id, "%s.dpll.base-freq-khz", hm2->llio->name);
    r += hal_pin_float_newf(HAL_OUT, &(hm2->dpll.pins->phase_error), hm2->llio->comp_id, "%s.dpll.phase-error-us", hm2->llio->name);
    r += hal_pin_u32_newf(HAL_IN, &(hm2->dpll.pins->time_const), hm2->llio->comp_id, "%s.dpll.time-const", hm2->llio->name);
    r += hal_pin_u32_newf(HAL_IN, &(hm2->dpll.pins->plimit), hm2->llio->comp_id, "%s.dpll.plimit", hm2->llio->name);
    r += hal_pin_u32_newf(HAL_OUT, &(hm2->dpll.pins->ddssize), hm2->llio->comp_id, "%s.dpll.ddsize", hm2->llio->name);
    r += hal_pin_u32_newf(HAL_OUT, &(hm2->dpll.pins->prescale), hm2->llio->comp_id, "%s.dpll.prescale", hm2->llio->name);
    if (r < 0) {
    ...
    *hm2->dpll.pins->plimit = 0x400000;
    ...
    r = hm2_register_tram_read_region(hm2, hm2->dpll.hm2_dpll_sync_addr, sizeof(rtapi_u32), &hm2->dpll.hm2_dpll_sync_reg);
    if (r < 0) {
        HM2_ERR("Error registering tram synch write. Aborting\n");
        goto fail0;
    }
    r = hm2_register_tram_read_region(hm2, hm2->dpll.control_reg1_addr, sizeof(rtapi_u32), &hm2->dpll.control_reg1_read);
    if (r < 0) {
...
void hm2_dpll_process_tram_read(hostmot2_t *hm2, long period) {
    ...
    *pins->phase_error = (rtapi_s32)*hm2->dpll.hm2_dpll_sync_reg * (period / 4294967296000.00);
...
void hm2_dpll_write(hostmot2_t *hm2, long period) {
    ...
    if (init_counter < 100) {
        ...
        hm2->llio->write(hm2->llio, hm2->dpll.phase_err_addr, &buff, sizeof(rtapi_u32));
    ...
    *pins->prescale = (0x40000000LL * hm2->dpll.clock_frequency) / ((1LL << *pins->ddssize) * *pins->base_freq * 1000.0);

    if (*pins->prescale < 1)
        *pins->prescale = 1;

    buff = (rtapi_u32)((*pins->base_freq * 1000.0 * (1LL << *pins->ddssize) * *pins->prescale) / hm2->dpll.clock_frequency);

    if (buff != hm2->dpll.base_rate_written) {
        hm2->llio->write(hm2->llio, hm2->dpll.base_rate_addr, &buff, sizeof(rtapi_u32));
        hm2->dpll.base_rate_written = buff;
    }
    buff = (rtapi_u32)(*pins->prescale << 24 | *pins->plimit);
    if (buff != hm2->dpll.control_reg0_written) {
        hm2->llio->write(hm2->llio, hm2->dpll.control_reg0_addr, &buff, sizeof(rtapi_u32));
        hm2->dpll.control_reg0_written = buff;
    }
    buff = (rtapi_u32)(*pins->time_const << 16);
    if (buff != hm2->dpll.control_reg1_written) {
        hm2->llio->write(hm2->llio, hm2->dpll.control_reg1_addr, &buff, sizeof(rtapi_u32));
        hm2->dpll.control_reg1_written = buff;
    }
    buff = (rtapi_u32)((-*hm2->dpll.pins->time2_us / period_us) * 0x10000) << 16 | ((rtapi_u32)((-*hm2->dpll.pins->time1_us / period_us) * 0x10000) & 0xFFFF);
    if (buff != hm2->dpll.timer_12_written) {
        hm2->llio->write(hm2->llio, hm2->dpll.timer_12_addr, &buff, sizeof(rtapi_u32));
        hm2->dpll.timer_12_written = buff;
    }
    buff = (rtapi_u32)((-*hm2->dpll.pins->time4_us / period_us) * 0x10000) << 16 | ((rtapi_u32)((-*hm2->dpll.pins->time3_us / period_us) * 0x10000) & 0xFFFF);
    if (buff != hm2->dpll.timer_34_written) {
        hm2->llio->write(hm2->llio, hm2->dpll.timer_34_addr, &buff, sizeof(rtapi_u32));
