...
int hm2_ioport_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->idrom.io_ports != md->instances) {
        HM2_ERR("IDROM IOPorts is %d but MD IOPort NumInstances is %d, inconsistent firmware, aborting driver load\n", hm2->idrom.io_ports, md->instances);
        return -EINVAL;
    }
...
void hm2_ioport_cleanup(hostmot2_t *hm2) {
    ...
    if (hm2->ioport.output_invert_reg != NULL)
        rtapi_kfree(hm2->ioport.output_invert_reg);
    if (hm2->ioport.written_output_invert != NULL)
        rtapi_kfree(hm2->ioport.written_output_invert);
}

static int do_alias(const char *orig_base, const char *alias_base, const char *suffix, int (*funct)(const char *, const char *)) {
...
static void count_instances(hostmot2_t *hm2, int pin, int *this_instance, int *total_instances) {
    ...
    for (i = 0; i != hm2->num_pins; i++) {
        ...
        if (hm2->pin[i].gtag == hm2->pin[pin].gtag && hm2->pin[i].sec_unit == hm2->pin[pin].sec_unit && hm2->pin[i].sec_pin == hm2->pin[pin].sec_pin)
            num_instances++;
    }
...
int hm2_ioport_gpio_export_hal(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->num_pins; i++) {
        ...
        // pins
        r = hal_pin_bit_newf(HAL_OUT, &(hm2->pin[i].instance->hal.pin.in), hm2->llio->comp_id, "%s.gpio.%03d.in", hm2->llio->name, i);
        if (r < 0) {
            HM2_ERR("error %d adding gpio pin, aborting\n", r);
            return -EINVAL;
        }

        r = hal_pin_bit_newf(HAL_OUT, &(hm2->pin[i].instance->hal.pin.in_not), hm2->llio->comp_id, "%s.gpio.%03d.in_not", hm2->llio->name, i);
        if (r < 0) {
            HM2_ERR("error %d adding gpio pin, aborting\n", r);
            return -EINVAL;
        }
        ...
        if ((hm2->pin[i].gtag == HM2_GTAG_IOPORT) || (hm2->pin[i].direction_at_start == HM2_PIN_DIR_IS_OUTPUT)) {

            r = hal_param_bit_newf(HAL_RW, &(hm2->pin[i].instance->hal.param.invert_output), hm2->llio->comp_id, "%s.gpio.%03d.invert_output", hm2->llio->name, i);
            if (r < 0) {
                HM2_ERR("error %d adding gpio param, aborting\n", r);
                return -EINVAL;
            }

            r = hal_param_bit_newf(HAL_RW, &(hm2->pin[i].instance->hal.param.is_opendrain), hm2->llio->comp_id, "%s.gpio.%03d.is_opendrain", hm2->llio->name, i);
            if (r < 0) {
                HM2_ERR("error %d adding gpio param, aborting\n", r);
                return -EINVAL;
            }
        ...
        if (hm2->pin[i].gtag == HM2_GTAG_IOPORT) {
            ...
            r = hal_pin_bit_newf(HAL_IN, &(hm2->pin[i].instance->hal.pin.out), hm2->llio->comp_id, "%s.gpio.%03d.out", hm2->llio->name, i);
            if (r < 0) {
                HM2_ERR("error %d adding gpio pin, aborting\n", r);
                return -EINVAL;
            }
            ...
            *(hm2->pin[i].instance->hal.pin.out) = 0;
            ...
            // parameters
            r = hal_param_bit_newf(HAL_RW, &(hm2->pin[i].instance->hal.param.is_output), hm2->llio->comp_id, "%s.gpio.%03d.is_output", hm2->llio->name, i);
            if (r < 0) {
                HM2_ERR("error %d adding gpio param, aborting\n", r);
                return -EINVAL;
            }
        ...
        if (hm2->pin[i].gtag != HM2_GTAG_IOPORT && hm2->pin[i].direction_at_start == HM2_PIN_DIR_IS_OUTPUT && (gtag_name = hm2_get_general_function_hal_name(hm2->pin[i].gtag)) && (funct_name = hm2_get_pin_secondary_hal_name(&hm2->pin[i]))) {
            int this_instance = -1;
            int total_instances = 0;
            count_instances(hm2, i, &this_instance, &total_instances);
            char orig_base[HAL_NAME_LEN];
            char alias_base[HAL_NAME_LEN];
            size_t ret = snprintf(orig_base, sizeof(orig_base), "%s.gpio.%03d", hm2->llio->name, i);
            ...
            if (total_instances == 0) {
                ...
                size_t ret = snprintf(alias_base, sizeof(alias_base), "%s.%s.%02d.%s", hm2->llio->name, gtag_name, hm2->pin[i].sec_unit, funct_name);
                if (ret >= sizeof(alias_base))
                    return -EINVAL;
            } else {
                size_t ret = snprintf(alias_base, sizeof(alias_base), "%s.%s.%02d.%d.%s", hm2->llio->name, gtag_name, hm2->pin[i].sec_unit, this_instance, funct_name);
                if (ret >= sizeof(alias_base))
                    return -EINVAL;
            }
            r = do_alias(orig_base, alias_base, ".invert_output", hal_param_alias);
            if (r < 0) {
                HM2_ERR("Failed to add %s.invert_output alias, continuing\n", orig_base);
            }
            r = do_alias(orig_base, alias_base, ".is_opendrain", hal_param_alias);
            if (r < 0) {
...
void hm2_ioport_gpio_read(hostmot2_t *hm2) {
    ...
    hm2->llio->read(hm2->llio, hm2->ioport.data_addr, hm2->ioport.data_read_reg, hm2->ioport.num_instances * sizeof(rtapi_u32));
    ...
    for (port = 0; port < hm2->ioport.num_instances; port++) {
        for (unsigned port_pin = 0; port_pin < hm2->idrom.port_width; port_pin++) {
            ...
            if (hm2->pin[io_pin].direction != HM2_PIN_DIR_IS_INPUT)
                continue;
...
void hm2_ioport_gpio_write(hostmot2_t *hm2) {
    ...
    hm2->llio->write(hm2->llio, hm2->ioport.data_addr, hm2->ioport.data_write_reg, hm2->ioport.num_instances * sizeof(rtapi_u32));
}
