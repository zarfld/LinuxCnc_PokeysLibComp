...
int hm2_bspi_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    if (hm2->config.num_bspis > md->instances) {
        HM2_ERR("config defines %d bspis, but only %d are available, "
                "not loading driver\n",
                hm2->config.num_bspis, md->instances);
        return -EINVAL;
    }
    ...
    hm2->bspi.instance = (hm2_bspi_instance_t *)hal_malloc(hm2->bspi.num_instances * sizeof(hm2_bspi_instance_t));
...
int hm2_bspi_setup_chan(char *name, int chan, int cs, int bits, double mhz, int delay, int cpol, int cpha, int noclear, int noecho, int samplelate) {
    hostmot2_t *hm2;
    rtapi_u32 buff = 0;
    int i;
    double board_mhz;
    ...
    buff = (noecho != 0) << 31 | (noclear != 0) << 30 | (samplelate != 0) << 29 | ((delay <= 0) ? 0x10 : (rtapi_u32)((delay * board_mhz / 1000.0) - 1) & 0x1f) << 24 | (cs & 0xF) << 16 | (((rtapi_u16)(board_mhz / (mhz * 2) - 1) & 0xFF)) << 8 | (cpha != 0) << 7 | (cpol != 0) << 6 | (((rtapi_u16)(bits - 1)) & 0x3F);
...
void hm2_bspi_print_module(hostmot2_t *hm2) {
    ...
    for (i = 0; i < hm2->bspi.num_instances; i++) {
        HM2_PRINT("    clock_frequency: %d Hz (%s MHz)\n", hm2->bspi.instance[i].clock_freq, hm2_hz_to_mhz(hm2->bspi.instance[i].clock_freq));
        HM2_PRINT("    instance %d:\n", i);
        HM2_PRINT("    HAL name = %s\n", hm2->bspi.instance[i].name);
        for (j = 0; j < 16; j++) {
            HM2_PRINT("         frame %i config = %08x\n", j, hm2->bspi.instance[i].cd[j]);
