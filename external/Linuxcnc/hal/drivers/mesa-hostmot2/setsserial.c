...
int set_nvram_param(rtapi_u32 addr, rtapi_u32 value) {
    ...
    if (setup_start() < 0)
        goto fail0;
    if (nv_access(LBPNONVOLEEPROM) < 0)
        goto fail0;
...
fail0:            // It's all gone wrong
    buff = 0x800; // Stop
    HM2WRITE(remote->command_reg_addr, buff);
    rtapi_print_msg(RTAPI_MSG_ERR, "Problem with Smart Serial parameter setting, see dmesg\n");
    return -1;
}
            ...
            if (t < erase_sz) { // found a non-zero byte
                ...
                for (i = 0; i < erase_sz; i += write_sz) {
                    ...
                    for (j = 0; j < write_sz; j += 8) {
                        ...
                        m = block_start + i + j;
                        data0 = (fw->data[m] + (fw->data[m + 1] << 8) + (fw->data[m + 2] << 16) + (fw->data[m + 3] << 24));
                        data1 = (fw->data[m + 4] + (fw->data[m + 5] << 8) + (fw->data[m + 6] << 16) + (fw->data[m + 7] << 24));
...
int rtapi_app_main(void) {
    ...
    if (!remote) {
        rtapi_print_msg(RTAPI_MSG_ERR, "Unable to find sserial remote corresponding to %s\n", cmd_list[1]);
        return -1;
    }
    ...
    if (!strncmp("set", cmd_list[0], 3) && cnt == 3) {
        ...
        if (!addr) {
            rtapi_print_msg(RTAPI_MSG_ERR, "Unable to find parameter corresponding to %s\n", cmd_list[1]);
            return -1;
        }
        ...
        rtapi_print_msg(RTAPI_MSG_ERR, "Unknown command or wrong number of parameters to "
                                       "setsserial command");
        return -1;
    }
