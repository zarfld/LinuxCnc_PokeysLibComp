...
static int spi1_transfer(const spix_port_t *sp, uint32_t *wptr, size_t txlen, int rw) {
    ...
    reg_wr(&aux->spi0_cntl0, AUX_SPI_CNTL0_SPEED(rw ? rp->clkdivr : rp->clkdivw) | AUX_SPI_CNTL0_ENABLE | AUX_SPI_CNTL0_MSB_OUT | AUX_SPI_CNTL0_IN_RISING | AUX_SPI_CNTL0_VAR_WIDTH | rp->cemask);
...
static int rpi3_detect(const char *dtcs[]) {
    ...
    for (i = 0; dtcs[i] != NULL; i++) {
        if (!strcmp(dtcs[i], DTC_RPI_MODEL_4B) || !strcmp(dtcs[i], DTC_RPI_MODEL_4CM) || !strcmp(dtcs[i], DTC_RPI_MODEL_3BP) || !strcmp(dtcs[i], DTC_RPI_MODEL_3AP) || !strcmp(dtcs[i], DTC_RPI_MODEL_3B)) {
