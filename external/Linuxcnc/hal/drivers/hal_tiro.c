...
int rtapi_app_main(void) {
    ...
    if ((num_chan <= 0) || (num_chan > MAX_CHAN)) {
        rtapi_print_msg(RTAPI_MSG_ERR, "TIRO: ERROR: invalid num_chan: %d\n", num_chan);
        return -1;
    }
    ...
    for (n = 0; n < num_chan; n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "TIRO: ERROR: counter %d var export failed\n", n + 1);
            hal_exit(comp_id);
            return -1;
        }
        /* init counter */
    ...
    /* export functions */
    retval = hal_export_funct("tiro.capture-position", capture, counter_array, 1, 0, comp_id);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "TIRO: ERROR: capture funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "TIRO: installed %d encoder counters\n", num_chan);
    hal_ready(comp_id);
    return 0;
}
...
static int export_counter(int num, counter_t *addr) {
    ...
    retval = hal_pin_s32_newf(HAL_OUT, &(addr->count), comp_id, "tiro.%d.counts", num);
    if (retval != 0) {
        return retval;
    }
    /* export pin for scaled position captured by update() */
    retval = hal_pin_float_newf(HAL_OUT, &(addr->pos), comp_id, "tiro.%d.position", num);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for scaling */
    retval = hal_param_float_newf(HAL_RW, &(addr->pos_scale), comp_id, "tiro.%d.position-scale", num);
    if (retval != 0) {
        return retval;
    }
