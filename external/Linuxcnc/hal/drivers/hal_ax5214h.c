...
int rtapi_app_main(void) {
    ...
    for (n = 0; n < num_boards; n++) {
        ...
        retval = hal_export_functf(read_board, &(board_array[n]), 0, 0, comp_id, "ax5214h.%d.read", n);
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "AX5214H: ERROR: port %d read funct export failed\n", n);
            hal_exit(comp_id);
            return -1;
        }
        /* export write function */
        retval = hal_export_functf(write_board, &(board_array[n]), 0, 0, comp_id, "ax5214h.%d.write", n);
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "AX5214H: ERROR: port %d write funct export failed\n", n);
            hal_exit(comp_id);
            return -1;
        }
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "AX5214H: installed driver for %d boards\n", num_boards);
    hal_ready(comp_id);
    return 0;
}
...
static int pins_and_params(char *argv[]) {
    ...
    while ((num_boards < MAX_BOARDS) && (argv[n] != 0)) {
        ...
        if (board_addr[num_boards] == 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "AX5124H: ERROR: bad port address '%s'\n", argv[n]);
            return -1;
        }
        n++;
        if (argv[n] == 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "AX5124H: ERROR: no config info for port %s\n", argv[n - 1]);
            return -1;
        }
        ...
        for (m = 0; m < 8; m++) {
            ...
            if ((argv[n][m] == 'i') || (argv[n][m] == 'I')) {
                ...
                rtapi_print_msg(RTAPI_MSG_ERR, "AX5124H: ERROR: bad config info for port %s: '%s'\n", argv[n - 1], argv[n]);
                return -1;
            }
    ...
    for (n = 0; n < num_boards; n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "AX5214H: ERROR: board %d (%04X) var export failed\n", n, board_addr[n]);
            hal_exit(comp_id);
            return -1;
        }
    }
...
static int export_input_pin(int boardnum, int pinnum, io_pin_t *pin) {
    ...
    retval = hal_pin_bit_newf(HAL_OUT, &(pin->data), comp_id, "ax5214h.%d.in-%02d", boardnum, pinnum);
    if (retval != 0) {
        return retval;
    }
    /* export additional pin for inverted input data */
    retval = hal_pin_bit_newf(HAL_OUT, &(pin->io.not ), comp_id, "ax5214h.%d.in-%02d-not", boardnum, pinnum);
    /* initialize HAL pins */
    *(pin->data) = 0;
    *(pin->io.not ) = 1;
    return retval;
...
static int export_output_pin(int boardnum, int pinnum, io_pin_t *pin) {
    ...
    retval = hal_pin_bit_newf(HAL_IN, &(pin->data), comp_id, "ax5214h.%d.out-%02d", boardnum, pinnum);
    if (retval != 0) {
        return retval;
    }
    /* export parameter for polarity */
    retval = hal_param_bit_newf(HAL_RW, &(pin->io.invert), comp_id, "ax5214h.%d.out-%02d-invert", boardnum, pinnum);
