...
int rtapi_app_main(void) {
    ...
    for (i = 0; i < e.num_insts; i++) {
        ...
        if (inst->dir == HAL_OUT) {
            retval = hal_pin_u32_newf(HAL_IN, &(inst->hal[0].en), comp_id, "%s.input", this);
        } else {
            retval = hal_pin_u32_newf(HAL_OUT, &(inst->hal[0].en), comp_id, "%s.output", this);
        }
        v = 0;
        for (j = 1; j <= inst->num_pins; j++) { // 1-based indexing
            ...
            retval = hal_pin_bit_newf(inst->dir, &(inst->hal[j].bit), comp_id, "%s.%s-%s", this, token, (inst->dir == HAL_IN) ? "in" : "out");
            retval += hal_pin_u32_newf(HAL_IN, &(inst->hal[j].en), comp_id, "%s.%s-val", this, token);
            *(inst->hal[j].en) = v++;
