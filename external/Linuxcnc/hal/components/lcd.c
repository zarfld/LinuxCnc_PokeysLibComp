...
int rtapi_app_main(void) {
    ...
    if (lcd == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "lcd component: Out of Memory\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    for (i = 0; i < lcd->num_insts; i++) {
        ...
        for (f = 0; fmt_strings[i][f]; f++) {
            ...
            if (fmt_strings[i][f + 1] == '|' || fmt_strings[i][f + 1] == 0) {
                ...
                retval = snprintf(inst->pages[p].fmt, f - f1 + 2, "%s", fmt_strings[i] + f1);
                inst->pages[p].length = f - f1 + 2;
                inst->pages[p].args = hal_malloc(inst->pages[p].num_args * sizeof(hal_float_t));
                f1 = f + 2;
                {
                    int a = -1, s = -1;
                    ...
                    while (page.fmt[++s]) {
                        ...
                        if (page.fmt[s] == '%') {
                            ...
                            switch (type) {
                                case 'f':
                                    retval = hal_pin_float_newf(HAL_IN, (hal_float_t **)&(inst->pages[p].args[a]), comp_id, "lcd.%02i.page.%02i.arg.%02i", i, p, a);
                                    if (retval != 0) {
                                        return retval;
                                    }
                                    break;
                                case 'u':
                                case 'c':
                                    retval = hal_pin_u32_newf(HAL_IN, (hal_u32_t **)&(inst->pages[p].args[a]), comp_id, "lcd.%02i.page.%02i.arg.%02i", i, p, a);
                                    if (retval != 0) {
                                        return retval;
                                    }
                                    ...
                                    break;
                                case 's':
                                    retval = hal_pin_s32_newf(HAL_IN, (hal_s32_t **)&(inst->pages[p].args[a]), comp_id, "lcd.%02i.page.%02i.arg.%02i", i, p, a);
                                    if (retval != 0) {
                                        return retval;
                                    }
                                    break;
                                case 'b':
                                    retval = hal_pin_bit_newf(HAL_IN, (hal_bit_t **)&(inst->pages[p].args[a]), comp_id, "lcd.%02i.page.%02i.arg.%02i", i, p, a);
                                    if (retval != 0) {
                                        return retval;
                                    }
                                    break;
    ...
    for (i = 0; i < lcd->num_insts; i++) {
        retval = hal_pin_u32_newf(HAL_IN, &(lcd->insts[i].page_num), comp_id, "lcd.%02i.page_num", i);
        if (retval != 0) {
            return retval;
        }
        lcd->insts[i].last_page = -1; // force screen refresh
        retval = hal_pin_u32_newf(HAL_OUT, &(lcd->insts[i].out), comp_id, "lcd.%02i.out", i);
        if (retval != 0) {
            return retval;
        }
        retval = hal_pin_float_newf(HAL_IN, &(lcd->insts[i].contrast), comp_id, "lcd.%02i.contrast", i);
        if (retval != 0) {
            return retval;
        }
        retval = hal_param_u32_newf(HAL_RW, &(lcd->insts[i].dp), comp_id, "lcd.%02i.decimal-separator", i);
        lcd->insts[i].dp = '.';
        if (retval != 0) {
            return retval;
        }
...
static void write_one(lcd_inst_t *inst) {
    ...
    if (*inst->page_num >= inst->num_pages)
    ...
    switch (inst->pages[*inst->page_num].fmt[inst->f_ptr]) {
        case '\\': // escape chars
            ...
            switch (c1) {
                ...
                default: // check for hex
                    ...
                    if (strchr(digits, c1) && strchr(digits, c2)) {
                        inst->buff[0] = (16 * (strchr(digits, c1) - digits) + (strchr(digits, c2) - digits));
                        inst->buff[1] = 0;
                    } else {
                        inst->buff[0] = 0;
                    }
            ...
            *inst->out = inst->buff[0];
            ...
            return;
        case '%':
            retval = parse_fmt(inst->pages[*inst->page_num].fmt, &inst->f_ptr, inst->buff, inst->pages[*inst->page_num].args[inst->a_ptr++], (char)inst->dp);
            if (retval >= 0) {
                *inst->out = inst->buff[0];
                inst->c_ptr = 1;
                inst->f_ptr++;
...
static int parse_fmt(char *in, int *ptr, char *out, void *val, char dp) {
    ...
    const double pow10[] = { 1, 10, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11, 1e12, 1e13, 1e14, 1e15, 1e16, 1e17, 1e18, 1e19, 1e20 };
        ...
        switch (in[*ptr]) {
            ...
            case 'F': // The reason for all this messing about, no %f format in kernel code!!!!
                    ...
                    return 'f';
                {
                    ...
                    if (c < 1)
                        c = num_digits_baseN(v, base) + m + 1 + s;
                    ...
                    return 'b';
                {
                    ...
                    if (in[++(*ptr)] == '\\') {
                        ...
                        if (c2 > '9')
                            c2 &= 0xDF;
                        if (strchr(digits, c1) && strchr(digits, c2)) {
                            bt = (16 * (strchr(digits, c1) - digits) + (strchr(digits, c2) - digits));
                        }
                    } else {
                        bt = in[*ptr];
                    }
                    if (in[++(*ptr)] == '\\') {
                        ...
                        c2 = in[++(*ptr)] & 0xDF;
                        if (strchr(digits, c1) && strchr(digits, c2)) {
                            bf = (16 * (strchr(digits, c1) - digits) + (strchr(digits, c2) - digits));
                        }
                    } else {
                        bf = in[*ptr];
                    }
