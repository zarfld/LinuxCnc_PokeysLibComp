...
void keyup(kb_inst_t *inst) {
    ...
    if (r >= inst->nrows || c >= inst->ncols || inst->hal.key[r * inst->ncols + c] == NULL) {
        return;
    }
...
void keydown(kb_inst_t *inst) {
    ...
    if (inst->num_keys >= inst->param.rollover)
        return;
    inst->num_keys++;
...
void loop(void *arg, long period) {
    ...
    if (inst->scan) { // scanning request
        ...
        if (scan == inst->now[inst->row] && scan != inst->then[inst->row]) {
            ...
            for (c = 0; c < inst->ncols; c++) {
                ...
                if ((inst->then[inst->row] & mask) && !(scan & mask)) { // keyup
                    *inst->hal.keycode = inst->keyup + (inst->row << inst->rowshift) + c;
                    keyup(inst);
                } else if (!(inst->then[inst->row] & mask) && (scan & mask)) { // keydown
                    *inst->hal.keycode = inst->keydown + (inst->row << inst->rowshift) + c;
        ...
        *inst->hal.rows[inst->row] = inst->param.invert;
        inst->row++;
        if (inst->row >= inst->nrows)
            inst->row = 0;
        *inst->hal.rows[inst->row] = !inst->param.invert;
    } else {
        if (*inst->hal.keycode == 0x40)
            return;
...
int rtapi_app_main(void) {
    ...
    if (kb == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb component: Out of Memory\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    for (i = 0; i < kb->num_insts; i++) {
        ...
        if (inst->ncols == 0 || inst->nrows == 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: Invalid size format. should be NxN\n");
            hal_exit(comp_id);
            return -1;
        }

        if (inst->ncols > 32) {
            rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: maximum number of columns is 32. Sorry\n");
            hal_exit(comp_id);
            return -1;
        }
        ...
        for (inst->rowshift = 1; inst->ncols > (1u << inst->rowshift); inst->rowshift++)
            ;
        for (inst->keydown = 0xC0, inst->keyup = 0x80; (inst->nrows << inst->rowshift) > inst->keydown; inst->keydown <<= 1, inst->keyup <<= 1)
            ;
        ...
        for (c = 0; c < inst->ncols; c++) {
            for (r = 0; r < inst->nrows; r++) {
                retval = hal_pin_bit_newf(HAL_OUT, &(inst->hal.key[r * inst->ncols + c]), comp_id, "%s.key.r%xc%x", inst->name, r, c);
                if (retval != 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: Failed to create output pin\n");
                    hal_exit(comp_id);
                    return -1;
                }
            }
        ...
        if (inst->scan) { // internally generated scanning
            ...
            for (r = 0; r < inst->nrows; r++) {
                retval = hal_pin_bit_newf(HAL_OUT, &(inst->hal.rows[r]), comp_id, "%s.row-%02i-out", inst->name, r);
                if (retval != 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: Failed to create output row pin\n");
                    hal_exit(comp_id);
                    return -1;
                }
            }
            for (c = 0; c < inst->ncols; c++) {
                retval = hal_pin_bit_newf(HAL_IN, &(inst->hal.cols[c]), comp_id, "%s.col-%02i-in", inst->name, c);
                if (retval != 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: Failed to create input col pin\n");
                    hal_exit(comp_id);
                    return -1;
                }
            }

            retval = hal_pin_u32_newf(HAL_OUT, &(inst->hal.keycode), comp_id, "%s.keycode", inst->name);
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: Failed to create output pin\n");
                hal_exit(comp_id);
                return -1;
            ...
            retval = hal_param_bit_newf(HAL_RW, &(inst->param.invert), comp_id, "%s.negative-logic", inst->name);
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: Failed to create output pin\n");
                hal_exit(comp_id);
                return -1;
            ...
            retval = hal_param_u32_newf(HAL_RW, &(inst->param.rollover), comp_id, "%s.key_rollover", inst->name);
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: Failed to create rollover param\n");
                hal_exit(comp_id);
                return -1;
            }
        ...
        {
            retval = hal_pin_u32_newf(HAL_IN, &(inst->hal.keycode), comp_id, "%s.keycode", inst->name);
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR, "matrix_kb: Failed to create input pin\n");
                hal_exit(comp_id);
                return -1;
            }
        }
