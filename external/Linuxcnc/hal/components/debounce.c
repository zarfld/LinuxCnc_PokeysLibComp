...
int rtapi_app_main(void) {
    ...
    while ((num_groups < MAX_GROUP) && (cfg[num_groups] != 0)) {
        if ((cfg[num_groups] < 1) || (cfg[num_groups] > MAX_GROUP_SIZE)) {
            rtapi_print_msg(RTAPI_MSG_ERR, "DEBOUNCE: ERROR: bad group size '%d'\n", cfg[num_groups]);
            return -1;
        }
    ...
    if (num_groups == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "DEBOUNCE: ERROR: no channels configured\n");
        return -1;
    }
    ...
    for (n = 0; n < num_groups; n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "DEBOUNCE: ERROR: group %d export failed\n", n);
            hal_exit(comp_id);
            return -1;
        }
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "DEBOUNCE: installed %d groups of debounce filters, %d total\n", num_groups, num_filters);
    hal_ready(comp_id);
    return 0;
}
...
static int export_filter(int num, debounce_t *addr, int group_num) {
    ...
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "DEBOUNCE: ERROR: '%s' pin export failed\n", buf);
        return retval;
    }
    ...
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "DEBOUNCE: ERROR: '%s' param export failed\n", buf);
        return retval;
    }
#endif
