...
int rtapi_app_main(void) {
    ...
    for (n = 0; n < MAX_SUMMERS && wsum_sizes[n] != -1; n++) {
        if (wsum_sizes[n] < 1) {
            rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: Invalid number of bits '%i' for summer %i\n", wsum_sizes[n], n);
            return -1;
        } else {
            num_summers++;
    ...
    if (num_summers == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: no summers specified\n");
        return -1;
    }
    ...
    if (comp_id < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: hal_init() failed\n");
        return -1;
    }
    /* allocate shared memory for summer base info */
    wsum_array = hal_malloc(num_summers * sizeof(wsum_t));
    if (wsum_array == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: hal_malloc() for summer array failed\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    if (wsum_bit_array == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: hal_malloc() for summer bit array failed\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    for (n = 0; n < num_summers; n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: group %d export failed\n", n);
            hal_exit(comp_id);
            return -1;
        }
    ...
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: process_wsums funct export failed\n");
        return -1;
    }

    rtapi_print_msg(RTAPI_MSG_INFO, "WEIGHTED_SUM: installed %d weighted summers\n", num_summers);
    hal_ready(comp_id);
    return 0;
}
...
static int export_wsum(int num, int num_bits, wsum_t *addr, wsum_bit_t *bitaddr) {
    ...
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: '%s' param export failed\n", buf);
        return retval;
    }
    ...
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "WEIGHTED_SUM: ERROR: '%s' pin export failed\n", buf);
        return retval;
    }
