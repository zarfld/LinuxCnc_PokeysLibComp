...
int rtapi_app_main(void) {
    ...
    if ((howmany <= 0) || (howmany > MAX_CHAN)) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SIGGEN: ERROR: invalid number of channels: %d\n", howmany);
        return -1;
    }
    ...
    if (siggen_array == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SIGGEN: ERROR: hal_malloc() failed\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    for (n = 0; n < howmany; n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "SIGGEN: ERROR: siggen %d var export failed\n", n);
            hal_exit(comp_id);
            return -1;
        }
    }
    rtapi_print_msg(RTAPI_MSG_INFO, "SIGGEN: installed %d signal generators\n", howmany);
    hal_ready(comp_id);
    return 0;
}
...
static int export_siggen(int num, hal_siggen_t *addr, char *prefix) {
    ...
    /* export pins */
    retval = hal_pin_float_newf(HAL_OUT, &(addr->square), comp_id, "%s.square", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_float_newf(HAL_OUT, &(addr->sawtooth), comp_id, "%s.sawtooth", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_float_newf(HAL_OUT, &(addr->triangle), comp_id, "%s.triangle", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_float_newf(HAL_OUT, &(addr->sine), comp_id, "%s.sine", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_float_newf(HAL_OUT, &(addr->cosine), comp_id, "%s.cosine", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_bit_newf(HAL_OUT, &(addr->clock), comp_id, "%s.clock", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_float_newf(HAL_IN, &(addr->frequency), comp_id, "%s.frequency", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_float_newf(HAL_IN, &(addr->amplitude), comp_id, "%s.amplitude", prefix);
    if (retval != 0) {
        return retval;
    }
    retval = hal_pin_float_newf(HAL_IN, &(addr->offset), comp_id, "%s.offset", prefix);
    if (retval != 0) {
        return retval;
    }
    ...
    *(addr->offset) = 0.0;
    ...
    retval = hal_export_functf(calc_siggen, &(siggen_array[num]), 1, 0, comp_id, "%s.update", prefix);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SIGGEN: ERROR: update funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    return 0;
