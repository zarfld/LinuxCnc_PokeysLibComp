...
int rtapi_app_main(void) {
    ...
    if ((num_chan <= 0) || (num_chan > MAX_CHAN)) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SUPPLY: ERROR: invalid num_chan: %d\n", num_chan);
        return -1;
    }
    ...
    for (n = 0; n < num_chan; n++) {
        ...
        if (retval != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "SUPPLY: ERROR: var export failed\n");
            hal_exit(comp_id);
            return -1;
        }
    }
...
static int export_supply(int num, hal_supply_t *addr) {
    ...
    *(addr->value) = 0.0;
    ...
    retval = hal_export_functf(update_supply, &(supply_array[num]), 1, 0, comp_id, "supply.%d.update", num);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SUPPLY: ERROR: update funct export failed\n");
        hal_exit(comp_id);
        return -1;
    }
    return 0;
