...
static char axisNames[NUM_AXIS] = { 'x', 'y', 'z', 'a' };
...
static int Plc_ExportFeed(Plc *this, int compId, int id) {
    ...
    // Export pins.
    error = hal_pin_bit_newf(HAL_IN, &this->pCycleStartIn, compId, "boss_plc.%d.cycle-start-in", id);

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pCycleHoldIn, compId, "boss_plc.%d.cycle-hold-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pFeedHoldOut, compId, "boss_plc.%d.feed-hold-out", id);
    }

    if (!error) {
        error = hal_pin_float_newf(HAL_IN, &this->pAdaptiveFeedIn, compId, "boss_plc.%d.adaptive-feed-in", id);
    }

    if (!error) {
        *this->pAdaptiveFeedIn = 1.0;
        error = hal_pin_float_newf(HAL_OUT, &this->pAdaptiveFeedOut, compId, "boss_plc.%d.adaptive-feed-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pToolChangeIn, compId, "boss_plc.%d.tool-change-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pToolChangedOut, compId, "boss_plc.%d.tool-changed-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pWaitUserOut, compId, "boss_plc.%d.wait-user-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pMistOnIn, compId, "boss_plc.%d.mist-on-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pMistOnOut, compId, "boss_plc.%d.mist-on-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pFloodOnIn, compId, "boss_plc.%d.flood-on-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pFloodOnOut, compId, "boss_plc.%d.flood-on-out", id);
    }
...
static int Plc_ExportLimits(Plc *this, int compId, int id) {
    ...
    // Export pins.
    error = hal_pin_bit_newf(HAL_IN, &this->pLimitOverrideIn, compId, "boss_plc.%d.limit-override-in", id);

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pLimitActiveOut, compId, "boss_plc.%d.limit-active-out", id);
    }
    ...
    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pZLimitPosIn, compId, "boss_plc.%d.%c-limit-pos-in", id, axisNames[2]);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pZJogEnIn, compId, "boss_plc.%d.%c-jog-en-in", id, axisNames[2]);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pZLimitNegIn, compId, "boss_plc.%d.%c-limit-neg-in", id, axisNames[2]);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pZLimitPosOut, compId, "boss_plc.%d.%c-limit-pos-out", id, axisNames[2]);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pZLimitNegOut, compId, "boss_plc.%d.%c-limit-neg-out", id, axisNames[2]);
    }

    // Export optional parameters.
    if (debug > 0) {
        if (!error) {
            error = hal_param_u32_newf(HAL_RO, &this->xLimit.state, compId, "boss_plc.%d.%c-limit-state", id, axisNames[0]);
        }

        if (!error) {
            error = hal_param_u32_newf(HAL_RO, &this->yLimit.state, compId, "boss_plc.%d.%c-limit-state", id, axisNames[1]);
        }
    }
...
static int Plc_ExportAmps(Plc *this, int compId, int id) {
    ...
    error = hal_pin_u32_newf(HAL_IO, &this->ampReadyDelay, compId, "boss_plc.%d.amp-ready-delay", id);
...
static int Plc_ExportSpindle(Plc *this, int compId, int id) {
    ...
    // Export parameters.
    error = hal_pin_u32_newf(HAL_IO, &this->brakeOnDelay, compId, "boss_plc.%d.brake-on-delay", id);

    if (!error) {
        error = hal_pin_u32_newf(HAL_IO, &this->brakeOffDelay, compId, "boss_plc.%d.brake-off-delay", id);
    }

    if (!error) {
        error = hal_pin_float_newf(HAL_IO, &this->spindleLoToHi, compId, "boss_plc.%d.spindle-lo-to-hi", id);
    }

    // Export optional parameters.
    if (debug > 0) {
        if (!error) {
            error = hal_param_u32_newf(HAL_RO, &this->spindleState, compId, "boss_plc.%d.spindle-state", id);
        }
    }

    // Export pins.
    if (!error) {
        error = hal_pin_float_newf(HAL_IN, &this->pSpindleSpeedIn, compId, "boss_plc.%d.spindle-speed-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pSpindleIsOnIn, compId, "boss_plc.%d.spindle-is-on-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pSpindleFwdOut, compId, "boss_plc.%d.spindle-fwd-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pSpindleRevOut, compId, "boss_plc.%d.spindle-rev-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pSpindleIncIn, compId, "boss_plc.%d.spindle-inc-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pSpindleDecIn, compId, "boss_plc.%d.spindle-dec-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pSpindleIncOut, compId, "boss_plc.%d.spindle-inc-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pSpindleDecOut, compId, "boss_plc.%d.spindle-dec-out", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pBrakeEnIn, compId, "boss_plc.%d.brake-en-in", id);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pBrakeEnOut, compId, "boss_plc.%d.brake-en-out", id);
    }
...
static int Plc_ExportJog(Plc *this, int compId, int id) {
    ...
    for (i = 0, error = 0; i < NUM_JOG_SEL && !error; i++) {
        error = hal_pin_float_newf(HAL_IO, &this->jogScale[i], compId, "boss_plc.%d.jog-scale-%d", id, i);
    }

    if (!error) {
        for (i = 0; i < NUM_JOG_SEL && !error; i++) {
            error = hal_pin_bit_newf(HAL_IN, &this->pJogSelIn[i], compId, "boss_plc.%d.jog-sel-in-%d", id, i);
        }
    }

    // Export pins.
    if (!error) {
        error = hal_pin_float_newf(HAL_OUT, &this->pJogScaleOut, compId, "boss_plc.%d.jog-scale-out", id);
    }
...
static void Plc_RefreshFeed(Plc *this, long period) {
    ...
    *this->pFeedHoldOut = *this->pCycleHoldIn || (*this->pSpindleSpeedIn && !*this->pSpindleIsOnIn) || (*this->pSpindleIsOnIn && (this->lastSpindleSpeed != *this->pSpindleSpeedIn)) || (*this->pFeedHoldOut && !riseCycleStart);
    ...
    *this->pToolChangedOut = (*this->pToolChangeIn && riseCycleStart) || (*this->pToolChangedOut && *this->pToolChangeIn);

    // Indicates waiting for user to press cycle start.
    *this->pWaitUserOut = *this->pFeedHoldOut || (*this->pToolChangeIn && !*this->pToolChangedOut);
...
static void Plc_RefreshLimits(Plc *this, long period) {
    ...
    *this->pZLimitPosOut = *this->pZLimitPosIn && !(*this->pZJogEnIn && *this->pLimitOverrideIn);
    *this->pZLimitNegOut = *this->pZLimitNegIn && !(*this->pZJogEnIn && *this->pLimitOverrideIn);

    // Generate limit active signal for pilot lamp.
    *this->pLimitActiveOut = Limit_IsActive(&this->xLimit) || Limit_IsActive(&this->yLimit) || *this->pZLimitPosIn || *this->pZLimitNegIn;
}
...
static void Plc_RefreshSpindle(Plc *this, long period) {
    ...
    switch (this->spindleState) {
        ...
        case SS_WAIT_BRAKE_OFF:
            ...
            } else if ((*this->pSpindleSpeedIn != 0.0) && !Timer_IsEnabled(&this->spindleTimer)) {

                this->spindleState = SS_WAIT_ON;

                if (*this->pSpindleSpeedIn > *(this->spindleLoToHi) || (*this->pSpindleSpeedIn < 0.0 && *this->pSpindleSpeedIn >= -*(this->spindleLoToHi))) {
    ...
    *this->pSpindleIncOut = *this->pSpindleIncIn && !*this->pSpindleDecIn && *this->pSpindleIsOnIn;
...
static int Limit_Export(Limit *this, int compId, int id, char axis) {
    ...
    error = hal_pin_float_newf(HAL_IN, &this->pPositionIn, compId, "boss_plc.%d.%c-position-in", id, axis);

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pJogEnIn, compId, "boss_plc.%d.%c-jog-en-in", id, axis);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pIn, compId, "boss_plc.%d.%c-limit-in", id, axis);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pPosOut, compId, "boss_plc.%d.%c-limit-pos-out", id, axis);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pNegOut, compId, "boss_plc.%d.%c-limit-neg-out", id, axis);
    }
...
static int Amp_Export(Amp *this, int compId, int id, char axis) {
    ...
    error = hal_pin_bit_newf(HAL_IN, &this->pEnableIn, compId, "boss_plc.%d.%c-amp-enable-in", id, axis);

    if (!error) {
        error = hal_pin_bit_newf(HAL_IN, &this->pReadyIn, compId, "boss_plc.%d.%c-amp-ready-in", id, axis);
    }

    if (!error) {
        error = hal_pin_bit_newf(HAL_OUT, &this->pFaultOut, compId, "boss_plc.%d.%c-amp-fault-out", id, axis);
    }
...
static void Amp_Refresh(Amp *this, long period, hal_u32_t readyDelay) {
    ...
    *this->pFaultOut = *this->pEnableIn && !*this->pReadyIn && !Timer_IsEnabled(&this->timer);
