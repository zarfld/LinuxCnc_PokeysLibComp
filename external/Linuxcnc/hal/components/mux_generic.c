...
int rtapi_app_main(void) {
    ...
    if (mux == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "mux_generic component: Out of Memory\n");
        hal_exit(comp_id);
        return -1;
    }
    ...
    for (i = 0; i < mux->num_insts; i++) {
        ...
        if (s != 1) {
            ...
            for (p = 0; p < inst->num_bits; p++) {
                retval = hal_pin_bit_newf(HAL_IN, &inst->sel_bit[p], comp_id, "mux-gen.%02i.sel-bit-%02i", i, p);
                if (retval != 0) {
                    goto fail0;
                }
            }
        ...
        retval = hal_pin_u32_newf(HAL_IN, &(inst->sel_int), comp_id, "mux-gen.%02i.sel-int", i);
        if (retval != 0) {
            goto fail0;
        }
        ...
        for (p = 0; p < inst->size; p++) {
            retval = rtapi_snprintf(hal_name, HAL_NAME_LEN, "mux-gen.%02i.in-%s-%02i", i, types[inst->in_type], p);
            if (retval >= HAL_NAME_LEN) {
                goto fail0;
            }
            retval = hal_pin_new(hal_name, inst->in_type, HAL_IN, (void **)&(inst->inputs[p]), comp_id);
            if (retval != 0) {
                goto fail0;
            }
        }
        ...
        // Behaviour-modifiers
        retval = hal_pin_bit_newf(HAL_IN, &inst->suppress, comp_id, "mux-gen.%02i.suppress-no-input", i);
        if (retval != 0) {
            goto fail0;
        }
        retval = hal_pin_u32_newf(HAL_IN, &inst->debounce, comp_id, "mux-gen.%02i.debounce-us", i);
        if (retval != 0) {
            goto fail0;
        }
        retval = hal_param_u32_newf(HAL_RO, &inst->timer, comp_id, "mux-gen.%02i.elapsed", i);
        if (retval != 0) {
            goto fail0;
        }
        retval = hal_param_u32_newf(HAL_RO, &inst->selection, comp_id, "mux-gen.%02i.selected", i);
        if (retval != 0) {
            goto fail0;
        }

        // output pins
        retval = rtapi_snprintf(hal_name, HAL_NAME_LEN, "mux-gen.%02i.out-%s", i, types[inst->out_type]);
        if (retval >= HAL_NAME_LEN) {
            goto fail0;
        }
        retval = hal_pin_new(hal_name, inst->out_type, HAL_OUT, (void **)&(inst->output), comp_id);
        if (retval != 0) {
            goto fail0;
        }
    }
...
void write_fp(void *arg, long period) {
    ...
    switch (inst->in_type * 8 + inst->out_type) {
        ...
        case 021: // HAL_FLOAT => HAL_BIT
            inst->output->b = (inst->inputs[s]->f > EPS || inst->inputs[s]->f < -EPS) ? 1 : 0;
            break;
...
void write_nofp(void *arg, long period) {
    ...
    switch (inst->in_type * 8 + inst->out_type) {
        ...
        case 043: // HAL_U32 => HAL_S32
            inst->output->s = ((unsigned int)inst->inputs[s]->u > MAX_S32) ? MAX_S32 : inst->inputs[s]->u;
            break;
