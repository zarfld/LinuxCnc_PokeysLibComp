...
int init(int argc, char *argv[]) {
    ...
    for (i = 0; i < me->num_insts; i++) {
        ...
        if (hal_pin_u32_newf(HAL_IN, &(hal->keycode), comp_id, "sendkeys.%i.keycode", i) < 0) {
            free(codes);
            free(pins);
            rtapi_print_msg(RTAPI_MSG_ERR, "sendkeys.N.keycode error\n");
            return -ENOMEM;
        }
        if (hal_pin_s32_newf(HAL_OUT, &(hal->current_event), comp_id, "sendkeys.%i.current-event", i) < 0) {
            free(codes);
            free(pins);
            rtapi_print_msg(RTAPI_MSG_ERR, "sendkeys.N.current-event error\n");
            return -ENOMEM;
        }
        if (hal_pin_bit_newf(HAL_IN, &(hal->init), comp_id, "sendkeys.%i.init", i) < 0) {
            free(codes);
            free(pins);
        ...
        for (j = 0; j < param->num_codes; j++) {
            if (hal_param_u32_newf(HAL_RW, &(hal->event[j]), comp_id, "sendkeys.%i.scan-event-%02i", i, j) < 0) {
                free(codes);
                free(pins);
        ...
        for (j = 0; j < param->num_triggers; j++) {
            if (hal_param_u32_newf(HAL_RW, &(hal->event[j + param->num_codes]), comp_id, "sendkeys.%i.pin-event-%02i", i, j) < 0) {
                free(codes);
                free(pins);
        ...
        for (j = 0; j < param->num_triggers; j++) {
            if (hal_pin_bit_newf(HAL_IN, &(hal->trigger[j]), comp_id, "sendkeys.%i.trigger-%02i", i, j) < 0) {
    ...
    while (1) {
        for (i = 0; i < me->num_insts; i++) {
            ...
            if (*hal->init && !param->inited) {
                ...
                if (param->fd < 0) {
                    rtapi_print_msg(RTAPI_MSG_ERR, "Cannot open /dev/uinput. Suggest chmod 666 /dev/uinput\n");
                    abort();
                }
            ...
            if (*hal->keycode != param->oldcode) {
                ...
                if ((int)(*hal->keycode & 0x3F) > param->num_events)
                    continue;
                if (hal->event[*hal->keycode & 0x3F] == 0)
                    continue;
