...
static struct option long_options[] = { { "bits", 1, 0, 'b' },          { "device", 1, 0, 'd' },         { "debug", 0, 0, 'g' },         { "help", 0, 0, 'h' },          { "name", 1, 0, 'n' },        { "parity", 1, 0, 'p' },      { "rate", 1, 0, 'r' },     { "stopbits", 1, 0, 's' }, { "target", 1, 0, 't' }, { "max-frequency", 1, 0, 'F' },
                                        { "min-frequency", 1, 0, 'f' }, { "base-frequency", 1, 0, 'B' }, { "motor-voltage", 1, 0, 'V' }, { "motor-current", 1, 0, 'I' }, { "motor-speed", 1, 0, 'S' }, { "motor-poles", 1, 0, 'P' }, { "register", 1, 0, 'x' }, { "regdump", 0, 0, 'y' },  { 0, 0, 0, 0 } };
...
int read_data(hycomm_param_t *hc_param, hycomm_data_t *hc_data, haldata_t *haldata) {
    ...
    *(haldata->Out_F) = hc_data->ret_data * 0.01;
    ...
    if (*haldata->spindle_on && (fabs(*haldata->speed_command) > 0.0) && (*haldata->Set_F > 0.0)) {
...
int main(int argc, char **argv) {
    ...
    *(haldata->errorcount) = 0;
    ...
    while (done == 0) {
        ...
        if (haldata->looptime > 1.0)
            haldata->looptime = 1.0;
