...
static unsigned char table_crc_hi[] = { 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40, 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40, 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0, 0x80, 0x41, 0x00, 0xC1, 0x81, 0x40,
...
static int check_crc16(hycomm_param_t *hc_param, uint8_t *msg, const int msg_length) {
    int ret;
    uint16_t crc_calc;
    ...
    if (crc_calc == crc_received) {
        ...
        char s_error[64];
        snprintf(s_error, sizeof(s_error), "invalid crc received %0X - crc_calc %0X", crc_received, crc_calc);
        ret = INVALID_CRC;
...
static unsigned int compute_response_length(hycomm_param_t *hc_param, uint8_t *query) {
    (void)hc_param;
    int resp_length;
...
int receive_msg(hycomm_param_t *hc_param, int msg_length_computed, uint8_t *msg, int *msg_length) {
    int select_ret;
    int read_ret;
    fd_set rfds;
    struct timeval tv;
...
static int hycomm_check_response(hycomm_param_t *hc_param, uint8_t *query, uint8_t *response) {
    ...
    ret = receive_msg(hc_param, response_length_computed, response, &response_length);
...
void hycomm_init(hycomm_param_t *hc_param, const char *device, int baud, const char *parity, int data_bit, int stop_bit) {
