...
retCode parse_transaction_section(const int mb_tx_num) {
    ...
    if (iniFindInt(gbl.ini_file_ptr, tag, section, &this_mb_tx->mb_tx_nelem) != 0 && this_mb_tx->mb_tx_names == NULL) {
...
retCode init_mb_links() {
    ...
    for (tx_counter = 0; tx_counter < gbl.tot_mb_tx; tx_counter++) {
        ...
        if (isNewLink != 0) {                                                                        // initialize new link
            ...
            if (this_mb_link->lp_link_type == linkRTU) { // serial
                ...
                this_mb_link->modbus = modbus_new_rtu(this_mb_link->lp_serial_device, this_mb_link->lp_serial_baud, this_mb_link->lp_serial_parity, this_mb_link->lp_serial_data_bit, this_mb_link->lp_serial_stop_bit);
                if (this_mb_link->modbus == NULL) {
                    ERR(gbl.init_dbg, "modbus_new_rtu failed [%s] [%d] [%c] [%d] [%d]", this_mb_link->lp_serial_device, this_mb_link->lp_serial_baud, this_mb_link->lp_serial_parity, this_mb_link->lp_serial_data_bit, this_mb_link->lp_serial_stop_bit);
                    return retERR;
                }
            } else { // tcp
    ...
    for (lk_counter = 0; lk_counter < gbl.tot_mb_links; lk_counter++) {
        ...
        if (this_mb_link->lp_link_type == linkRTU) { // serial
            DBG(gbl.init_dbg, "LINK %d (RTU) link_type[%d] device[%s] baud[%d] data[%d] parity[%c] stop[%d] fd[%d]", lk_counter, this_mb_link->lp_link_type, this_mb_link->lp_serial_device, this_mb_link->lp_serial_baud, this_mb_link->lp_serial_data_bit, this_mb_link->lp_serial_parity, this_mb_link->lp_serial_stop_bit, modbus_get_socket(this_mb_link->modbus));
        } else { // tcp
            DBG(gbl.init_dbg, "LINK %d (TCP) link_type[%d] IP[%s] port[%d] fd[%d]", lk_counter, this_mb_link->lp_link_type, this_mb_link->lp_tcp_ip, this_mb_link->lp_tcp_port, modbus_get_socket(this_mb_link->modbus));
        }
    }
...
retCode init_mb_tx() {
    ...
    for (tx_counter = 0; tx_counter < gbl.tot_mb_tx; tx_counter++) {
        ...
        DBG(gbl.init_dbg, "MB_TX %d lk_n[%d] tx_n[%d] cfg_dbg[%d] lk_dbg[%d] t_inc[%0.3f] nxt_t[%0.3f]", tx_counter, this_mb_tx->mb_link_num, this_mb_tx->mb_tx_num, this_mb_tx->cfg_debug, this_mb_tx->protocol_debug, this_mb_tx->time_increment, this_mb_tx->next_time);
    }
