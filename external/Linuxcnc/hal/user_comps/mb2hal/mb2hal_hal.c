...
retCode create_HAL_pins() {
    ...
    for (tx_counter = 0; tx_counter < gbl.tot_mb_tx; tx_counter++) {
        if (create_each_mb_tx_hal_pins(&gbl.mb_tx[tx_counter]) != retOK) {
            ERR(gbl.init_dbg, "failed to initialize hal pins in tx_num[%d] [%d] [%s]", tx_counter, gbl.mb_tx[tx_counter].mb_tx_fnct, gbl.mb_tx[tx_counter].mb_tx_fnct_name);
            return retERR;
        }
    }
...
retCode create_each_mb_tx_hal_pins(mb_tx_t *mb_tx) {
    ...
    if (mb_tx->num_errors == NULL) {
        ERR(gbl.init_dbg, "[%d] [%s] NULL hal_malloc num_errors", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name);
        return retERR;
    }
    ...
    **(mb_tx->num_errors) = 0;
    ...
    switch (mb_tx->mb_tx_fnct) {
        ...
        case mbtx_15_WRITE_MULTIPLE_COILS:
            ...
            if (mb_tx->bit == NULL || mb_tx->bit_inv == NULL) {
                ERR(gbl.init_dbg, "[%d] [%s] NULL hal_malloc [%d] elements", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, mb_tx->mb_tx_nelem);
                return retERR;
            }
    ...
    for (pin_counter = 0; pin_counter < mb_tx->mb_tx_nelem; pin_counter++) {
        ...
        if (ret >= HAL_NAME_LEN || ret < 0)
            ERR(gbl.init_dbg, "hal pin name too long");
        DBG(gbl.init_dbg, "mb_tx_num [%d] pin_name [%s]", mb_tx->mb_tx_num, hal_pin_name);
        ...
        switch (mb_tx->mb_tx_fnct) {
            ...
            case mbtx_15_WRITE_MULTIPLE_COILS:
                if (gbl.version < 1001) {
                    if (0 != hal_pin_bit_newf(HAL_IN, mb_tx->bit + pin_counter, gbl.hal_mod_id, "%s", hal_pin_name)) {
                        ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_bit_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                        return retERR;
                    }
                } else {
                    if (0 != hal_pin_bit_newf(HAL_IN, mb_tx->bit + pin_counter, gbl.hal_mod_id, "%s.bit", hal_pin_name)) {
                        ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_bit_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                        return retERR;
                    }
                }
            ...
            case mbtx_02_READ_DISCRETE_INPUTS:
                if (gbl.version < 1001) {
                    if (0 != hal_pin_bit_newf(HAL_OUT, mb_tx->bit + pin_counter, gbl.hal_mod_id, "%s", hal_pin_name)) {
                        ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_bit_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                        return retERR;
                    }
                    *mb_tx->bit[pin_counter] = 0;
                } else {
                    if (0 != hal_pin_bit_newf(HAL_OUT, mb_tx->bit + pin_counter, gbl.hal_mod_id, "%s.bit", hal_pin_name)) {
                        ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_bit_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                        return retERR;
                    }
                    if (0 != hal_pin_bit_newf(HAL_OUT, mb_tx->bit_inv + pin_counter, gbl.hal_mod_id, "%s.bit-inv", hal_pin_name)) {
                        ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_bit_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                        return retERR;
                    }
                    *mb_tx->bit[pin_counter] = 0;
            ...
            case mbtx_04_READ_INPUT_REGISTERS:
            case mbtx_03_READ_HOLDING_REGISTERS:
                if (0 != hal_pin_float_newf(HAL_OUT, mb_tx->float_value + pin_counter, gbl.hal_mod_id, "%s.float", hal_pin_name)) {
                    ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_float_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                    return retERR;
                }
                if (0 != hal_pin_s32_newf(HAL_OUT, mb_tx->int_value + pin_counter, gbl.hal_mod_id, "%s.int", hal_pin_name)) {
                    ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_s32_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                    return retERR;
                }
            ...
            case mbtx_16_WRITE_MULTIPLE_REGISTERS:
                if (gbl.version < 1001) {
                    if (0 != hal_pin_float_newf(HAL_IN, mb_tx->float_value + pin_counter, gbl.hal_mod_id, "%s", hal_pin_name)) {
                        ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_float_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                        return retERR;
                    }
                    *mb_tx->float_value[pin_counter] = 0;
                } else {
                    if (0 != hal_pin_float_newf(HAL_IN, mb_tx->float_value + pin_counter, gbl.hal_mod_id, "%s.float", hal_pin_name)) {
                        ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_float_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                        return retERR;
                    }
                    if (0 != hal_pin_s32_newf(HAL_IN, mb_tx->int_value + pin_counter, gbl.hal_mod_id, "%s.int", hal_pin_name)) {
                        ERR(gbl.init_dbg, "[%d] [%s] [%s] hal_pin_s32_newf failed", mb_tx->mb_tx_fnct, mb_tx->mb_tx_fnct_name, hal_pin_name);
                        return retERR;
                    }
                    // if (0 != hal_param_float_newf(HAL_RW, mb_tx->scale + pin_counter, gbl.hal_mod_id,
