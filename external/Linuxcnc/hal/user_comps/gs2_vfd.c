...
static struct option long_options[] = { { "bits", 1, 0, 'b' }, { "device", 1, 0, 'd' }, { "debug", 0, 0, 'g' }, { "help", 0, 0, 'h' }, { "name", 1, 0, 'n' }, { "parity", 1, 0, 'p' }, { "rate", 1, 0, 'r' }, { "stopbits", 1, 0, 's' }, { "target", 1, 0, 't' }, { "verbose", 0, 0, 'v' }, { "accel-seconds", required_argument, NULL, 'A' }, { "decel-seconds", required_argument, NULL, 'D' }, { "braking-resistor", no_argument, NULL, 'R' }, { "disable", no_argument, NULL, 'X' }, { 0, 0, 0, 0 } };
...
static char *ratestrings[] = { "110", "300", "600", "1200", "2400", "4800", "9600", "19200", "38400", "57600", "115200", NULL };
...
int gs2_set_accel_time(modbus_t *mb_ctx, float accel_time) {
    ...
    if (r != 1) {
        ...
        r = modbus_write_register(mb_ctx, GS2_REG_ACCELERATION_TIME_1, data);
        if (r != 1) {
            fprintf(stderr, "failed to set register P0x%04x to 0x%04x (%d): %s\n", GS2_REG_ACCELERATION_TIME_1, data, data, strerror(errno));
            return -1;
        }
    }
...
int gs2_set_decel_time(modbus_t *mb_ctx, float decel_time) {
    ...
    if (r != 1) {
        fprintf(stderr, "failed to set register P0x%04x to 0x%04x: %s\n", GS2_REG_STOP_METHOD, stop_method, strerror(errno));
        return -1;
    }
    ...
    if (r != 1) {
        fprintf(stderr, "failed to set register P0x%04x to 0x%04x (%d): %s\n", GS2_REG_DECELERATION_TIME_1, data, data, strerror(errno));
        return -1;
    }
...
int gs2_set_braking_resistor(modbus_t *mb_ctx, int braking_resistor) {
    ...
    r = modbus_write_register(mb_ctx, GS2_REG_OVER_VOLTAGE_STALL_PREVENTION, data);
    if (r != 1) {
        fprintf(stderr, "failed to set register P0x%04x to 0x%04x: %s\n", GS2_REG_OVER_VOLTAGE_STALL_PREVENTION, data, strerror(errno));
        return -1;
    }
...
void gs2_show_config(modbus_t *mb_ctx) {
    ...
    for (reg = &gs2_register[0]; reg->name != NULL; reg++) {
        ...
        if (r != 1) {
            fprintf(stderr, "failed to read register P%d.%02d (%s)\n", reg->param_group, reg->param_number, reg->name);
            return;
        }
        printf("P%d.%02d %s: 0x%04x (%d)\n", reg->param_group, reg->param_number, reg->name, data, data);
    }
}
...
int read_data(modbus_t *mb_ctx, slavedata_t *slavedata, haldata_t *hal_data_block) {
    ...
    retval = modbus_read_registers(mb_ctx, slavedata->read_reg_start, slavedata->read_reg_count, receive_data);
