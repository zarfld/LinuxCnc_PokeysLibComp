...
static void update_drawing_size(GtkWidget *widget, GdkEventConfigure *event, gpointer data);
...
static void calculate_offset(int chan_num) {
    ...
    scope_data_t *dptr = ctrl_usr->disp_buf + ctrl_usr->vert.data_offset[chan_num];
...
void refresh_display(void) {
    ...
    if (trig_chan >= 0 && trig_chan < 16 && vert->chan_enabled[trig_chan]) {
        draw_triggerline(ctrl_shm->trig_chan, ctrl_shm->trig_chan == vert->selected);
    }
    ...
    for (n = 0; n < 16; n++) {
        if ((vert->chan_enabled[n]) && (vert->data_offset[n] >= 0) && (n + 1 != vert->selected)) {
            draw_waveform(n + 1, FALSE);
        }
    }
    /* draw highlighted waveform (if any) last */
    if (vert->selected > 0 && (vert->chan_enabled[vert->selected - 1]) && (vert->data_offset[vert->selected - 1] >= 0)) {
...
int normal_colors[16][3] = {
    { 204, 0, 0 }, { 0, 204, 204 }, { 102, 204, 0 }, { 102, 0, 204 }, { 204, 153, 0 }, { 0, 204, 51 }, { 0, 51, 204 }, { 204, 0, 153 }, { 153, 95, 61 }, { 141, 153, 61 }, { 72, 153, 61 }, { 61, 153, 118 }, { 61, 118, 153 }, { 72, 61, 153 }, { 141, 61, 153 }, { 153, 61, 95 },
};

int selected_colors[16][3] = {
    { 255, 204, 204 }, { 204, 255, 255 }, { 229, 255, 204 }, { 229, 204, 255 }, { 255, 242, 204 }, { 204, 255, 216 }, { 204, 216, 255 }, { 255, 204, 242 }, { 229, 186, 160 }, { 220, 229, 160 }, { 169, 229, 160 }, { 160, 229, 203 }, { 160, 203, 229 }, { 169, 160, 229 }, { 220, 160, 229 }, { 229, 160, 186 },
};
...
static void init_display_window(void) {
    ...
    gtk_box_pack_start(GTK_BOX(ctrl_usr->waveform_win), disp->drawing, TRUE, TRUE, 0);
    /* hook up a function to handle expose events */
    g_signal_connect(disp->drawing, "size_allocate", G_CALLBACK(update_drawing_size), NULL);
    g_signal_connect(disp->drawing, "realize", G_CALLBACK(create_context), NULL);
    g_signal_connect(disp->drawing, "draw", G_CALLBACK(refresh_display), NULL);
    g_signal_connect(disp->drawing, "button_release_event", G_CALLBACK(handle_release), NULL);
    g_signal_connect(disp->drawing, "button_press_event", G_CALLBACK(handle_click), NULL);
    g_signal_connect(disp->drawing, "motion_notify_event", G_CALLBACK(handle_motion), NULL);
    g_signal_connect(disp->drawing, "scroll_event", G_CALLBACK(handle_scroll), NULL);
    gtk_widget_add_events(GTK_WIDGET(disp->drawing), GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK | GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_BUTTON2_MOTION_MASK | GDK_BUTTON1_MOTION_MASK | GDK_SCROLL_MASK);
    ...
    for (i = 0; i < 16; i++) {
        alloc_color(&(disp->color_normal[i]), normal_colors[i][0], normal_colors[i][1], normal_colors[i][2]);
        alloc_color(&(disp->color_selected[i]), selected_colors[i][0], selected_colors[i][1], selected_colors[i][2]);
    }
}
...
static void change_zoom(int dir, int x) {
    ...
    double old_pixels_per_sample, pixels_per_div, pixels_per_sec, new_pixels_per_sample, old_fraction, new_fraction;
    ...
    pixels_per_div = disp->width * 0.1;
    pixels_per_sec = pixels_per_div / horiz->disp_scale;
    disp->pixels_per_sample = new_pixels_per_sample = pixels_per_sec * horiz->sample_period;
...
static double snap(int y) {
    ...
    if (mod > .045)
        new_position = new_position + (.05 - mod);
    if (mod < .005)
...
static int handle_motion(GtkWidget *widget, GdkEventButton *event, gpointer data) {
    ...
    gdk_window_get_device_position(gtk_widget_get_window(disp->drawing), event->device, &x, &y, &mod);
...
static double distance_point_line(int x, int y, int x1, int y1, int x2, int y2) {
    struct pt M = { x2 - x1, y2 - y1 }, Q = { x - x1, y - y1 }, R;
...
static void draw_triggerline(int chan_num, int highlight) {
    ...
    double fp_level = chan->scale * ((chan->position - trig->level) * 10) + chan->vert_offset;
...
static void create_context(void) {
    ...
    disp->surface = gdk_window_create_similar_surface(gtk_widget_get_window(disp->drawing), CAIRO_CONTENT_COLOR, disp->width, disp->height);
