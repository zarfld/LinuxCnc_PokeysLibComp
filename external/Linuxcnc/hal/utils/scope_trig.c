...
static void trigger_selection_made(GtkWidget *treeview, GdkEventButton *event, GtkWidget *dialog);
...
void refresh_trigger(void) {
    ...
    fp_level = chan->scale * ((chan->position - trig->level) * 10) + chan->vert_offset;
...
static void init_trigger_mode_window(void) {
    ...
    trig->auto_button = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(trig->normal_button), _("Auto"));
    /* and a regular button */
    trig->force_button = gtk_button_new_with_label(_("Force"));
    /* now put them into the box */
    gtk_box_pack_start(GTK_BOX(ctrl_usr->trig_mode_win), trig->normal_button, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(ctrl_usr->trig_mode_win), trig->auto_button, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(ctrl_usr->trig_info_win), trig->force_button, FALSE, FALSE, 0);
    /* hook callbacks to buttons */
    g_signal_connect(trig->normal_button, "clicked", G_CALLBACK(normal_button_clicked), NULL);
    g_signal_connect(trig->auto_button, "clicked", G_CALLBACK(auto_button_clicked), NULL);
    g_signal_connect(trig->force_button, "clicked", G_CALLBACK(force_button_clicked), NULL);
...
static void init_trigger_info_window(void) {
    ...
    hbox = gtk_hbox_new_in_box(TRUE, 0, 0, ctrl_usr->trig_info_win, TRUE, TRUE, 0);
    ...
    trig->level_adj = gtk_adjustment_new(TRIG_LEVEL_RESOLUTION / 2, 0, TRIG_LEVEL_RESOLUTION, 1, 1, 0);
    trig->level_slider = gtk_scale_new(GTK_ORIENTATION_VERTICAL, GTK_ADJUSTMENT(trig->level_adj));
    ...
    trig->level = gtk_adjustment_get_value(GTK_ADJUSTMENT(trig->level_adj)) / TRIG_LEVEL_RESOLUTION;
    /* connect the slider to a function that re-sets the trigger level */
    g_signal_connect(trig->level_adj, "value_changed", G_CALLBACK(level_changed), NULL);
    gtk_widget_show(trig->level_slider);
    /* box for the position slider */
    vbox = gtk_vbox_new_in_box(FALSE, 0, 0, hbox, TRUE, TRUE, 0);
    gtk_label_new_in_box(_("Pos"), vbox, FALSE, FALSE, 0);
    trig->pos_adj = gtk_adjustment_new(TRIG_POS_RESOLUTION / 2, 0, TRIG_POS_RESOLUTION, 1, 1, 0);
    trig->pos_slider = gtk_scale_new(GTK_ORIENTATION_VERTICAL, GTK_ADJUSTMENT(trig->pos_adj));
    ...
    trig->position = gtk_adjustment_get_value(GTK_ADJUSTMENT(trig->pos_adj)) / TRIG_POS_RESOLUTION;
    /* connect the slider to a function that re-sets trigger position */
    g_signal_connect(trig->pos_adj, "value_changed", G_CALLBACK(pos_changed), NULL);
    ...
    trig->level_label = gtk_label_new_in_box(" ---- ", ctrl_usr->trig_info_win, FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(ctrl_usr->trig_info_win), trig->edge_button, FALSE, FALSE, 0);
    g_signal_connect(trig->edge_button, "clicked", G_CALLBACK(edge_button_clicked), NULL);
    ...
    gtk_box_pack_start(GTK_BOX(ctrl_usr->trig_info_win), trig->source_button, FALSE, FALSE, 0);
    g_signal_connect(trig->source_button, "clicked", G_CALLBACK(source_button_clicked), NULL);
...
static void dialog_select_trigger_source(void) {
    ...
    dialog = gtk_dialog_new_with_buttons(_("Trigger Source"), NULL, GTK_DIALOG_MODAL, _("_Cancel"), GTK_RESPONSE_CANCEL, NULL);
    ...
    g_signal_connect(trig_list, "button-press-event", G_CALLBACK(trigger_selection_made), dialog);
...
static void trigger_selection_made(GtkWidget *treeview, GdkEventButton *event, GtkWidget *dialog) {
    if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
        ...
        if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(treeview), (int)event->x, (int)event->y, &path, NULL, NULL, NULL)) {
            int *row;
