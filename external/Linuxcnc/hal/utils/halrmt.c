...
typedef enum { cmdHello, cmdSet, cmdGet, cmdQuit, cmdShutdown, cmdHelp, cmdUnknown } commandTokenType;

typedef enum { hcEcho, hcVerbose, hcEnable, hcConfig, hcCommMode, hcCommProt, hcComps, hcPins, hcPinVals, hcSigs, hcSigVals, hcParams, hcParamVals, hcFuncts, hcThreads, hcComp, hcPin, hcPinVal, hcSig, hcSigVal, hcParam, hcParamVal, hcFunct, hcThread, hcLoadRt, hcUnload, hcLoadUsr, hcLinkps, hcLinksp, hcLinkpp, hcNet, hcUnlinkp, hcLock, hcUnlock, hcNewSig, hcDelSig, hcSetP, hcSetS, hcAddF, hcDelF, hcSave, hcStart, hcStop, hcUnknown } halCommandType;

typedef enum { rtNoError, rtHandledNoError, rtStandardError, rtCustomError, rtCustomHandledError } cmdResponseType;

struct option longopts[] = { { "port", 1, NULL, 'p' }, { "name", 1, NULL, 'n' }, { "sessions", 1, NULL, 's' }, { "connectpw", 1, NULL, 'w' }, { "enablepw", 1, NULL, 'e' }, { 0, 0, 0, 0 } };

const char *commands[] = { "HELLO", "SET", "GET", "QUIT", "SHUTDOWN", "HELP", "" };
const char *halCommands[] = { "ECHO", "VERBOSE", "ENABLE", "CONFIG", "COMM_MODE", "COMM_PROT", "COMPS", "PINS", "PINVALS", "SIGNALS", "SIGVALS", "PARAMS", "PARAMVALS", "FUNCTS", "THREADS", "COMP", "PIN", "PINVAL", "SIGNAL", "SIGVAL", "PARAM", "PARAMVAL", "FUNCT", "THREAD", "LOADRT", "UNLOAD", "LOADUSR", "LINKPS", "LINKSP", "LINKPP", "NET", "UNLINKP", "LOCK", "UNLOCK", "NEWSIG", "DELSIG", "SETP", "SETS", "ADDF", "DELF", "SAVE", "START", "STOP", "" };
...
static int doLinkpp(char *first_pin_name, char *second_pin_name, connectionRecType *context) {
    ...
    if (first_pin->type != second_pin->type) {
        snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: pins '%s' and '%s' not of the same type", linenumber, first_pin_name, second_pin_name);
        sockWriteError(nakStr, context);
        return -EINVAL;
    }
...
static int doNewsig(char *name, char *type, connectionRecType *context) {
    ...
    if (strcasecmp(type, "bit") == 0)
        ...
        snprintf(errorStr, sizeof(errorStr), "HAL:%d: Unknown signal type '%s'", linenumber, type);
        sockWriteError(nakStr, context);
        retval = -EINVAL;
    }
    if (retval < 0) {
...
static int doSetp(char *name, char *value, connectionRecType *context) {
    ...
    if (param == 0) {
        ...
        if (pin == 0) {
            rtapi_mutex_give(&(hal_data->mutex));
            snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: parameter or pin '%s' not found\n", linenumber, name);
            sockWriteError(nakStr, context);
            return -EINVAL;
        } else {
            /* found it */
            ...
            if (pin->dir == HAL_OUT) {
                rtapi_mutex_give(&(hal_data->mutex));
                snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: pin '%s' is not writable\n", linenumber, name);
                sockWriteError(nakStr, context);
                return -EINVAL;
            }
            if (pin->signal != 0) {
                rtapi_mutex_give(&(hal_data->mutex));
                snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: pin '%s' is connected to a signal\n", linenumber, name);
                sockWriteError(nakStr, context);
                return -EINVAL;
            }
        ...
        if (param->dir == HAL_RO) {
            rtapi_mutex_give(&(hal_data->mutex));
            rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: param '%s' is not writable\n", linenumber, name);
            return -EINVAL;
        }
...
static int doSets(char *name, char *value, connectionRecType *context) {
    ...
    if (sig == 0) {
        rtapi_mutex_give(&(hal_data->mutex));
        snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: signal '%s' not found\n", linenumber, name);
        sockWriteError(nakStr, context);
        return -EINVAL;
    }
    ...
    if (sig->writers > 0) {
        rtapi_mutex_give(&(hal_data->mutex));
        snprintf(errorStr, sizeof(errorStr), "HAL:%d: ERROR: signal '%s' already has writer(s)\n", linenumber, name);
        sockWriteError(nakStr, context);
        return -EINVAL;
    }
...
static int unloadrt_comp(char *mod_name) {
    ...
    pid = fork();
    if (pid < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: unloadrt fork() failed\n", linenumber);
        /* reconnect to the HAL shmem area */
        comp_id = hal_init(comp_name);
        if (comp_id < 0) {
    ...
    if (pid == 0) {
        ...
        execv(argv[0], argv);
        /* should never get here */
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: execv(%s) failed\n", linenumber, argv[0]);
        exit(1);
    }
    /* this is the parent process, wait for child to end */
    retval = waitpid(pid, &status, 0);
    ...
    /* check result of waitpid() */
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: waitpid(%d) failed\n", linenumber, pid);
        return -1;
    }
    if (WIFEXITED(status) == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: child did not exit normally\n", linenumber);
        return -1;
    }
    retval = WEXITSTATUS(status);
    if (retval != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: rmmod failed, returned %d\n", linenumber, retval);
        return -1;
    }
    /* print success message */
...
static int doLoadUsr(char *args[]) {
    ...
    if (hal_get_lock() & HAL_LOCK_LOAD) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: HAL is locked, loading of programs is not permitted\n", linenumber);
        return -EPERM;
    }
    ...
    while (**args == '-') {
        ...
        while (*cp1 != '\0') {
            if (*cp1 == 'w') {
                ...
                rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: unknown loadusr option '-%c'\n", linenumber, *cp1);
                return -EINVAL;
            }
            cp1++;
        }
    ...
    if (prog_path[0] == '\0') {
        ...
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: Can't find program '%s'\n", linenumber, prog_name);
        return -1;
    }
    ...
    pid = fork();
    if (pid < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: loadusr fork() failed\n", linenumber);
        /* reconnect to the HAL shmem area */
        comp_id = hal_init(comp_name);
        if (comp_id < 0) {
            fprintf(stderr, "halrmt: hal_init() failed after fork: %d\n", comp_id);
    ...
    if (pid == 0) {
        ...
        /* should never get here */
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: execv(%s) failed: %s\n", linenumber, prog_name, strerror(errno));
        exit(1);
    }
    ...
    if (wait_comp_flag) {
        ...
        while (!ready) {
            ...
            count++;
            if (count == 100) {
                fprintf(stderr, "Waiting for component '%s' to become ready.", new_comp_name);
                fflush(stderr);
    ...
    wait_common:
        ...
        if (WIFEXITED(status) == 0) {
            rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: program '%s' did not exit normally\n", linenumber, prog_name);
            return -1;
        }
        if (ignore_flag == 0) {
            retval = WEXITSTATUS(status);
            if (retval != 0) {
                rtapi_print_msg(RTAPI_MSG_ERR, "HAL:%d: ERROR: program '%s' failed, returned %d\n", linenumber, prog_name, retval);
                return -1;
            }
        }
        /* print success message */
...
static void getPinInfo(char *pattern, int valuesOnly, connectionRecType *context) {
    ...
    while (next != 0) {
        ...
        if (strncmp(pattern, pin->name, len) == 0) {
            ...
            if (valuesOnly == 0)
                snprintf(context->outBuf, sizeof(context->outBuf), "PIN %s %s %02d %s %s", pin->name, data_value2((int)pin->type, dptr), comp->comp_id, data_type((int)pin->type), pin_data_dir((int)pin->dir));
            else
                snprintf(context->outBuf, sizeof(context->outBuf), "PINVAL %s %s", pin->name, data_value2((int)pin->type, dptr));
            sockWrite(context);
        }
        next = pin->next_ptr;
    }
...
static void getSigInfo(char *pattern, int valuesOnly, connectionRecType *context) {
    ...
    while (next != 0) {
        ...
        if (strncmp(pattern, sig->name, len) == 0) {
            dptr = SHMPTR(sig->data_ptr);
            if (valuesOnly == 0)
                snprintf(context->outBuf, sizeof(context->outBuf), "SIGNAL %s  %s  %s", sig->name, data_value((int)sig->type, dptr), data_type((int)sig->type));
            else
...
static void getParamInfo(char *pattern, int valuesOnly, connectionRecType *context) {
    ...
    while (next != 0) {
        ...
        if (strncmp(pattern, param->name, len) == 0) {
            ...
            if (valuesOnly == 0)
                snprintf(context->outBuf, sizeof(context->outBuf), "PARAM %s %s %02d %s %s", param->name, data_value((int)param->type, SHMPTR(param->data_ptr)), comp->comp_id, data_type((int)param->type), param_data_dir((int)param->dir));
            else
                snprintf(context->outBuf, sizeof(context->outBuf), "PARAMVAL %s %s", param->name, data_value((int)param->type, SHMPTR(param->data_ptr)));
            sockWrite(context);
        }
        next = param->next_ptr;
    }
...
static void getFunctInfo(char *pattern, connectionRecType *context) {
    ...
    while (next != 0) {
        ...
        if (strncmp(pattern, fptr->name, len) == 0) {
            ...
            snprintf(context->outBuf, sizeof(context->outBuf), "FUNCT %s %02d %08lX %08lX %s %3d", fptr->name, comp->comp_id, (unsigned long)fptr->funct, (unsigned long)fptr->arg, (fptr->uses_fp ? "YES" : "NO"), fptr->users);
            sockWrite(context);
        }
        next = fptr->next_ptr;
    }
...
static void getThreadInfo(char *pattern, connectionRecType *context) {
    ...
    while (next_thread != 0) {
        ...
        if (strncmp(pattern, tptr->name, len) == 0) {
            ...
            if (ret >= sizeof(name)) {
                rtapi_print_msg(RTAPI_MSG_ERR, "unexpected: pin name too long for %s thread", tptr->name);
                runtime_pin_value = 0;
            } else {
                pin = halpr_find_pin_by_name(name);
                if (pin) {
            ...
            snprintf(context->outBuf, sizeof(context->outBuf), "THREAD %s %11u %s %u %u", tptr->name, (unsigned int)tptr->period, (tptr->uses_fp ? "YES" : "NO "), runtime_pin_value, (unsigned int)tptr->maxtime);
            sockWrite(context);
    ...
    while (next != 0) {
        ...
        if (comp->type == COMPONENT_TYPE_REALTIME) {
            ...
            if (comp->insmod_args == 0) {
                ...
                fprintf(dst, "loadrt %s %s\n", comp->name, (char *)SHMPTR(comp->insmod_args));
            }
        }
        next = comp->next_ptr;
    }
...
static void save_params(FILE *dst) {
    ...
    while (next != 0) {
        ...
        if (param->dir != HAL_RO) {
            ...
            fprintf(dst, "setp %s %s\n", param->name, data_value((int)param->type, SHMPTR(param->data_ptr)));
        }
        next = param->next_ptr;
    }
    rtapi_mutex_give(&(hal_data->mutex));
...
static cmdResponseType getCommMode(char *s, connectionRecType *context) {
    ...
    switch (context->commMode) {
        case 0:
            snprintf(context->outBuf, sizeof(context->outBuf), pCommModeStr, "ASCII");
            break;
        case 1:
            snprintf(context->outBuf, sizeof(context->outBuf), pCommModeStr, "BINARY");
            break;
    }
    return rtNoError;
}
...
int commandGet(connectionRecType *context) {
    ...
    switch (cmd) {
        case hcEcho:
            ret = getEcho(pch, context);
            break;
        case hcVerbose:
            ret = getVerbose(pch, context);
            break;
        case hcEnable:
            ret = getEnable(pch, context);
            break;
        case hcConfig:
            ret = getConfig(pch, context);
            break;
        case hcCommMode:
            ret = getCommMode(pch, context);
            break;
        case hcCommProt:
            ret = getCommProt(pch, context);
            break;
        case hcComps:
            ret = getComps(strtok(NULL, delims), context);
            break;
        case hcPins:
            ret = getPins(strtok(NULL, delims), context);
            break;
        case hcPinVals:
            ret = getPinVals(pch, context);
            break;
        case hcSigs:
            ret = getSignals(strtok(NULL, delims), context);
            break;
        case hcSigVals:
            ret = getSignalVals(strtok(NULL, delims), context);
            break;
        case hcParams:
            ret = getParams(strtok(NULL, delims), context);
            break;
        case hcParamVals:
            ret = getParamVals(strtok(NULL, delims), context);
            break;
        case hcFuncts:
            ret = getFuncts(strtok(NULL, delims), context);
            break;
        case hcThreads:
            ret = getThreads(strtok(NULL, delims), context);
            break;
        case hcComp:
            ret = getComp(strtok(NULL, delims), context);
            break;
        case hcPin:
            ret = getPin(strtok(NULL, delims), context);
            break;
        case hcPinVal:
            ret = getPinVal(strtok(NULL, delims), context);
            break;
        case hcSig:
            ret = getSignal(strtok(NULL, delims), context);
            break;
        case hcSigVal:
            ret = getSignalVal(strtok(NULL, delims), context);
            break;
        case hcParam:
            ret = getParam(strtok(NULL, delims), context);
            break;
        case hcParamVal:
            ret = getParamVal(strtok(NULL, delims), context);
            break;
        case hcFunct:
            ret = getFunct(strtok(NULL, delims), context);
            break;
        case hcThread:
            ret = getThread(strtok(NULL, delims), context);
            break;
        case hcLoadRt:;
        case hcUnload:;
        case hcLoadUsr:;
...
static cmdResponseType setNet(char *p1, connectionRecType *context) {
    ...
    do {
        ...
        i++;
        if (i >= MAX_TOK)
            return rtStandardError; // too many pins to link to the net
    } while (1);

    if (i == 0)
        return rtStandardError; // no pins to link to the net
    else if (doNet(p1, argv, context) == 0)
        return rtNoError;
    else
        return rtCustomHandledError;
}
...
int commandSet(connectionRecType *context) {
    ...
    switch (cmd) {
        case hcEcho:
            ret = setEcho(tokens[0], context);
            break;
        case hcVerbose:
            ret = setVerbose(tokens[0], context);
            break;
        case hcEnable:
            ret = setEnable(tokens[0], context);
            break;
        case hcConfig:
            ret = setConfig(tokens[0], context);
            break;
        case hcCommMode:
            ret = setCommMode(tokens[0], context);
            break;
        case hcCommProt:
            ret = setCommProt(tokens[0], context);
            break;
        case hcComps:
            break;
        case hcPins:
        ...
        case hcThread:
            break;
        case hcLoadRt:
            ret = setLoadRt(tokens[0], context);
            break;
        case hcUnload:
            ret = setUnload(tokens[0], context);
            break;
        case hcLoadUsr:
            ret = setLoadUsr(tokens[0], context);
            break;
        case hcLinkps:
            setLinkps(tokens[0], tokens[1], context);
            break;
        case hcLinksp:
            setLinksp(tokens[0], tokens[1], context);
            break;
        case hcLinkpp:
            setLinkpp(tokens[0], tokens[1], context);
            break;
        case hcNet:
            setNet(tokens[0], context);
            break;
        case hcUnlinkp:
            setUnlink(tokens[0], context);
            break;
        case hcLock:
            setLock(tokens[0], context);
            break;
        case hcUnlock:
            setUnlock(tokens[0], context);
            break;
            ;
        case hcNewSig:
            setNewsig(tokens[0], tokens[1], context);
            break;
        case hcDelSig:
            setDelsig(tokens[0], context);
            break;
        case hcSetP:
            setSetp(tokens[0], tokens[1], context);
            break;
        case hcSetS:
            setSets(tokens[0], tokens[1], context);
            break;
        case hcAddF:
            setAddf(tokens[0], tokens[1], tokens[2], context);
            break;
        case hcDelF:
            setDelf(tokens[0], tokens[1], context);
            break;
        case hcSave:
            setSave(tokens[0], tokens[1], context);
            break;
        case hcStart:
            setStart(context);
            break;
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argv, "e:n:p:s:w:", longopts, NULL)) != -1) {
        switch (opt) {
            case 'e':
                snprintf(enablePWD, sizeof(enablePWD), "%s", optarg);
                break;
            case 'n':
            ...
            case 'p':
                ...
                break;
            case 's':
                sscanf(optarg, "%d", &maxSessions);
                break;
            case 'w':
                snprintf(pwd, sizeof(pwd), "%s", optarg);
                break;
        }
    }
                ...
                case 'i':
                    ...
                    if (inifile == NULL) {
                        ...
                        if ((n < argc) && (argv[n][0] != '-')) {
                            ...
                            fprintf(stderr, "No missing INI filename for -i option\n");
                            exit(-1);
                        }
                    }
                    break;
