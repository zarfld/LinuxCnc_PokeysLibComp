...
static char *license = "              Copyright (C) 2003 John Kasunich\n\
...
int main(int argc, gchar *argv[]) {
    ...
    while (1) {
        ...
        switch (c) {
            case 'h':
                rtapi_print_msg(RTAPI_MSG_ERR, _("Usage:\n  halscope [-h] [-i infile] [-o outfile]"
                                                 " [num_samples]\n"));
                return -1;
                break;
            case 'i':
    ...
    if (!halpr_find_funct_by_name("scope.sample")) {
        char buf[1000];
        snprintf(buf, sizeof(buf), EMC2_BIN_DIR "/halcmd loadrt scope_rt num_samples=%d", num_samples);
        if (system(buf) != 0) {
    ...
    if (shm_id < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SCOPE: ERROR: failed to get shared memory (key=0x%x, size=%lu)\n", SCOPE_SHM_KEY, (unsigned long)sizeof(scope_shm_control_t));
        hal_exit(comp_id);
        return -1;
    }
    retval = rtapi_shmem_getptr(shm_id, &shm_base);
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "SCOPE: ERROR: failed to map shared memory\n");
    ...
    g_signal_connect(ctrl_usr->main_win, "destroy", G_CALLBACK(main_window_closed), NULL);
    g_signal_connect(ctrl_usr->main_win, "focus-in-event", G_CALLBACK(set_focus), NULL);
    /* define menu windows */
...
static void about(void) {
    gtk_show_about_dialog(GTK_WINDOW(ctrl_usr->main_win), "copyright", "Copyright (C) 2003 John Kasunich", "license", license, "website", "https://linuxcnc.org/", NULL);
}
...
static void open_configuration(GtkWindow *parent) {
    ...
    filew = gtk_file_chooser_dialog_new(_("Open Configuration File:"), parent, GTK_FILE_CHOOSER_ACTION_OPEN, _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Open"), GTK_RESPONSE_ACCEPT, NULL);
...
static void save_configuration(GtkWindow *parent) {
    ...
    filew = gtk_file_chooser_dialog_new(_("Save Configuration File:"), parent, GTK_FILE_CHOOSER_ACTION_SAVE, _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Save"), GTK_RESPONSE_ACCEPT, NULL);
...
static void define_menubar(GtkWidget *vboxtop) {
    ...
    GtkWidget *menubar, *filemenu, *fileopenconfiguration, *filesaveconfiguration, *fileopendatafile, *filesavedatafile, *filequit, *sep1, *sep2;
    ...
    g_signal_connect_swapped(fileopenconfiguration, "activate", G_CALLBACK(open_configuration), 0);
    ...
    g_signal_connect_swapped(filesaveconfiguration, "activate", G_CALLBACK(save_configuration), 0);
    ...
    g_signal_connect_swapped(fileopendatafile, "activate", G_CALLBACK(menuitem_response), "file/open datafile");
    ...
    g_signal_connect_swapped(filesavedatafile, "activate", G_CALLBACK(save_log_cb), 0);
    ...
    g_signal_connect_swapped(filequit, "activate", G_CALLBACK(quit), 0);
    ...
    g_signal_connect_swapped(helpabout, "activate", G_CALLBACK(about), NULL);
...
static void define_scope_windows(void) {
    ...
    /* left side */
    ctrl_usr->horiz_info_win = gtk_vbox_framed_new_in_box(_("Horizontal"), FALSE, 0, 0, vboxleft, FALSE, FALSE, 1);
    /* horizontal row of select buttons */
    ctrl_usr->waveform_win = gtk_vbox_new_in_box(FALSE, 0, 0, vboxleft, TRUE, TRUE, 0);
    gtk_widget_set_vexpand(ctrl_usr->waveform_win, TRUE);
    gtk_widget_set_hexpand(ctrl_usr->waveform_win, TRUE);
    ctrl_usr->chan_sel_win = gtk_hbox_new_in_box(TRUE, 0, 0, vboxleft, FALSE, FALSE, 0);
    ctrl_usr->chan_info_win = gtk_hbox_framed_new_in_box(_("Selected Channel"), FALSE, 0, 0, vboxleft, FALSE, FALSE, 0);
    /* right side */
    ...
    ctrl_usr->run_mode_win = gtk_vbox_framed_new_in_box(_("Run Mode"), TRUE, 0, 0, vboxleft, FALSE, FALSE, 0);
    ctrl_usr->trig_info_win = gtk_vbox_framed_new_in_box(_("Trigger"), FALSE, 0, 0, vboxright, TRUE, TRUE, 0);
    ctrl_usr->trig_mode_win = gtk_vbox_new_in_box(TRUE, 0, 0, ctrl_usr->trig_info_win, FALSE, FALSE, 0);
    ctrl_usr->vert_info_win = gtk_vbox_framed_new_in_box(_("Vertical"), FALSE, 0, 0, vboxleft, TRUE, TRUE, 0);
...
static void init_run_mode_window(void) {
    ...
    ctrl_usr->rm_normal_button = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(ctrl_usr->rm_stop_button), _("Normal"));
    ctrl_usr->rm_single_button = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(ctrl_usr->rm_stop_button), _("Single"));
    ctrl_usr->rm_roll_button = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(ctrl_usr->rm_stop_button), _("Roll"));
    /* now put them into the box */
    gtk_box_pack_start(GTK_BOX(ctrl_usr->run_mode_win), ctrl_usr->rm_normal_button, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(ctrl_usr->run_mode_win), ctrl_usr->rm_single_button, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(ctrl_usr->run_mode_win), ctrl_usr->rm_roll_button, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(ctrl_usr->run_mode_win), ctrl_usr->rm_stop_button, FALSE, FALSE, 0);
    /* hook callbacks to buttons */
    g_signal_connect(ctrl_usr->rm_normal_button, "clicked", G_CALLBACK(rm_normal_button_clicked), NULL);
    g_signal_connect(ctrl_usr->rm_single_button, "clicked", G_CALLBACK(rm_single_button_clicked), NULL);
    g_signal_connect(ctrl_usr->rm_roll_button, "clicked", G_CALLBACK(rm_roll_button_clicked), NULL);
    g_signal_connect(ctrl_usr->rm_stop_button, "clicked", G_CALLBACK(rm_stop_button_clicked), NULL);
