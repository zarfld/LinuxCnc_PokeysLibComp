...
static gboolean configure_window(GtkWidget *widget, GdkEventConfigure *event, gpointer data);
...
static void init_horiz_window(void) {
    ...
    horiz->zoom_slider = gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, GTK_ADJUSTMENT(horiz->zoom_adj));
    ...
    g_signal_connect(horiz->zoom_adj, "value_changed", G_CALLBACK(zoom_changed), NULL);
    gtk_widget_show(horiz->zoom_slider);
    /* add a slider for position control */
    horiz->pos_adj = gtk_adjustment_new(500, 0, 1000, 1, 1, 0);
    horiz->pos_slider = gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, GTK_ADJUSTMENT(horiz->pos_adj));
    ...
    g_signal_connect(horiz->pos_adj, "value_changed", G_CALLBACK(pos_changed), NULL);
    ...
    horiz->record_button = gtk_button_new_with_label(_("----- Samples\nat ---- kHz"));
    ...
    g_signal_connect(horiz->record_button, "clicked", G_CALLBACK(acquire_popup), NULL);
    ...
    g_signal_connect(horiz->disp_area, "configure_event", G_CALLBACK(configure_window), NULL);
    g_signal_connect(horiz->disp_area, "draw", G_CALLBACK(refresh_pos_disp), NULL);
    g_signal_connect(horiz->disp_area, "button_press_event", G_CALLBACK(horiz_press), 0);
    g_signal_connect(horiz->disp_area, "button_release_event", G_CALLBACK(horiz_release), 0);
    g_signal_connect(horiz->disp_area, "motion_notify_event", G_CALLBACK(horiz_motion), 0);
    gtk_widget_set_events(GTK_WIDGET(horiz->disp_area), GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK);
...
void refresh_state_info(void) {
    scope_horiz_t *horiz;
    static const gchar *state_names[] = { "IDLE", "INIT", "PRE-TRIG", "TRIGGER?", "TRIGGERED", "DONE", "RESET" };
...
static void dialog_realtime_not_loaded(void) {
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(ctrl_usr->main_win), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_NONE, _("Realtime component not loaded"));
    gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), _("HALSCOPE uses a realtime component called scope_rt'\n"
    ...
    gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("OK"), GTK_RESPONSE_OK, _("Quit"), GTK_RESPONSE_CLOSE, NULL);
...
static void dialog_realtime_not_linked(void) {
    ...
    dialog = gtk_dialog_new_with_buttons(title, GTK_WINDOW(ctrl_usr->main_win), GTK_DIALOG_MODAL, _("_OK"), GTK_RESPONSE_OK, _("Quit"), GTK_RESPONSE_CLOSE, NULL);
    ...
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), label, FALSE, TRUE, 5);

    /* a separator */
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), gtk_separator_new(GTK_ORIENTATION_HORIZONTAL), FALSE, FALSE, 0);

    /* thread name display */
    hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);
    gtk_box_set_homogeneous(GTK_BOX(hbox), TRUE);
    gtk_label_new_in_box(_("Thread:"), hbox, TRUE, TRUE, 0);
    horiz->thread_name_label = gtk_label_new_in_box("------", hbox, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), hbox, FALSE, TRUE, 0);

    /* sample period display */
    hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);
    gtk_box_set_homogeneous(GTK_BOX(hbox), TRUE);
    gtk_label_new_in_box(_("Sample Period:"), hbox, TRUE, TRUE, 0);
    horiz->sample_period_label = gtk_label_new_in_box("------", hbox, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), hbox, FALSE, TRUE, 0);
    ...
    horiz->sample_rate_label = gtk_label_new_in_box("------", hbox, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), hbox, FALSE, TRUE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), scrolled_window, TRUE, TRUE, 5);
    ...
    g_signal_connect(selection, "changed", G_CALLBACK(acquire_selection_made), horiz);
    ...
    while (next != 0) {
        ...
        if (thread->period <= 1000000000) {
            ...
            if ((horiz->thread_name != NULL) && (strcmp(horiz->thread_name, thread->name) == 0)) {
    ...
    horiz->mult_spinbutton = gtk_spin_button_new(GTK_ADJUSTMENT(horiz->mult_adj), 1, 0);
    gtk_box_pack_start(GTK_BOX(hbox), horiz->mult_spinbutton, FALSE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), hbox, FALSE, TRUE, 0);

    /* connect the multiplier spinbutton to a function */
    g_signal_connect(horiz->mult_adj, "value_changed", G_CALLBACK(mult_changed), NULL);
    ...
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), label, TRUE, TRUE, 0);
    ...
    buttons[1] = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(buttons[0]), buf);
    snprintf(buf, BUFLEN, _("%5d samples (4 channels)"), ctrl_shm->buf_len / 4);
    buttons[2] = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(buttons[0]), buf);
    snprintf(buf, BUFLEN, _("%5d samples (8 channels)"), ctrl_shm->buf_len / 8);
    buttons[3] = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(buttons[0]), buf);
    snprintf(buf, BUFLEN, _("%5d samples (16 channels)"), ctrl_shm->buf_len / 16);
    buttons[4] = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(buttons[0]), buf);
    /* now put them into the box and make visible */
    for (n = 0; n < 5; n++) {
        gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), buttons[n], FALSE, FALSE, 0);
    }
    ...
    g_signal_connect(buttons[0], "clicked", G_CALLBACK(rec_len_button), (gpointer)1);
    g_signal_connect(buttons[1], "clicked", G_CALLBACK(rec_len_button), (gpointer)2);
    g_signal_connect(buttons[2], "clicked", G_CALLBACK(rec_len_button), (gpointer)4);
    g_signal_connect(buttons[3], "clicked", G_CALLBACK(rec_len_button), (gpointer)8);
    g_signal_connect(buttons[4], "clicked", G_CALLBACK(rec_len_button), (gpointer)16);
...
static void dialog_realtime_not_running(void) {
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(ctrl_usr->main_win), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_NONE, _("Realtime thread(s) not running"));
    gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), _("HALSCOPE uses code in a realtime HAL thread to sample\n"
...
void save_log_cb(GtkWindow *parent) {
    ...
    dialog = gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_OK, _("No channels selected"));
    gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), _("You need to choose at least one channel."));
...
static void log_popup(GtkWindow *parent) {
    ...
    filew = gtk_file_chooser_dialog_new(_("Pick log file to write to:"), parent, GTK_FILE_CHOOSER_ACTION_SAVE, _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Save"), GTK_RESPONSE_ACCEPT, NULL);
...
static void rec_len_button(GtkWidget *widget, gpointer gdata) {
    ...
    if (retval < 0) {
        /* too many channels already enabled */
        dialog = gtk_message_dialog_new(GTK_WINDOW(ctrl_usr->main_win), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_OK, _("Not enough channels"));
        gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), _("This record length cannot handle the channels\n"
...
static gint horiz_motion(GtkWidget *widget, GdkEventMotion *event) {
    ...
    newpos = gtk_adjustment_get_value(GTK_ADJUSTMENT(horiz->pos_adj)) + motion * 100 / scale;
...
static gboolean configure_window(GtkWidget *widget, GdkEventConfigure *event, gpointer data) {
    ...
    horiz->surface = gdk_window_create_similar_image_surface(gtk_widget_get_window(widget), CAIRO_FORMAT_A1, horiz->width, horiz->height, 0);
