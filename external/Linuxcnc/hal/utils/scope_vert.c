...
static void selection_made(GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *col, GtkWidget *dialog);
...
int set_channel_off(int chan_num) {
    ...
    ignore_click = 1;
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(vert->chan_sel_buttons[chan_num - 1]), FALSE);
...
static void init_chan_sel_window(void) {
    ...
    for (n = 0; n < 16; n++) {
        ...
        gtk_box_pack_start(GTK_BOX(ctrl_usr->chan_sel_win), button, TRUE, TRUE, 0);
        gtk_widget_show(button);
        /* hook a callback function to it */
        g_signal_connect(button, "clicked", G_CALLBACK(chan_sel_button), (gpointer)n + 1);
...
static void init_chan_info_window(void) {
    ...
    vert->chan_num_label = gtk_label_new_in_box("--", ctrl_usr->chan_info_win, FALSE, FALSE, 5);
    ...
    gtk_box_pack_start(GTK_BOX(ctrl_usr->chan_info_win), vert->source_name_button, FALSE, FALSE, 3);
    ...
    g_signal_connect(vert->source_name_button, "clicked", G_CALLBACK(change_source_button), NULL);
    gtk_widget_show(vert->source_name_button);

    vert->readout_label = gtk_label_new_in_box("", ctrl_usr->chan_info_win, FALSE, FALSE, 0);
    gtk_label_set_justify(GTK_LABEL(vert->readout_label), GTK_JUSTIFY_LEFT);
    gtk_label_size_to_fit(GTK_LABEL(vert->readout_label), "f(99999.9999) = 99999.9999 (ddt 99999.9999)");
}
...
static void init_vert_info_window(void) {
    ...
    hbox = gtk_hbox_new_in_box(TRUE, 0, 0, ctrl_usr->vert_info_win, TRUE, TRUE, 0);
    ...
    vert->scale_slider = gtk_scale_new(GTK_ORIENTATION_VERTICAL, GTK_ADJUSTMENT(vert->scale_adj));
    ...
    g_signal_connect(vert->scale_adj, "value_changed", G_CALLBACK(scale_changed), NULL);
    gtk_widget_show(vert->scale_slider);
    /* box for the position slider */
    vbox = gtk_vbox_new_in_box(FALSE, 0, 0, hbox, TRUE, TRUE, 0);
    gtk_label_new_in_box(_("Pos"), vbox, FALSE, FALSE, 0);
    vert->pos_adj = gtk_adjustment_new(VERT_POS_RESOLUTION / 2, 0, VERT_POS_RESOLUTION, 1, 1, 0);
    vert->pos_slider = gtk_scale_new(GTK_ORIENTATION_VERTICAL, GTK_ADJUSTMENT(vert->pos_adj));
    ...
    g_signal_connect(vert->pos_adj, "value_changed", G_CALLBACK(pos_changed), NULL);
    ...
    vert->scale_label = gtk_label_new_in_box(" ---- ", ctrl_usr->vert_info_win, FALSE, FALSE, 0);
    /* Offset control */
    vert->offset_button = gtk_button_new_with_label(_("Offset\n----"));
    vert->offset_label = gtk_bin_get_child(GTK_BIN(vert->offset_button));
    gtk_box_pack_start(GTK_BOX(ctrl_usr->vert_info_win), vert->offset_button, FALSE, FALSE, 0);
    g_signal_connect(vert->offset_button, "clicked", G_CALLBACK(offset_button), NULL);
...
static gboolean dialog_set_offset(int chan_num) {
    ...
    vert = &(ctrl_usr->vert);
    chan = &(ctrl_usr->chan[chan_num - 1]);
    snprintf(msg, BUFLEN - 1,
             _("Set the vertical offset\n"
               "for channel %d."),
             chan_num);

    /* create dialog window, disable resizing and place it in center of screen */
    dialog = gtk_dialog_new_with_buttons(_("Set Offset"), NULL, GTK_DIALOG_MODAL, _("_OK"), GTK_RESPONSE_OK, _("_Cancel"), GTK_RESPONSE_CANCEL, NULL);
    ...
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), label, FALSE, FALSE, 5);
    ...
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), vert->offset_ac, FALSE, TRUE, 0);

    vert->offset_entry = gtk_entry_new();
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), vert->offset_entry, FALSE, TRUE, 0);
    ...
    /* signals */
    g_signal_connect(vert->offset_ac, "toggled", G_CALLBACK(offset_changed), &data);
    g_signal_connect(vert->offset_entry, "changed", G_CALLBACK(offset_changed), &data);
    g_signal_connect(vert->offset_entry, "activate", G_CALLBACK(offset_activated), dialog);
...
static void offset_changed(GtkEditable *editable, struct offset_data *data) {
    ...
    data->ac_coupled = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctrl_usr->vert.offset_ac));
...
static void chan_sel_button(GtkWidget *widget, gpointer gdata) {
    ...
    if (vert->chan_enabled[chan_num - 1] == 0) {
        ...
        if (count >= ctrl_shm->sample_len) {
            ...
            dialog = gtk_message_dialog_new(GTK_WINDOW(ctrl_usr->main_win), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE, _("Too many channels"));
            gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), _("You cannot add another channel.\n\n"
...
static void change_page(GtkNotebook *notebook, GtkWidget *page, guint page_num, gpointer user_data) {
    (void)notebook;
    (void)page;
    (void)user_data;
    scope_vert_t *vert;
...
static gboolean dialog_select_source(int chan_num) {
    ...
    dialog = gtk_dialog_new_with_buttons(title, NULL, GTK_DIALOG_MODAL, _("_OK"), GTK_RESPONSE_ACCEPT, _("_Cancel"), GTK_RESPONSE_CANCEL, NULL);
    ...
    gtk_box_pack_start(GTK_BOX(GTK_CONTAINER(content_area)), vert->notebook, TRUE, TRUE, 0);
    ...
    for (n = 0; n < 3; n++) {
        ...
        g_signal_connect(vert->lists[n], "row-activated", G_CALLBACK(selection_made), dialog);
        g_signal_connect(gtk_tree_view_get_selection(GTK_TREE_VIEW(vert->lists[n])), "changed", G_CALLBACK(selection_changed), signal_name);
    }

    /* signals */
    g_signal_connect(vert->notebook, "switch-page", G_CALLBACK(change_page), vert);
...
void channel_changed(void) {
    ...
    gtk_adjustment_set_value(GTK_ADJUSTMENT(vert->pos_adj), chan->position * VERT_POS_RESOLUTION);
...
static void style_with_css(GtkWidget *widget, int color_index) {
    ...
    snprintf(buf, sizeof(buf),
             ...
             normal_colors[color_index][0], normal_colors[color_index][1], normal_colors[color_index][2], normal_colors[color_index][0], normal_colors[color_index][1], normal_colors[color_index][2], normal_colors[color_index][0], normal_colors[color_index][1], normal_colors[color_index][2]);
