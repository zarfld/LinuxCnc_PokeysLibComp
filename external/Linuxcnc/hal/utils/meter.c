...
static void page_switched(GtkNotebook *notebook, GtkWidget *page, guint page_num, gpointer user_data);
...
int main(int argc, gchar *argv[]) {
    ...
    if (!small) {
        ...
        g_signal_connect(button_select, "clicked", G_CALLBACK(popup_probe_window), meter->probe);
...
static char *data_value(int type, void *valptr) {
    ...
    switch (type) {
        ...
        case HAL_S32:
            ...
            break;
        case HAL_U32:
            snprintf(buf, 24, "%10lu (0x%08lX)", (unsigned long)*((hal_u32_t *)valptr), *((unsigned long *)valptr));
            value_str = buf;
            break;
        default:
...
static void create_probe_window(probe_t *probe) {
    ...
    for (n = 0; n < 3; n++) {
        ...
        gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(probe->lists[n]), FALSE);
        ...
        g_signal_connect_swapped(probe->lists[n], "row-activated", G_CALLBACK(gtk_widget_destroy), probe->window);
    ...
    /* signals */
    g_signal_connect(probe->notebook, "switch-page", G_CALLBACK(page_switched), probe);
    g_signal_connect_swapped(button_close, "clicked", G_CALLBACK(gtk_widget_destroy), probe->window);
    g_signal_connect(probe->window, "destroy", G_CALLBACK(gtk_widget_destroyed), &(probe->window));
}
