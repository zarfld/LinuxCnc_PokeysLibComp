...
static const char *command_table[] = {
    "loadrt", "loadusr", "unload", "lock", "unlock", "linkps", "linksp", "linkpp", "unlinkp", "net", "newsig", "delsig", "getp", "gets", "setp", "sets", "ptype", "stype", "addf", "delf", "show", "list", "status", "save", "source", "start", "stop", "quit", "exit", "help", "alias", "unalias", NULL,
};
...
static const char *show_table[] = {
    "all", "alias", "comp", "pin", "sig", "param", "funct", "thread", NULL,
};

static const char *save_table[] = {
    "all", "alias", "comp", "sig", "link", "linka", "net", "neta", "param", "thread", NULL,
};

static const char *list_table[] = { "comp", "alias", "pin", "sig", "param", "funct", "thread", NULL };

static const char *status_table[] = { "alias", "lock", "mem", "all", NULL };

static const char *pintype_table[] = { "bit", "float", "u32", "s32", NULL };
...
static int direction_match(hal_pin_dir_t dir1, hal_pin_dir_t dir2) {
    if (dir1 == HAL_DIR_UNSPECIFIED || dir2 == -1)
        return 1;
...
static char *funct_generator_common(const char *text, int state, int inuse) {
    ...
    while (next) {
        ...
        next = funct->next_ptr;
        if ((strncmp(text, funct->name, len) == 0) && (inuse == funct->users))
...
static char *signal_generator(const char *text, int state) {
    ...
    while (next) {
        hal_sig_t *sig = SHMPTR(next);
        next = sig->next_ptr;
        if (match_type != HAL_TYPE_UNSPECIFIED && match_type != sig->type)
            continue;
        if (!writer_match(match_direction, sig->writers))
            continue;
...
static char *setp_generator(const char *text, int state) {
    ...
    while (next) {
        hal_pin_t *pin = SHMPTR(next);
        next = pin->next_ptr;
        if (pin->dir != HAL_OUT && pin->signal == 0 && strncmp(text, pin->name, len) == 0)
            return strdup(pin->name);
    }
...
static char *parameter_alias_generator(const char *text, int state) {
    ...
    while (next) {
        ...
        next = param->next_ptr;
        if (param->oldname == 0)
            continue; // no alias here, move along
        if (strncmp(text, param->name, len) == 0)
            return strdup(param->name);
    }
    return NULL;
...
static char *pin_generator(const char *text, int state) {
    ...
    while (next) {
        ...
        if (!writer_match(pin->dir, match_writers))
            continue;
        if (!direction_match(pin->dir, match_direction))
            continue;
        if (match_type != HAL_TYPE_UNSPECIFIED && match_type != pin->type)
            continue;
        if (strncmp(text, name, len) == 0)
            return strdup(name);
    }
...
static char *loadusr_generator(const char *text, int state) {
    ...
    while (d && (ent = readdir(d))) {
        char *result;
        if (!startswith(ent->d_name, "hal"))
            continue;
        if (startswith(ent->d_name, "halcmd"))
            continue;
        if (strncmp(text, ent->d_name, len) != 0)
            continue;
...
static char *loadrt_generator(const char *text, int state) {
    ...
    while (d && (ent = readdir(d))) {
        char *result;
        if (!strstr(ent->d_name, MODULE_EXT))
            continue;
        if (startswith(ent->d_name, "rtapi."))
            continue;
        if (strncmp(text, ent->d_name, len) != 0)
            continue;
        result = strdup(ent->d_name);
