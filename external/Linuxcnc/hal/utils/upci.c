...
int upci_scan_bus(void) {
    ...
    while (1) {
        ...
        n = sscanf(lineptr, "%hx %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x", &busdevfunc, &vendordev, (unsigned *)&(dev->p.irq), &dev->p.base_addr[0], &dev->p.base_addr[1], &dev->p.base_addr[2], &dev->p.base_addr[3], &dev->p.base_addr[4], &dev->p.base_addr[5], &dev->p.rom_base_addr, &dev->p.size[0], &dev->p.size[1], &dev->p.size[2], &dev->p.size[3], &dev->p.size[4], &dev->p.size[5], &dev->p.rom_size);
        if (n != 17) {
        ...
        while (n >= 0) {
            ndev = devices[n];
            if ((dev->p.vendor_id == ndev->p.vendor_id) && (dev->p.device_id == ndev->p.device_id) && (dev->p.ss_vendor_id == ndev->p.ss_vendor_id) && (dev->p.ss_device_id == ndev->p.ss_device_id)) {
                /* found a match */
...
int upci_find_device(struct upci_dev_info *p) {
    ...
    while (n < num_devs) {
        dev = devices[n];
        if ((dev->p.vendor_id == p->vendor_id) && (dev->p.device_id == p->device_id) && (dev->p.ss_vendor_id == p->ss_vendor_id) && (dev->p.ss_device_id == p->ss_device_id) && (dev->p.instance == p->instance)) {
            /* found a match */
            return n;
        }
        n++;
...
int upci_open_region(int devnum, int region_num) {
    ...
    if (reg->size == 0) {
        errmsg(__func__, "region %d not found in device %d", region_num, devnum);
        return -1;
    }
    ...
    if (reg->type == UPCI_REG_IO) {
        ...
        reg->mapped_ptr = mmap(0, reg->size, PROT_READ | PROT_WRITE, MAP_SHARED, memfd, reg->base_addr);
        if (reg->mapped_ptr == MAP_FAILED) {
            reg->mapped_ptr = NULL;
            errmsg(__func__, "can't map /dev/mem for device %d, region %d: %s", devnum, region_num, strerror(errno));
            decr_mem_usage();
            return -1;
        }
    }
