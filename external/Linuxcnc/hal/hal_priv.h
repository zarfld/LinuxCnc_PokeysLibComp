...
#ifdef __cplusplus
template <class T> bool hal_shmchk(T *t) {
    char *c = (char *)t;
    return c > hal_shmem_base && c < hal_shmem_base + HAL_SIZE;
}

template <class T> int hal_shmoff(T *t) {
    return t ? (char *)t - hal_shmem_base : 0;
}

template <class T> T *hal_shmptr(int p) {
    return p ? (T *)(hal_shmem_base + p) : nullptr;
}

template <class T> class hal_shmfield {
  public:
    hal_shmfield() : off{} {
    }
...
template <class T> hal_shmfield<T> hal_make_shmfield(T *t) {
...
/* offset 0 is reserved for a null-ish pointer, so SHMCHK(hal_shmem_base) is
   false by design */
#define SHMCHK(ptr) (((char *)(ptr)) > (hal_shmem_base) && ((char *)(ptr)) < (hal_shmem_base + HAL_SIZE))
#endif
