...
int hal_init(const char *name) {
    ...
    if (halpr_find_comp_by_name(hal_name) != 0) {
        ...
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: duplicate component name '%s'\n", hal_name);
        rtapi_exit(comp_id);
        return -EINVAL;
    }
    ...
    if (comp == 0) {
        ...
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: insufficient memory for component '%s'\n", hal_name);
        rtapi_exit(comp_id);
        return -ENOMEM;
    }
    ...
    rtapi_mutex_give(&(hal_data->mutex));
    /* done */
    rtapi_print_msg(RTAPI_MSG_DBG, "HAL: component '%s' initialized, ID = %02d\n", hal_name, comp_id);
    ref_cnt++;
    return comp_id;
}
...
int hal_exit(int comp_id) {
    ...
    if (hal_data == 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: exit called before init\n");
        return -EINVAL;
    }
...
/*! \todo Another #if 0 */
    ...
    rtapi_exit(comp_id);
    /* done */
    rtapi_print_msg(RTAPI_MSG_DBG, "HAL: component %02d removed, name = '%s'\n", comp_id, name);
...
int hal_set_unready(int comp_id) {
    ...
    rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: hal_set_unready(): component %d not found\n", comp_id);
    return -EINVAL;
}
...
int hal_unready(int comp_id) {
    ...
    if (comp->ready < 1) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: Component '%s' already unready\n", comp->name);
        rtapi_mutex_give(&(hal_data->mutex));
        return -EINVAL;
    }
    comp->ready = 0;
...
int hal_pin_float_new(const char *name, hal_pin_dir_t dir, hal_float_t **data_ptr_addr, int comp_id) {
    return hal_pin_new(name, HAL_FLOAT, dir, (void **)data_ptr_addr, comp_id);
}
...
static int hal_pin_newfv(hal_type_t type, hal_pin_dir_t dir, void **data_ptr_addr, int comp_id, const char *fmt, va_list ap) {
    char name[HAL_NAME_LEN + 1];
    int sz;
...
int hal_pin_u64_newf(hal_pin_dir_t dir, hal_u64_t **data_ptr_addr, int comp_id, const char *fmt, ...) {
    va_list ap;
    int ret;
    va_start(ap, fmt);
...
int hal_pin_s64_newf(hal_pin_dir_t dir, hal_s64_t **data_ptr_addr, int comp_id, const char *fmt, ...) {
    va_list ap;
    int ret;
    va_start(ap, fmt);
...
int hal_pin_new(const char *name, hal_type_t type, hal_pin_dir_t dir, void **data_ptr_addr, int comp_id) {
    ...
    if (type != HAL_BIT && type != HAL_FLOAT && type != HAL_S32 && type != HAL_U32 && type != HAL_S64 && type != HAL_U64 && type != HAL_PORT) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: pin type not one of HAL_BIT, HAL_FLOAT, HAL_S32, HAL_U32, HAL_S64, HAL_U64 or HAL_PORT\n");
        return -EINVAL;
    }
    ...
    if (type == HAL_PORT && dir == HAL_IO) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: pin direction must be one of HAL_IN or HAL_OUT for hal port\n");
        return -EINVAL;
    }
    if (strlen(name) > HAL_NAME_LEN) {
...
int hal_link(const char *pin_name, const char *sig_name) {
    ...
    if ((pin->dir == HAL_IO) && (pin->type == HAL_PORT)) {
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: signal '%s' is a port and cannot have I/O pin(s)\n", sig_name);
        return -EINVAL;
    }
    ...
    if ((pin->type == HAL_PORT) && (pin->dir == HAL_IN) && (sig->readers > 0)) {
        ...
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: signal '%s' can only have one input pin\n", sig_name);
        return -EINVAL;
    }
    ...
    *data_ptr_addr = data_addr;
    ...
    bool drive_pin_default_value_onto_signal = (pin->type != HAL_PORT) && (pin->dir != HAL_IN || sig->readers == 0) && (sig->writers == 0) && (sig->bidirs == 0);
...
static int hal_param_newfv(hal_type_t type, hal_param_dir_t dir, void *data_addr, int comp_id, const char *fmt, va_list ap) {
    char name[HAL_NAME_LEN + 1];
    int sz;
    sz = rtapi_vsnprintf(name, sizeof(name), fmt, ap);
    if (sz == -1 || sz > HAL_NAME_LEN) {
...
int hal_param_u64_newf(hal_param_dir_t dir, hal_u64_t *data_addr, int comp_id, const char *fmt, ...) {
    va_list ap;
    int ret;
    va_start(ap, fmt);
...
int hal_param_s64_newf(hal_param_dir_t dir, hal_s64_t *data_addr, int comp_id, const char *fmt, ...) {
    va_list ap;
    int ret;
    va_start(ap, fmt);
...
int hal_param_new(const char *name, hal_type_t type, hal_param_dir_t dir, void *data_addr, int comp_id) {
    ...
    if (type != HAL_BIT && type != HAL_FLOAT && type != HAL_S32 && type != HAL_U32 && type != HAL_S64 && type != HAL_U64) {
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: pin type not one of HAL_BIT, HAL_FLOAT, HAL_S32, HAL_U32, Hal_S64 or HAL_U64\n");
        return -EINVAL;
    }
...
int hal_get_pin_value_by_name(const char *hal_name, hal_type_t *type, hal_data_u **data, bool *connected) {
    hal_pin_t *pin;
    hal_sig_t *sig;
...
int hal_get_signal_value_by_name(const char *hal_name, hal_type_t *type, hal_data_u **data, bool *has_writers) {
    hal_sig_t *sig;
...
int hal_get_param_value_by_name(const char *hal_name, hal_type_t *type, hal_data_u **data) {
    hal_param_t *param;
...
static int hal_export_functfv(void (*funct)(void *, long), void *arg, int uses_fp, int reentrant, int comp_id, const char *fmt, va_list ap) {
    char name[HAL_NAME_LEN + 1];
    int sz;
    sz = rtapi_vsnprintf(name, sizeof(name), fmt, ap);
    if (sz == -1 || sz > HAL_NAME_LEN) {
        rtapi_print_msg(RTAPI_MSG_ERR, "hal_export_functfv: length %d too long for name starting '%s'\n", sz, name);
        return -ENOMEM;
    }
    return hal_export_funct(name, funct, arg, uses_fp, reentrant, comp_id);
}

int hal_export_functf(void (*funct)(void *, long), void *arg, int uses_fp, int reentrant, int comp_id, const char *fmt, ...) {
    va_list ap;
    int ret;
    va_start(ap, fmt);
...
int hal_export_funct(const char *name, void (*funct)(void *, long), void *arg, int uses_fp, int reentrant, int comp_id) {
    ...
    if (comp->type == COMPONENT_TYPE_USER) {
        ...
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL: ERROR: component %d is not realtime\n", comp_id);
        return -EINVAL;
    }
    if (comp->ready) {
        rtapi_mutex_give(&(hal_data->mutex));
...
int hal_create_thread(const char *name, unsigned long period_nsec, int uses_fp) {
    ...
    if (new->period < prev_period) {
        rtapi_mutex_give(&(hal_data->mutex));
        rtapi_print_msg(RTAPI_MSG_ERR, "HAL_LIB: ERROR: new thread period %ld is less than existing thread period %ld\n", period_nsec, prev_period);
        return -EINVAL;
    }
    ...
    retval = rtapi_task_new(thread_task, new, new->priority, lib_module_id, HAL_STACKSIZE, uses_fp);
    if (retval < 0) {
        rtapi_mutex_give(&(hal_data->mutex));
...
static int proc_write_newinst(struct file *file, const char *buffer, unsigned long count, void *data) {
    if (hal_data->pending_constructor) {
        rtapi_print_msg(RTAPI_MSG_DBG, "HAL: running constructor for %s %s\n", hal_data->constructor_prefix, hal_data->constructor_arg);
        hal_data->pending_constructor(hal_data->constructor_prefix, hal_data->constructor_arg);
...
static bool hal_port_compute_copy(unsigned read, unsigned write, unsigned size, unsigned count, unsigned *end_bytes_to_read, unsigned *beg_bytes_to_read, unsigned *final_pos) {
    unsigned bytes_avail, end_bytes_avail;
...
bool hal_port_read(const hal_port_t *port, char *dest, unsigned count) {
    ...
    if (!port || !*port || !count) {
        ...
        if (hal_port_compute_copy(read, write, port_shm->size, count, &end_bytes_to_read, &beg_bytes_to_read, &final_pos)) {
...
bool hal_port_write(const hal_port_t *port, const char *src, unsigned count) {
    unsigned read, write, bytes_avail, end_bytes_avail, end_bytes_to_write, beg_bytes_to_write, final_pos;
...
int hal_stream_write(hal_stream_t *stream, union hal_stream_data *buf) {
    ...
    int in = hal_stream_atomic_load_in(stream), newin = hal_stream_advance(stream, in);
...
int hal_stream_read(hal_stream_t *stream, union hal_stream_data *buf, unsigned *this_sample) {
    ...
    int out = hal_stream_atomic_load_out(stream), newout = hal_stream_advance(stream, out);
    int num_pins = stream->fifo->num_pins;
    int stride = num_pins + 1;
    ...
    if (typestring) {
        ...
        for (i = 0; i < result; i++) {
            if (type[i] != fifo->type[i]) {
                ...
                rtapi_print_msg(RTAPI_MSG_ERR, "Type mismatch: types[%d] = %s vs %s\n", i, halpr_type_string(fifo->type[i], typename0, sizeof(typename0)), halpr_type_string(type[i], typename1, sizeof(typename1)));
                result = -EINVAL;
                goto fail;
            }
        }
