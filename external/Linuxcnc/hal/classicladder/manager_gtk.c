...
void ManagerDisplaySections(char ForgetSectionSelected) {
    ...
    for (NumSec = 0; NumSec < NBR_SECTIONS; NumSec++) {
        ...
        if (pSection->Used) {
            ...
            // fill the element
            gtk_list_store_set(ListStore, &iter, SECTION_NUMBER, NumSec, SECTION_NAME, pSection->Name, SECTION_LANGUAGE, (pSection->Language == SECTION_IN_SEQUENTIAL) ? (_("Sequential")) : (_("Ladder")), SECTION_TYPE, BufferForSRx, SECTION_DEBUG, buffer_debug, -1);
...
void AddSectionWindowInit() {
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonOk), "clicked", GTK_SIGNAL_FUNC(ButtonAddSectionDoneClickSignal), 0);
    ...
    gtk_signal_connect(GTK_OBJECT(AddSectionWindow), "delete_event", GTK_SIGNAL_FUNC(AddSectionWindowDeleteEvent), 0);
}

GtkUIManager *ManageruiManager;

static GtkActionEntry ManagerEntriesArray[] = { { "AddSection", GTK_STOCK_ADD, N_("Add section"), NULL, N_("Add New Section"), G_CALLBACK(ButtonAddClickSignal) },
...
static const gchar *Manager_ui_strings = "<ui>"
...
void ManagerInitGtk() {
    ...
    gtk_signal_connect(GTK_OBJECT(ListViewSections), "cursor-changed", GTK_SIGNAL_FUNC(TreeViewCursorChangedSignal), 0);

    // v0.9.20	ManagerDisplaySections( );
    gtk_signal_connect(GTK_OBJECT(ManagerWindow), "delete_event", GTK_SIGNAL_FUNC(ManagerWindowDeleteEvent), 0);
