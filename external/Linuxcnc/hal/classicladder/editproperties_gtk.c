...
void PropertiesInitGtk() {
    ...
    for (NumParam = 0; NumParam < NBR_PARAMS_PER_OBJ; NumParam++) {
        ...
        gtk_widget_show(PropEntryParam[NumParam]);
        gtk_signal_connect(GTK_OBJECT(PropEntryParam[NumParam]), "activate", GTK_SIGNAL_FUNC(SaveElementProperties), NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonApplyProperties), "clicked", GTK_SIGNAL_FUNC(SaveElementProperties), 0);
    gtk_widget_set_sensitive(ButtonApplyProperties, FALSE);
    gtk_widget_show(ButtonApplyProperties);

    //    gtk_widget_show (PropertiesWindow);

    gtk_signal_connect(GTK_OBJECT(PropertiesWindow), "delete_event", GTK_SIGNAL_FUNC(PropertiesWindowDeleteEvent), 0);
}
