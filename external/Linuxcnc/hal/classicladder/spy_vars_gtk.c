...
void UpdateAllLabelsBoolsVars() {
    ...
    for (ColumnVar = 0; ColumnVar < NBR_TYPE_BOOLS_SPY; ColumnVar++) {
        for (OffVar = 0; OffVar < NBR_BOOLS_VAR_SPY; OffVar++) {
            ...
            switch (ColumnVar) {
                case 0:
                    snprintf(BufNumVar, 256, "<span foreground=\"black\" weight=\"bold\">%cB%d</span>", '%', OffVar + ValOffsetBoolVar[ColumnVar]);
                    break;
                case 1:
                    snprintf(BufNumVar, 256, "<span foreground=\"red\" weight=\"bold\">%cI%d</span>", '%', OffVar + ValOffsetBoolVar[ColumnVar]);
                    break;
                case 2:
                    snprintf(BufNumVar, 256, "<span foreground=\"blue\" weight=\"bold\">%cQ%d</span>", '%', OffVar + ValOffsetBoolVar[ColumnVar]);
                    break;
            }
...
void BoolVarsWindowInitGtk() {
    ...
    for (ColumnVar = 0; ColumnVar < NBR_TYPE_BOOLS_SPY; ColumnVar++) {
        ...
        gtk_signal_connect(GTK_OBJECT(offsetboolvar[ColumnVar]), "activate", GTK_SIGNAL_FUNC(OffsetBoolVar_activate_event), (void *)ColumnVar);
        ...
        for (OffVar = 0; OffVar < NBR_BOOLS_VAR_SPY; OffVar++) {
            ...
            gtk_signal_connect(GTK_OBJECT(chkvar[ColumnVar][OffVar]), "toggled", GTK_SIGNAL_FUNC(chkvar_press_event), (void *)NumCheckWidget);
            NumCheckWidget++;
        }
    }
    UpdateAllLabelsBoolsVars();

    gtk_signal_connect(GTK_OBJECT(SpyBoolVarsWindow), "delete_event", GTK_SIGNAL_FUNC(BoolVarsWindowDeleteEvent), 0);
...
void ModifyVarWindowInitGtk() {
    ...
    gtk_signal_connect(GTK_OBJECT(ModifyVariableValueEdit), "activate", GTK_SIGNAL_FUNC(ApplyModifiedVar), (void *)NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonOk), "clicked", GTK_SIGNAL_FUNC(ApplyModifiedVar), (void *)NULL);
    ButtonCancel = gtk_button_new_from_stock(GTK_STOCK_CANCEL);
    gtk_box_pack_start(GTK_BOX(hboxOkCancel), ButtonCancel, FALSE, FALSE, 0);
    gtk_widget_show(ButtonCancel);
    gtk_signal_connect(GTK_OBJECT(ButtonCancel), "clicked", GTK_SIGNAL_FUNC(ModifyVarWindowDeleteEvent), NULL);

    gtk_signal_connect(GTK_OBJECT(ModifyVarValueWindow), "delete_event", GTK_SIGNAL_FUNC(ModifyVarWindowDeleteEvent), 0);
}
    ...
    for (NumVarSpy = 0; NumVarSpy < i; NumVarSpy++) {
        ...
        for (ColumnVar = 0; ColumnVar < 3; ColumnVar++) {
            ...
            if (ColumnVar == 1) {
                ...
                gtk_signal_connect(GTK_OBJECT(EntryVarSpy[NumEntry]), "activate", GTK_SIGNAL_FUNC(EntryVarSpy_activate_event), (void *)(intptr_t)NumVarSpy);
            }
            if (ColumnVar == 2) {
                EntryVarSpy[NumEntry] = gtk_entry_new();
                gtk_widget_show(EntryVarSpy[NumEntry]);
        ...
        gtk_button_set_image(GTK_BUTTON(ModifyVarSpy[NumVarSpy]), gtk_image_new_from_stock(GTK_STOCK_EDIT, GTK_ICON_SIZE_SMALL_TOOLBAR));
        ...
        gtk_signal_connect(GTK_OBJECT(ModifyVarSpy[NumVarSpy]), "clicked", GTK_SIGNAL_FUNC(OpenModifyVarWindow_clicked_event), (void *)(intptr_t)NumVarSpy);
    }
    gtk_signal_connect(GTK_OBJECT(SpyFreeVarsWindow), "delete_event", GTK_SIGNAL_FUNC(FreeVarsWindowDeleteEvent), 0);
}
