...
void DrawSeqStep(cairo_t *cr, int x, int y, int Size, StrStep *pStep, char DrawingOption) {
    ...
    if (pStep->InitStep) {
        cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
        cairo_rectangle(cr, x + PxOffsetActiveStep, y + PxOffsetActiveStep, Size - 2 * PxOffsetActiveStep, Size - 2 * PxOffsetActiveStep);
        cairo_stroke(cr);
    }
    // step number
...
void DrawSeqTransition(cairo_t *cr, int x, int y, int Size, StrTransition *pTransi, char DrawingOption) {
    ...
    // |
    my_cairo_draw_color_line(cr, 0, x + SizeDiv2, (cNoDirectStepOnTop ? y + SizeDiv3 : y) - 1, x + SizeDiv2, cNoDirectStepOnBottom ? y + Size - SizeDiv3 : y + Size + 1);

    // -
    my_cairo_draw_line(cr, x + SizeDiv3, y + SizeDiv2, x + Size - SizeDiv3, y + SizeDiv2);
    ...
    for (ScanSwitch = 1; ScanSwitch < NBR_SWITCHS_MAX; ScanSwitch++) {
        ...
        if (NumStep != -1) {
            ...
            // ===
            my_cairo_draw_color_line(cr, 0, x + SizeDiv3, y + Size - SizeDiv4 + SizeLittle, StepX + Size - SizeDiv3, y + Size - SizeDiv4 + SizeLittle);
            my_cairo_draw_color_line(cr, 0, x + SizeDiv3, y + Size - SizeDiv4 + 2 * SizeLittle + 1, StepX + Size - SizeDiv3, y + Size - SizeDiv4 + 2 * SizeLittle + 1);
            //			my_cairo_draw_color_line(cr, 1,
            //				x+SizeDiv3, y+Size-SizeDiv4+3, StepX+Size-SizeDiv3, y+Size-SizeDiv4+3);
            // | with the step
            my_cairo_draw_color_line(cr, 0, StepX + SizeDiv2, y + Size - SizeDiv4 + 2 * SizeLittle, StepX + SizeDiv2, y + Size + 1);
        }
    }
    ...
    for (ScanSwitch = 1; ScanSwitch < NBR_SWITCHS_MAX; ScanSwitch++) {
        ...
        if (NumStep != -1) {
            StepX = Seq->Step[NumStep].PosiX * Size - ShiftX;
            // ===
            my_cairo_draw_color_line(cr, 0, x + SizeDiv3, y + SizeDiv4 - SizeLittle, StepX + Size - SizeDiv3, y + SizeDiv4 - SizeLittle);
            my_cairo_draw_color_line(cr, 0, x + SizeDiv3, y + SizeDiv4 - 2 * SizeLittle - 1, StepX + Size - SizeDiv3, y + SizeDiv4 - 2 * SizeLittle - 1);
            //			my_cairo_draw_color_line(cr, 1,
            //				x+SizeDiv3, y+SizeDiv4-3, StepX+Size-SizeDiv3, y+SizeDiv4-3);
            // | with the step
            my_cairo_draw_color_line(cr, 0, StepX + SizeDiv2, y - 1, StepX + SizeDiv2, y + SizeDiv4 - 2 * SizeLittle);
        }
    }
    ...
    if (pTransi->NumTransLinkedForStart[0] != -1) {
        for (ScanSwitch = 0; ScanSwitch < NBR_SWITCHS_MAX; ScanSwitch++) {
            ...
            if (NumTransi != -1) {
                ...
                my_cairo_draw_color_line(cr, 0, x + SizeDiv2, y + SizeDiv3, TransiX + SizeDiv2, y + SizeDiv3);
            }
        }
    }
    ...
    if (pTransi->NumTransLinkedForEnd[0] != -1) {
        for (ScanSwitch = 0; ScanSwitch < NBR_SWITCHS_MAX; ScanSwitch++) {
            ...
            if (NumTransi != -1) {
                TransiX = Seq->Transition[NumTransi].PosiX * Size - ShiftX;
                my_cairo_draw_color_line(cr, 0, x + SizeDiv2, y + Size - SizeDiv3, TransiX + SizeDiv2, y + Size - SizeDiv3);
            }
        }
    }
    ...
    if (pTransi->NumStepToActiv[0] != -1 && pTransi->NumStepToDesactiv[0] != -1) {
        ...
        if (pTransi->PosiX != StepX || pTransi->PosiY + 1 != StepY) {
            if (pTransi->NumTransLinkedForEnd[0] == -1) {
                ...
                my_cairo_draw_color_line(cr, 0, x + SizeDiv3, y + Size - SizeDiv3, x + SizeDiv2, y + Size - SizeDiv3 + SizeDiv4);
                my_cairo_draw_color_line(cr, 0, x + Size - SizeDiv3, y + Size - SizeDiv3, x + SizeDiv2, y + Size - SizeDiv3 + SizeDiv4);
                my_cairo_draw_color_line(cr, 0, x + SizeDiv2, y + Size - SizeDiv3, x + SizeDiv2, y + Size - SizeDiv3 + SizeDiv4);
                snprintf(BufTxt, sizeof(BufTxt), "%d", pStep->StepNumber);
                cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
                ...
                my_cairo_draw_color_line(cr, 0, StepX + SizeDiv3, StepY - SizeDiv2, StepX + SizeDiv2, StepY - SizeDiv4);
                my_cairo_draw_color_line(cr, 0, StepX + Size - SizeDiv3, StepY - SizeDiv2, StepX + SizeDiv2, StepY - SizeDiv4);
                my_cairo_draw_color_line(cr, 0, StepX + SizeDiv2, StepY - SizeDiv4, StepX + SizeDiv2, StepY + 1);
...
void DrawSeqComment(cairo_t *cr, int x, int y, int Size, StrSeqComment *pSeqComment) {
    ...
    cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
    cairo_rectangle(cr, x + Margin, y + Margin, 4 * Size - Margin, Size - Margin);
    cairo_stroke(cr);
...
void DrawSequentialPage(cairo_t *cr, int PageNbr, int SeqPxSize, char DrawingOption) {
    ...
    for (ScanSeqComment = 0; ScanSeqComment < NBR_SEQ_COMMENTS; ScanSeqComment++) {
        ...
        if (pSeqComment->NumPage == PageNbr)
            DrawSeqComment(cr, pSeqComment->PosiX * SeqPxSize - ShiftX, pSeqComment->PosiY * SeqPxSize - ShiftY, SeqPxSize, pSeqComment);
    }
    if (EditDatas.ModeEdit)
    ...
    switch (NumElement) {
        ...
        case EDIT_SEQ_INIT_STEP:
            ...
            if (NumElement == EDIT_SEQ_INIT_STEP)
                cairo_rectangle(cr, x + SizeDiv4 + 2, y + SizeDiv4 + 2, Size - 2 * SizeDiv4 - 4, Size - 2 * SizeDiv4 - 4);
            cairo_stroke(cr);
            break;
        case ELE_SEQ_TRANSITION:
            my_cairo_draw_line(cr, x + SizeDiv2, y + SizeDiv4, x + SizeDiv2, y + Size - SizeDiv4);
            my_cairo_draw_line(cr, x + SizeDiv3, y + SizeDiv2, x + Size - SizeDiv3, y + SizeDiv2);
            break;
        case EDIT_SEQ_STEP_AND_TRANS:
            cairo_rectangle(cr, x + SizeDiv4, y + 2, Size - 2 * SizeDiv4, SizeDiv2 - 2);
            cairo_stroke(cr);
            my_cairo_draw_line(cr, x + SizeDiv2, y + SizeDiv2, x + SizeDiv2, y + Size - 2);
            my_cairo_draw_line(cr, x + SizeDiv3, y + Size - SizeDiv4, x + Size - SizeDiv3, y + Size - SizeDiv4);
            break;
        case EDIT_SEQ_START_MANY_TRANS:
            my_cairo_draw_line(cr, x + 2, y + SizeDiv4, x + Size - 2, y + SizeDiv4);
            my_cairo_draw_line(cr, x + SizeDiv4, y + SizeDiv4 - 4, x + SizeDiv4, y + SizeDiv4 + 4);
            my_cairo_draw_line(cr, x + Size - SizeDiv4, y + SizeDiv4, x + Size - SizeDiv4, y + SizeDiv4 + 4);
            break;
        case EDIT_SEQ_END_MANY_TRANS:
            my_cairo_draw_line(cr, x + 2, y + Size - SizeDiv4, x + Size - 2, y + Size - SizeDiv4);
            my_cairo_draw_line(cr, x + SizeDiv4, y + Size - SizeDiv4 - 4, x + SizeDiv4, y + Size - SizeDiv4 + 4);
            my_cairo_draw_line(cr, x + Size - SizeDiv4, y + Size - SizeDiv4, x + Size - SizeDiv4, y + Size - SizeDiv4 - 4);
            break;
        case EDIT_SEQ_START_MANY_STEPS:
            my_cairo_draw_line(cr, x + 2, y + SizeDiv4, x + Size - 2, y + SizeDiv4);
            my_cairo_draw_line(cr, x + 2, y + SizeDiv4 + 2, x + Size - 2, y + SizeDiv4 + 2);
            my_cairo_draw_line(cr, x + SizeDiv4, y + SizeDiv4 - 4, x + SizeDiv4, y + SizeDiv4 + 4);
            my_cairo_draw_line(cr, x + Size - SizeDiv4, y + SizeDiv4 + 2, x + Size - SizeDiv4, y + SizeDiv4 + 4);
            break;
        case EDIT_SEQ_END_MANY_STEPS:
            my_cairo_draw_line(cr, x + 2, y + Size - SizeDiv4, x + Size - 2, y + Size - SizeDiv4);
            my_cairo_draw_line(cr, x + 2, y + Size - SizeDiv4 - 2, x + Size - 2, y + Size - SizeDiv4 - 2);
            my_cairo_draw_line(cr, x + SizeDiv4, y + Size - SizeDiv4 - 4, x + SizeDiv4, y + Size - SizeDiv4 + 4);
            my_cairo_draw_line(cr, x + Size - SizeDiv4, y + Size - SizeDiv4 - 2, x + Size - SizeDiv4, y + Size - SizeDiv4 - 4);
            break;
        case EDIT_SEQ_LINK:
            my_cairo_draw_line(cr, x + SizeDiv3, y + SizeDiv4, x + Size - SizeDiv3, y + Size - SizeDiv4);
