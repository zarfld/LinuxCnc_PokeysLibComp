...
void InitSocketServer(int UseUdpMode, int PortNbr) {
    ...
    if (ModbusConfig.ModbusDebugLevel >= 2) {
        printf(_("INFO CLASSICLADDER--- INIT SOCKET!!!\n"));
    }
    // Create a socket
...
#ifdef __WIN32__
                ThreadHandle = CreateThread(NULL /*no security attributes*/, 16 * 1024L /*default stack size*/, (LPTHREAD_START_ROUTINE)SocketServerTcpMainLoop /* thread function*/, NULL /*argument to thread function*/, THREAD_QUERY_INFORMATION /*use default creation flags*/, &ThreadId /* returns the thread identifier*/);
