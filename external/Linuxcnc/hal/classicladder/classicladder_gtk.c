...
void CreateFileSelection(char *Prompt, int CreateFileSelectionType) {
    ...
    if (CreateFileSelectionType == CREATE_FILE_SELECTION_TO_SAVE_PROJECT) {
        FileSelector = gtk_file_chooser_dialog_new(Prompt, NULL, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_SAVE, GTK_RESPONSE_ACCEPT, NULL);
        gtk_file_chooser_set_do_overwrite_confirmation(GTK_FILE_CHOOSER(FileSelector), TRUE);
    } else {
        FileSelector = gtk_file_chooser_dialog_new(Prompt, NULL, GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
    }
    ...
    if (CreateFileSelectionType == CREATE_FILE_SELECTION_TO_SAVE_PROJECT)
        g_signal_connect((gpointer)FileSelector, "response", G_CALLBACK(on_filechooserdialog_save_response), NULL);
    else
        g_signal_connect((gpointer)FileSelector, "response", G_CALLBACK(on_filechooserdialog_load_response), NULL);

    g_signal_connect_swapped((gpointer)FileSelector, "close", G_CALLBACK(gtk_widget_destroy), GTK_OBJECT(FileSelector));
...
*/
    ...
    if (comments) {
        ...
        gtk_show_about_dialog(GTK_WINDOW(MainSectionWindow), "program-name", CL_PRODUCT_NAME, "version", CL_RELEASE_VER_STRING, "copyright", "Copyright (C) " CL_RELEASE_COPYRIGHT_YEARS " Marc Le Douarain\nmarc . le - douarain /At\\ laposte \\DoT/ net",
                              //							"logo", example_logo,
                              "title", _("About ClassicLadder"), "website", "http://sites.google.com/site/classicladder", "comments", comments, NULL);
        free(comments);
    }
}
...
void FileRequestToExportSvgOrPng(char GoForSvg) {
    GtkWidget *dialog;
    dialog = gtk_file_chooser_dialog_new(GoForSvg ? _("Save SVG File") : _("Save PNG File"), GTK_WINDOW(MainSectionWindow), GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_SAVE, GTK_RESPONSE_ACCEPT, NULL);
...
void ShowMessageBox(const char *title, const char *text, const char *button) {
    ...
    g_signal_connect_swapped(GTK_OBJECT(okay_button), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(dialog));
...
void ShowConfirmationBoxWithChoiceOrNot(const char *title, const char *text, void *function_if_yes, char HaveTheChoice) {
    ...
    if (HaveTheChoice) {
        ...
        g_signal_connect_swapped(GTK_OBJECT(no_button), "clicked", GTK_SIGNAL_FUNC(gtk_widget_destroy), GTK_OBJECT(ConfirmDialog));
    ...
    gtk_signal_connect(GTK_OBJECT(yes_button), "clicked", GTK_SIGNAL_FUNC(DoFunctionOfConfirmationBox), function_if_yes);
...
void MainSectionWindowInitGtk() {
    ...
    gtk_signal_connect(GTK_OBJECT(MainSectionWindow), "destroy", GTK_SIGNAL_FUNC(QuitAppliGtk), NULL);

    PtrUIManager = InitMenusAndToolBar(vbox);
    gtk_window_add_accel_group(GTK_WINDOW(MainSectionWindow), gtk_ui_manager_get_accel_group(PtrUIManager));
    ...
    gtk_signal_connect(GTK_OBJECT(CheckDispSymbols), "toggled", GTK_SIGNAL_FUNC(CheckDispSymbols_toggled), NULL);
    ...
    gtk_widget_set_size_request(drawing_area, BLOCK_WIDTH_DEF * RUNG_WIDTH + 20, BLOCK_HEIGHT_DEF * RUNG_HEIGHT + 45);
    ...
    gtk_signal_connect(GTK_OBJECT(AdjustVScrollBar), "value-changed", GTK_SIGNAL_FUNC(VScrollBar_value_changed_event), 0);
    gtk_signal_connect(GTK_OBJECT(AdjustHScrollBar), "value-changed", GTK_SIGNAL_FUNC(HScrollBar_value_changed_event), 0);
...
// no more used since menu/toolbar added...
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonNew), "clicked", (GtkSignalFunc)ButtonNew_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonLoad), "clicked", (GtkSignalFunc)ButtonLoad_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonSave), "clicked", (GtkSignalFunc)ButtonSave_click, 0);
    gtk_widget_show(ButtonSave);
    ButtonSaveAs = gtk_button_new_with_label(_("Save As"));
    gtk_box_pack_start(GTK_BOX(hboxbottom), ButtonSaveAs, TRUE, TRUE, 0);
    gtk_signal_connect(GTK_OBJECT(ButtonSaveAs), "clicked", (GtkSignalFunc)ButtonSaveAs_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonReset), "clicked", (GtkSignalFunc)ButtonReset_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonRunStop), "clicked", (GtkSignalFunc)ButtonRunStop_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonSpyVars), "clicked", (GtkSignalFunc)OpenSpyVarsWindow, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonAbout), "clicked", (GtkSignalFunc)ButtonAbout_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonEdit), "clicked", (GtkSignalFunc)OpenEditWindow, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonSymbols), "clicked", (GtkSignalFunc)OpenSymbolsWindow, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonConfig), "clicked", (GtkSignalFunc)ButtonConfig_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonExportSVG), "clicked", (GtkSignalFunc)ButtonExportSvgOrPng_click, (void *)1);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonExportPNG), "clicked", (GtkSignalFunc)ButtonExportSvgOrPng_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonCopyToClipboard), "clicked", (GtkSignalFunc)ButtonCopyToClipboard_click, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonPrintPreview), "clicked", (GtkSignalFunc)PrintPreviewGnome, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(ButtonPrint), "clicked", (GtkSignalFunc)PrintGnome, 0);
    ...
    gtk_signal_connect_object(GTK_OBJECT(ButtonQuit), "clicked", ConfirmQuit, NULL);
    gtk_widget_show(ButtonQuit);
#endif

    /* Signal used to redraw */
#if GTK_MAJOR_VERSION >= 3
    gtk_signal_connect(GTK_OBJECT(drawing_area), "draw", GTK_SIGNAL_FUNC(draw_callback), NULL);
#else
    ...
    gtk_signal_connect(GTK_OBJECT(drawing_area), "scroll_event", GTK_SIGNAL_FUNC(mouse_scroll_event), NULL);
    gtk_widget_set_events(drawing_area, GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_SCROLL_MASK // mouse scroll
                                            | GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK);

    gtk_signal_connect(GTK_OBJECT(MainSectionWindow), "delete_event", GTK_SIGNAL_FUNC(MainSectionWindowDeleteEvent), 0);
...
void ShowErrorMessage(const char *title, const char *text, const char *button) {
    ...
    okay_button = gtk_button_new_with_label(button);
    /* Ensure that the dialog box is destroyed when the user clicks ok. */
    g_signal_connect_swapped(okay_button, "clicked", G_CALLBACK(ErrorMessageDeleteEvent), dialog);
    g_signal_connect(dialog, "delete_event", G_CALLBACK(ErrorMessageDeleteEvent), 0);
    gtk_container_add(GTK_CONTAINER(gtk_dialog_get_action_area(GTK_DIALOG(dialog))), okay_button);
    gtk_widget_grab_focus(okay_button);
    /* Add the label, and show everything we've added to the dialog. */
    gtk_container_add(GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), label);
...
void CheckForErrors(void) {
    ...
    if (ReadVar(VAR_ERROR_BIT, 0) == FALSE) {
        temp = 0;
    }
}
