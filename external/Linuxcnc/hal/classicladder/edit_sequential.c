    ...
    if (TypeSeqEleEdited != -1) {
        switch (TypeSeqEleEdited) {
            ...
            case ELE_SEQ_TRANSITION:
                rtapi_strxcpy(TextToWrite, CreateVarName(EditSeqDatas.Transition[OffsetSeqEleEdited].VarTypeCondi, EditSeqDatas.Transition[OffsetSeqEleEdited].VarNumCondi, InfosGene->DisplaySymbols));
    ...
    if (TypeSeqEleEdited != -1) {
        switch (TypeSeqEleEdited) {
            case ELE_SEQ_STEP:
                ...
                break;
            case ELE_SEQ_TRANSITION:
                if (TextParserForAVar(GetProperty(0), &EditSeqDatas.Transition[OffsetSeqEleEdited].VarTypeCondi, &EditSeqDatas.Transition[OffsetSeqEleEdited].VarNumCondi, NULL, FALSE /*PartialNames*/) == FALSE) {
...
/* return TRUE is okay */
char CommonSearchForManyStepsOrTransi(char ForManySteps, int TypeEle1, int OffEle1, int TypeEle2, int OffEle2, int *pOffsetTransiFound, int *pStepsBaseY, int *pTransitionsBaseY, int *pLeftX, int *pRightX) {
...
void DoManyStepsActOrDesact(int ForPage, int ToolBarFlagStart, int TypeEle1, int OffEle1, int TypeEle2, int OffEle2) {
    ...
    if (!CommonSearchForManyStepsOrTransi(TRUE, TypeEle1, OffEle1, TypeEle2, OffEle2, &OffsetTransiFound, &StepsBaseY, NULL, &LeftX, &RightX))
...
void DoManyTransitionsLinked(int ForPage, int ToolBarFlagStart, int TypeEle1, int OffEle1, int TypeEle2, int OffEle2) {
    ...
    if (!CommonSearchForManyStepsOrTransi(FALSE /*TRUE*/, TypeEle1, OffEle1, TypeEle2, OffEle2, NULL, NULL, &TransisBaseY, &LeftX, &RightX))
        return; /* search failed ! */
...
void DestroyIt(int Type, int Offset) {
    switch (Type) {
        case ELE_SEQ_STEP:
            ...
            break;
        case ELE_SEQ_TRANSITION:
            DestroyTransi(Offset);
            break;
        case ELE_SEQ_COMMENT:
            DestroyComment(Offset);
            break;
    }
}
...
void EditElementInSeqPage(double x, double y) {
    ...
    PosX = x / SEQ_SIZE_DEF;
    PosY = y / SEQ_SIZE_DEF;
    if ((PosX < SEQ_PAGE_WIDTH) && (PosY < SEQ_PAGE_HEIGHT) && (EditDatas.NumElementSelectedInToolBar != -1)) {
        int TypeFound = -1;
        int OffsetFound = -1;
        ...
        switch (EditDatas.NumElementSelectedInToolBar) {
            ...
            case EDIT_SEQ_STEP_AND_TRANS:
                if (EditDatas.NumElementSelectedInToolBar == ELE_SEQ_STEP || EditDatas.NumElementSelectedInToolBar == EDIT_SEQ_INIT_STEP || EditDatas.NumElementSelectedInToolBar == EDIT_SEQ_STEP_AND_TRANS) {
                    if (PosY & 1) {
                        if (PosY < SEQ_PAGE_HEIGHT - 1) {
                            if (TypeFound == ELE_SEQ_STEP) {
                                ...
                                if (TypeFound == -1) {
                                    OffsetFound = CreateStep(CurrentSeqPage, PosX, PosY, (EditDatas.NumElementSelectedInToolBar == EDIT_SEQ_INIT_STEP) ? TRUE : FALSE);
                                    if (OffsetFound != -1) {
                ...
                if (EditDatas.NumElementSelectedInToolBar == ELE_SEQ_TRANSITION || EditDatas.NumElementSelectedInToolBar == EDIT_SEQ_STEP_AND_TRANS) {
            ...
            case EDIT_SEQ_END_MANY_TRANS:
                ...
                if (CptNbrClicksDone == 2) {
                    if (EditDatas.NumElementSelectedInToolBar == EDIT_SEQ_START_MANY_STEPS || EditDatas.NumElementSelectedInToolBar == EDIT_SEQ_END_MANY_STEPS) {
                        printf(_("DO_MANY_STEPS:nbr clicks=2!!!, TypeBak=%d, TypeNow=%d\n"), TypeSeqEleEditedBak, TypeFound);
                        DoManyStepsActOrDesact(CurrentSeqPage, EditDatas.NumElementSelectedInToolBar == EDIT_SEQ_START_MANY_STEPS, TypeSeqEleEditedBak, OffsetSeqEleEditedBak, TypeFound, OffsetFound);
                    } else {
                        printf(_("DO_MANY_TRANSITIONS:nbr clicks=2!!!, TypeBak=%d, TypeNow=%d\n"), TypeSeqEleEditedBak, TypeFound);
                        DoManyTransitionsLinked(CurrentSeqPage, EditDatas.NumElementSelectedInToolBar == EDIT_SEQ_START_MANY_TRANS, TypeSeqEleEditedBak, OffsetSeqEleEditedBak, TypeFound, OffsetFound);
                    }
                    CptNbrClicksDone = 0;
                }
                break;
