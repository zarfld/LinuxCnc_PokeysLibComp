    ...
    do {
        ...
        do {
            ...
            if (pConv->StringBaseVarName[iScruteCarac] == '%') {
                ...
                do {
                    iScruteCarac++;
                } while ((pConv->StringBaseVarName[iScruteCarac] != 'd') && (pConv->StringBaseVarName[iScruteCarac] != '\0'));
                iScruteCarac++;
...
char *CreateVarName(int TypeVarSearched, int OffsetVarSearched, char SymbolNameIfAvail) {
    ...
    if (pConv) {
        ...
        if (pConv->iSize2 <= 0 && pConv->iSize3 <= 0) {
            ...
            if (pConv->iSize3 <= 0)
                snprintf(tcBuffer2, sizeof(tcBuffer2), pConv->StringBaseVarName, (OffsetVarSearched - iIdFound) / pConv->iSize2 + pConv->iFirstVal1, (OffsetVarSearched - iIdFound) % pConv->iSize2 + pConv->iFirstVal2);
            else
                snprintf(tcBuffer2, sizeof(tcBuffer2), pConv->StringBaseVarName, (OffsetVarSearched - iIdFound) / (pConv->iSize2 * pConv->iSize3) + pConv->iFirstVal1, ((OffsetVarSearched - iIdFound) / pConv->iSize3) % pConv->iSize2 + pConv->iFirstVal2, (OffsetVarSearched - iIdFound) % pConv->iSize3 + pConv->iFirstVal3);
        }
