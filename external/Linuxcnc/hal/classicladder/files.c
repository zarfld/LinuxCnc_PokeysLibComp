...
#else
static inline int dbg_printf(char *f, ...) {
    (void)f;
    return 0;
}
#endif

StrDatasForBase CorresDatasForBase[3] = { { BASE_MINS, TIME_BASE_MINS, "%.1fmn", "Mins" }, { BASE_SECS, TIME_BASE_SECS, "%.1fs", "Secs" }, { BASE_100MS, TIME_BASE_100MS, "%.0f00ms", "100msecs" } };
    ...
    if (File) {
        ...
        for (y = 0; y < RUNG_HEIGHT; y++) {
            ...
            for (x = 0; x < RUNG_WIDTH; x++) {
                fprintf(File, "%d-%d-%d/%d", BufRung->Element[x][y].Type, BufRung->Element[x][y].ConnectedWithTop, BufRung->Element[x][y].VarType, BufRung->Element[x][y].VarNum);
            ...
            if (LineOk) {
                switch (Line[0]) {
                    ...
                    default:
                        ...
                        dbg_printf(_("Section %d => Name=%s, Language=%d, SRnbr=%d, FirstRung=%d, LastRung=%d, SequentialPage=%d\n"), NumSection, pSection->Name, pSection->Language, pSection->SubRoutineNumber, pSection->FirstRung, pSection->LastRung, pSection->SequentialPage);
                        break;
                }
            }
        } while (LineOk);
        ...
        for (Pass = 0; Pass < 2; Pass++) {
            ...
            for (NumConf = 0; NumConf < NbrConf; NumConf++) {
                ...
                if (pConf->FirstClassicLadderIO != -1) {
                    fprintf(File, S_LINE "%d,%d,%d,%d,%d,%d,%d" E_LINE "\n", Pass, pConf->FirstClassicLadderIO, pConf->DeviceType, pConf->SubDevOrAdr, pConf->FirstChannel, pConf->NbrConsecutivesChannels, pConf->FlagInverted);
                }
            }
        }
        fclose(File);
    ...
    if (File) {
        ...
        for (NumLine = 0; NumLine < NBR_MODBUS_MASTER_REQ; NumLine++) {
            ...
            if (pConf->SlaveAdr[0] != '\0') {
                fprintf(File, S_LINE "%s,%d,%d,%d,%d,%d" E_LINE "\n", pConf->SlaveAdr, pConf->TypeReq, pConf->FirstModbusElement, pConf->NbrModbusElements, pConf->LogicInverted, pConf->OffsetVarMapped);
            }
        }
        fclose(File);
        Okay = TRUE;
            ...
            if (LineOk) {
                switch (Line[0]) {
                    ...
                    default:
                        ...
                        PtrStrings[0] = pSymbol->VarName;
                        LgtMaxStrings[0] = LGT_VAR_NAME;
                        PtrStrings[1] = pSymbol->Symbol;
                        LgtMaxStrings[1] = LGT_SYMBOL_STRING;
                        PtrStrings[2] = pSymbol->Comment;
                        LgtMaxStrings[2] = LGT_SYMBOL_COMMENT;
                        PtrStrings[3] = NULL;
                        LgtMaxStrings[3] = 0;
