...
static short int RulesForSpecialElements[][3] = { { ELE_TIMER, 2, 2 }, { ELE_MONOSTABLE, 2, 2 }, { ELE_COUNTER, 2, 4 }, { ELE_TIMER_IEC, 2, 2 }, { ELE_COMPAR, 3, 1 }, { ELE_OUTPUT_OPERATE, 3, 1 }, { -1, -1, -1 } /*end*/ };
        ...
        switch (EditDatas.ElementUnderEdit->Type) {
            ...
            case ELE_OUTPUT_RESET:
                if (TextParserForAVar(GetProperty(0), &VarTypeEntered, &VarNumEntered, NULL, FALSE /*PartialNames*/)) {
                    ...
                    if ((EditDatas.ElementUnderEdit->Type == ELE_OUTPUT || EditDatas.ElementUnderEdit->Type == ELE_OUTPUT_NOT || EditDatas.ElementUnderEdit->Type == ELE_OUTPUT_SET || EditDatas.ElementUnderEdit->Type == ELE_OUTPUT_RESET) && !TestVarIsReadWrite(VarTypeEntered, VarNumEntered)) {
...
void DeleteCurrentRung() {
    ...
    if (!((InfosGene->CurrentRung == InfosGene->FirstRung) && (InfosGene->CurrentRung == InfosGene->LastRung))) {
        ...
        for (y = 0; y < RUNG_HEIGHT; y++) {
            for (x = 0; x < RUNG_WIDTH; x++) {
                if ((RungArray[OldCurrent].Element[x][y].Type == ELE_COMPAR) || (RungArray[OldCurrent].Element[x][y].Type == ELE_OUTPUT_OPERATE)) {
        ...
        for (ScanY = 0; ScanY < RUNG_HEIGHT; ScanY++) {
            for (ScanX = 0; ScanX < RUNG_WIDTH; ScanX++) {
                ...
                if (GetSizesOfAnElement(pRungToCheck->Element[ScanX][ScanY].Type, &RuleSizeX, &RuleSizeY)) {
                    ...
                    if ((*PosiX >= ScanX - RuleSizeX - 1) && (*PosiX <= ScanX) && (*PosiY >= ScanY) && (*PosiY <= ScanY + RuleSizeY - 1)) {
                        /* We've got it ! */
...
int VerifyConstraintsAndRulesForElement(short int NumEle, int PosiX, int PosiY) {
    ...
    if ((NumEle == ELE_OUTPUT) || (NumEle == ELE_OUTPUT_NOT) || (NumEle == ELE_OUTPUT_SET) || (NumEle == ELE_OUTPUT_RESET) || (NumEle == ELE_OUTPUT_JUMP) || (NumEle == ELE_OUTPUT_CALL) || (NumEle == ELE_OUTPUT_OPERATE))
...
int IsASimpleElement(int Element) {
    int Res = FALSE;
    if ((Element == ELE_INPUT) || (Element == ELE_INPUT_NOT) || (Element == ELE_RISING_INPUT) || (Element == ELE_FALLING_INPUT) || (Element == ELE_OUTPUT) || (Element == ELE_OUTPUT_NOT) || (Element == ELE_OUTPUT_SET) || (Element == ELE_OUTPUT_RESET) || (Element == ELE_OUTPUT_JUMP) || (Element == ELE_OUTPUT_CALL))
        Res = TRUE;
    return Res;
}
...
void EditElementInRung(double x, double y) {
    ...
    if (ConvertDoublesToRungCoor(x, y, &RungX, &RungY) && (EditDatas.NumElementSelectedInToolBar != -1)) {
        ...
        if (EditDatas.NumElementSelectedInToolBar != EDIT_CNX_WITH_TOP && EditDatas.NumElementSelectedInToolBar != EDIT_POINTER && EditDatas.NumElementSelectedInToolBar != EDIT_LONG_CONNECTION) {
