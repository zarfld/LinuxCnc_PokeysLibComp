...
void process_options(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        static const struct option long_options[] = {
            { "nogui", no_argument, 0, 'n' }, { "modmaster", no_argument, 0, 'm' }, { "modslave", no_argument, 0, 's' }, { "debug", no_argument, 0, 'd' }, { "modbus_port", required_argument, 0, 'p' }, { "newpath", required_argument, 0, 'f' }, { 0, 0, 0, 0 },
        };
...
int main(int argc, char *argv[]) {
    ...
    if (ClassicLadder_AllocAll()) {
        ...
        if (nogui == TRUE) {
            ...
            if (pathswitch) {
                rtapi_strxcpy(InfosGene->CurrentProjectFileName, NewPath);
            }
            InfosGene->LadderState = STATE_RUN;
