    ...
    for (NumObj = 0; NumObj < NBR_OBJECTS; NumObj++) {
        ...
        switch (NumObj) {
            ...
            case 18:
                ...
                if (ret >= sizeof(BuffLabel))
                    snprintf(BuffValue, sizeof(BuffValue), "<path too long>");
                    ...
                    default: {
                        switch (NumObj) {
                            ...
                            case 2:
                                ...
                                break;
                            case 3:
                                snprintf(BuffValue, sizeof(BuffValue), "%d", pConf->FirstChannel);
                                break;
                            case 4:
                                snprintf(BuffValue, sizeof(BuffValue), "%d", pConf->NbrConsecutivesChannels);
                                break;
                        }
                        {
    ...
    for (NumLine = 0; NumLine < NBR_MODBUS_MASTER_REQ; NumLine++) {
        ...
        switch (pConf->TypeReq) {
            case MODBUS_REQ_INPUTS_READ:
                MaxVars = GetSizeVarsForTypeVar(ModbusConfig.MapTypeForReadInputs);
                DoVerify = TRUE;
                break;
            case MODBUS_REQ_COILS_WRITE:
                MaxVars = GetSizeVarsForTypeVar(ModbusConfig.MapTypeForWriteCoils);
                DoVerify = TRUE;
                break;
            case MODBUS_REQ_COILS_READ:
                MaxVars = GetSizeVarsForTypeVar(ModbusConfig.MapTypeForReadCoils);
                DoVerify = TRUE;
                break;
            case MODBUS_REQ_INPUT_REGS_READ:
                MaxVars = GetSizeVarsForTypeVar(ModbusConfig.MapTypeForReadInputRegs);
                DoVerify = TRUE;
                break;
            case MODBUS_REQ_HOLD_REGS_WRITE:
                MaxVars = GetSizeVarsForTypeVar(ModbusConfig.MapTypeForWriteHoldRegs);
                DoVerify = TRUE;
                break;
            case MODBUS_REQ_HOLD_REGS_READ:
                MaxVars = GetSizeVarsForTypeVar(ModbusConfig.MapTypeForReadHoldRegs);
                DoVerify = TRUE;
                break;
        }
        if (DoVerify) {
...
void OpenConfigWindowGtk() {
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(nbook), CreateGeneralParametersPage(), gtk_label_new(_("Period/object info")));
...
#ifdef MODBUS_IO_MASTER
    gtk_notebook_append_page(GTK_NOTEBOOK(nbook), CreateModbusComParametersPage(), gtk_label_new(_("Modbus communication setup")));
    gtk_notebook_append_page(GTK_NOTEBOOK(nbook), CreateModbusModulesIO(), gtk_label_new(_("Modbus I/O register setup")));
#endif
    ...
    gtk_signal_connect(GTK_OBJECT(ConfigWindow), "destroy", GTK_SIGNAL_FUNC(GetSettings), NULL);
