...
enum { NUM_ARRAY, VAR_NAME, SYMBOL, COMMENT, NBR_INFOS };
...
void DisplaySymbols(void) {
    ...
    for (ScanSymb = 0; ScanSymb < NBR_SYMBOLS; ScanSymb++) {
        if (SymbolArray[ScanSymb].VarName[0] == '\0') {
            ...
            gtk_list_store_set(ListStore, &iter, NUM_ARRAY, ScanSymb, VAR_NAME, SymbolArray[ScanSymb].VarName, SYMBOL, SymbolArray[ScanSymb].Symbol, COMMENT, Tempbuf, -1);
            return;
        }
...
void Callback_TextEdited(GtkCellRendererText *cell, gchar *path_string, gchar *new_text, gpointer data) {
    (void)cell;
    ...
    gtk_tree_model_get(treemodel, &iter, NUM_ARRAY, &OffsetArray, -1);
    gtk_list_store_set(ListStore, &iter, data, new_text, -1);
...
void SymbolsInitGtk() {
    ...
    gtk_signal_connect(GTK_OBJECT(SymbolsWindow), "delete_event", GTK_SIGNAL_FUNC(SymbolsWindowDeleteEvent), 0);
