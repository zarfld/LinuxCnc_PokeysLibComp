...
*/
int32_t PK_PoRelay8_DeviceIdentification(sPoKeysDevice* device, sPoRelay8 Relay)
{
...
*/
int32_t PK_PoRelay8_ConfigurationRead(sPoKeysDevice* device, sPoRelay8 Relay) // Retrieves I2C bus activation status
{
    if (device == NULL) return PK_ERR_NOT_CONNECTED;
    int i;
    for (i = 0; i < 9; i++)
...
int32_t PK_PoRelay8_ConfigurationWrite(sPoKeysDevice* device, sPoRelay8 Relay)
{
    if (device == NULL) return PK_ERR_NOT_CONNECTED;
    int i;
    for (i = 0; i < 9; i++)
...
int32_t PK_PoRelay8_ConfigurationSave(sPoKeysDevice* device, sPoRelay8 Relay)
{
    if (device == NULL) return PK_ERR_NOT_CONNECTED;
...
int32_t PK_PoRelay8_SetOutputs(sPoKeysDevice* device, sPoRelay8 Relay)
{
    if (device == NULL) return PK_ERR_NOT_CONNECTED;

    if (Relay.SendOption == 0)
    {
        sPoKeysCANmsg cnMessage;
        cnMessage.id = 0x7B;
        cnMessage.type = 0x20;
    }
    else if (Relay.SendOption == 1)
    {
        uint8_t iDataLength;
        uint8_t* buffer[3];
        PK_I2CWriteStart(device, 0x7B, buffer, iDataLength)
    }
    ...
    chkSum = getChecksum(device->request);
    device->request[4] = chkSum;
    if (PK_CANWrite(device, cnMessage) != PK_OK) return PK_ERR_TRANSFER;
    return PK_OK;
}

int32_t PK_PoRelay8_SetOutputsArray(sPoKeysDevice* device, sPoRelay8 Relay)
{
    if (device == NULL) return PK_ERR_NOT_CONNECTED;
...
int32_t PK_PoRelay8_ReEnablePoExtBus(sPoKeysDevice* device, sPoRelay8 Relay)
{
    if (device == NULL) return PK_ERR_NOT_CONNECTED;
