...
typedef struct {
    ...
    hal_bit_t
        *index_enable; // pointer for "pin in bit encoder.#.index-enable[29]"

    hal_float_t
        scale; // pointer for "param rw float encoder.#.scale[29]  "The scale factor used to convert counts to position units. It is in “counts per position unit”""

    // Custom parameters for communication with PoKeys
    hal_u32_t
        encoderOptions; // pointer for "pin io unsigned Encoders.#.encoderOptions [26]"
    ...
    //                      bit 7: mapped to macro for direction B
    hal_u32_t
        channelApin; // pointer for "pin io unsigned Encoders.#.channelApin [26]"         // Channel A encoder pin"
    hal_u32_t
...
int PKEncoder_export_pins(char *prefix, long extra_arg, int id, int njoints,
    ...
    if (Encoder_data == NULL) {
        ...
        if (encoder_data == NULL) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "PK_Encoders: all_encoder_data_t malloc failed\n");
            return -1;
        }
    } else {
    ...
    if (r != 0) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "PoKeys: %s:%s: %s.encoder.deb.out failed\n", __FILE__,
                        __FUNCTION__, prefix);
        return r;
    }
    ...
    for (j = 0; j < (njoints); j++) {
        ...
        if (r != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "PoKeys: %s:%s: %s.encoder.%01d.count failed\n",
                            __FILE__, __FUNCTION__, prefix, j);
            return r;
        }

        rtapi_print_msg(RTAPI_MSG_DBG,
                        "PoKeys: %s:%s: %s.encoder.%01d.position\n", __FILE__,
        ...
        if (r != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "PoKeys: %s:%s: %s.encoder.%01d.reset failed\n",
                            __FILE__, __FUNCTION__, prefix, j);
            return r;
        }

        rtapi_print_msg(RTAPI_MSG_DBG,
                        "PoKeys: %s:%s: %s.encoder.%01d.index-enable\n",
                        __FILE__, __FUNCTION__, prefix, j);
        ...
        if (r != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
                "PoKeys: %s:%s: %s.encoder.%01d.index-enable failed\n",
                __FILE__, __FUNCTION__, prefix, j);
            return r;
        }
    }
...
int PKEncoder_export_params(char *prefix, long extra_arg, int id, int njoints) {
    ...
    for (j = 0; j < (njoints); j++) {
        ...
        rtapi_print_msg(RTAPI_MSG_DBG, "PoKeys: %s:%s: %s.encoder.%01d.scale\n",
                        __FILE__, __FUNCTION__, prefix, j);
        r = hal_param_float_newf(HAL_RW, &(addr->scale), id,
                                 "%s.encoder.%01d.scale", prefix, j);
        if (r != 0) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "PoKeys: %s:%s: %s.encoder.%01d.scale failed\n",
                            __FILE__, __FUNCTION__, prefix, j);
            return r;
        }

        rtapi_print_msg(RTAPI_MSG_DBG,
                        "PoKeys: %s:%s: %s.encoder.%01d.encoderOptions\n",
                        __FILE__, __FUNCTION__, prefix, j);
        r = hal_param_u32_newf(HAL_RW, &(addr->encoderOptions), id,
                               "%s.encoder.%01d.encoderOptions", prefix, j);
        if (r != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
                "PoKeys: %s:%s: %s.encoder.%01d.encoderOptions failed\n",
                __FILE__, __FUNCTION__, prefix, j);
            return r;
        }

        rtapi_print_msg(RTAPI_MSG_DBG,
                        "PoKeys: %s:%s: %s.encoder.%01d.channelApin\n",
                        __FILE__, __FUNCTION__, prefix, j);
        r = hal_param_u32_newf(HAL_RW, &(addr->channelApin), id,
                               "%s.encoder.%01d.channelApin", prefix, j);
        if (r != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
                "PoKeys: %s:%s: %s.encoder.%01d.channelApin failed\n", __FILE__,
                __FUNCTION__, prefix, j);
            return r;
        }

        rtapi_print_msg(RTAPI_MSG_DBG,
                        "PoKeys: %s:%s: %s.encoder.%01d.channelBpin\n",
                        __FILE__, __FUNCTION__, prefix, j);
        r = hal_param_u32_newf(HAL_RW, &(addr->channelBpin), id,
                               "%s.encoder.%01d.channelBpin", prefix, j);
        if (r != 0) {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
                "PoKeys: %s:%s: %s.encoder.%01d.channelBpin failed\n", __FILE__,
                __FUNCTION__, prefix, j);
            return r;
        }
    }
...
void PKEncoder_Update(sPoKeysDevice *dev) {
    ...
    rtapi_print_msg(RTAPI_MSG_DBG,
                    "PoKeys: %s:%s: info_iBasicEncoderCount = %d\n", __FILE__,
                    __FUNCTION__, dev->info.iBasicEncoderCount);
    if (dev->info.iBasicEncoderCount > 0) {
        *(encoder_data->encoder_deb_out) = 216;
        rtapi_print_msg(RTAPI_MSG_DBG,
                        "PoKeys: %s:%s: PK_EncoderValuesGet(dev)\n", __FILE__,
                        __FUNCTION__);
        if (PK_EncoderValuesGet(dev) == PK_OK) {
            rtapi_print_msg(RTAPI_MSG_DBG,
                            "PoKeys: %s:%s: PK_EncoderValuesGet(dev) OK\n",
                            __FILE__, __FUNCTION__);
#ifdef ULAPI
            usleep(sleepdur);
#endif
            ...
            *(encoder_data->encoder_deb_out) = 219;
            if (dev->info.iUltraFastEncoders) {
                rtapi_print_msg(RTAPI_MSG_DBG,
                                "PoKeys: %s:%s: read UltraFastEncoders\n",
                                __FILE__, __FUNCTION__);
#ifdef ULAPI
                usleep(sleepdur);
#endif
                *(encoder_data->encoder_deb_out) = 220;
                ...
                for (int i = dev->info.iBasicEncoderCount;
                     i < (dev->info.iBasicEncoderCount +
                          dev->info.iUltraFastEncoders);
                     i++) {
                    rtapi_print_msg(RTAPI_MSG_DBG,
                                    "PoKeys: %s:%s: UltraFastEncoders %d\n",
                                    __FILE__, __FUNCTION__, i);
            ...
            rtapi_print_msg(RTAPI_MSG_DBG,
                            "PoKeys: %s:%s: resetEncoders = %d\n", __FILE__,
                            __FUNCTION__, resetEncoders);
            if (resetEncoders == true) {
                *(encoder_data->encoder_deb_out) = 140;
                rtapi_print_msg(RTAPI_MSG_DBG,
                                "PoKeys: %s:%s: PK_EncoderValuesSet(dev)\n",
                                __FILE__, __FUNCTION__);
                if (PK_EncoderValuesSet(dev) == PK_OK) {
                    rtapi_print_msg(
                        RTAPI_MSG_DBG,
                        "PoKeys: %s:%s: PK_EncoderValuesSet(dev) OK\n",
                        __FILE__, __FUNCTION__);
#ifdef ULAPI
                    usleep(sleepdur);
#endif
...
MODULE_INFO(
    linuxcnc,
    "param:encoder.#.scale:float:29:rw:The scale factor used to convert counts "
