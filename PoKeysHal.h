...
typedef struct
{
    uint8_t         AxesState;              // Axis states (bit-mapped) - see ePK_PEAxisState
    uint8_t         AxesConfig;             // Axis configuration - see ePK_PEv2_AxisConfig
    uint8_t         AxesSwitchConfig;       // Axis switch configuration - see ePK_PEv2_AxisSwitchOptions
    int32_t         CurrentPosition;        // Current position
    int32_t         PositionSetup;          // Position to be set as current position
    int32_t         ReferencePositionSpeed; // Reference position or speed (position or pulses/s)
    int8_t          InvertAxisEnable;       // Invert axis enable signal

    int32_t         SoftLimitMaximum;       // Soft limit maximum position
    int32_t         SoftLimitMinimum;       // Soft limit minimum position

    uint8_t         HomingSpeed;            // Homing speed per axis (in %)
    uint8_t         HomingReturnSpeed;      // Homing return speed per axis (in % of the homing speed)

    int32_t         HomeOffsets;            // Home position offset
    uint8_t			HomingAlgorithm;		   // Homing algorithm configuration

    uint8_t			FilterLimitMSwitch;	   // Digital filter for limit- switch
    uint8_t			FilterLimitPSwitch;	   // Digital filter for limit+ switch
    uint8_t			FilterHomeSwitch;	   // Digital filter for home switch

    int32_t         ProbePosition;          // Position where probe detected change
    int32_t         ProbeMaxPosition;       // Maximum position to travel to until stopping and returning error
    float           MaxSpeed;               // Maximum axis speed (in pulses per ms)
    float           MaxAcceleration;        // Maximum axis acceleration (in pulses/ms/ms)
    float           MaxDecceleration;       // Maximum axis deceleration (in pulses/ms/ms)


    int32_t         MPGjogMultiplier;       // MPG jog multiplier value
    uint8_t         MPGjogEncoder;          // MPG jog encoder ID
    uint8_t         PinHomeSwitch;          // Home switch pin (0 for external dedicated input)
    uint8_t         PinLimitMSwitch;        // Limit- switch pin (0 for external dedicated input)
    uint8_t         PinLimitPSwitch;        // Limit+ switch pin (0 for external dedicated input)
    uint8_t         AxisEnableOutputPins;   // Axis enabled output pin (0 for external dedicated output)
    uint32_t		HomeBackOffDistance;	   // Back-off distance after homing
    uint16_t        MPGjogDivider;          // Divider for the MPG jogging (enhanced encoder resolution)
    uint8_t         reserved;               // Motion buffer entries - moved further down...
    uint8_t         ReservedSafety;

    // ------ 64-bit region boundary ------

    hal_bit_t         ChargePumpEnabled;         // Charge pump output enabled
    hal_bit_t         EmergencySwitchPolarity;   // Emergency switch polarity (set to 1 to invert)

    hal_bit_t         PulseEngineActivated;      // Pulse engine activation status
    hal_bit_t         LimitStatusP;              // Limit+ status (bit-mapped)
    hal_bit_t         LimitStatusN;              // Limit- status (bit-mapped)
    hal_bit_t         HomeStatus;                // Home status (bit-mapped)

    hal_bit_t         ErrorInputStatus;          // Stepper motor driver error inputs status (bit-mapped)



    hal_bit_t         AxisEnabledMask;           // Bit-mapped ouput enabled mask


    hal_bit_t         AxisEnabledStatesMask;     // Bit-mapped states, where axis enabled and charge pump signals are active

    hal_bit_t         SoftLimitStatus;           // Bit-mapped soft-limit statuses per axes




    hal_bit_t         HomingStartMaskSetup;      // Bit-mapped axes indexes to be homed
    hal_bit_t         ProbeStartMaskSetup;       // Bit-mapped axis indexes for probing
    ...
    uint16_t		BacklashWidth;			// half of real backlash width
    int16_t			BacklashRegister;		// current value of the backlash register
    uint8_t			BacklashAcceleration;	// in pulses per ms^2
    hal_bit_t			BacklashCompensationEnabled;




    hal_bit_t			DedicatedLimitNInput;
    hal_bit_t			DedicatedLimitPInput;
    hal_bit_t			DedicatedHomeInput;
    hal_bit_t			TriggerIngnoredAxis;
